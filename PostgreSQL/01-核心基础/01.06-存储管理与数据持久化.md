...
flowchart TD
    ...
    B -->|点查询为主<br/>SELECT * WHERE id = ?| C[行存储 ✅]
    ...
    J --> J1[热数据: 行存储<br/>最近数据]
    J --> J2[冷数据: 列存储<br/>历史数据]
...
```mermaid
graph TB
    subgraph "PostgreSQL列存储架构"
        A[应用层] --> B[查询优化器]
        B --> C{存储类型}

        C -->|行存储| D[堆文件<br/>Heap File]
        C -->|列存储| E[cstore_fdw扩展]

        D --> D1[页面1: 行1, 行2, 行3...]
        D --> D2[页面2: 行4, 行5, 行6...]

        E --> E1[列文件1<br/>Column File 1]
        E --> E2[列文件2<br/>Column File 2]
        E --> E3[列文件N<br/>Column File N]
...
sequenceDiagram
    ...
    Query Optimizer->>Query Optimizer: 分析查询，识别列存储表
    Query Optimizer->>Query Optimizer: 优化：只读取需要的列
    Query Optimizer->>cstore_fdw: 执行查询计划

    cstore_fdw->>Column Files: 读取col1列文件
    Column Files->>Compression: 解压缩col1数据
    Compression-->>Column Files: 返回col1数据

    cstore_fdw->>Column Files: 读取col2列文件
    Column Files->>Compression: 解压缩col2数据
    Compression-->>Column Files: 返回col2数据

    cstore_fdw->>cstore_fdw: 执行GROUP BY和SUM聚合
    cstore_fdw-->>Query Optimizer: 返回聚合结果
    Query Optimizer-->>Client: 返回最终结果

    Note over cstore_fdw,Column Files: 只读取需要的列，I/O减少50-90%
    Note over Compression: 列压缩率70-90%
```

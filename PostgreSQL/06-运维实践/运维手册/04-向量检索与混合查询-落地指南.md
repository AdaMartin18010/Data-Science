...
- [向量检索与混合查询-落地指南（Runbook）](#向量检索与混合查询-落地指南runbook)
  ...
  - [1. 目标](#1-目标)
  - [2. 前置](#2-前置)
    - [2.1 pgvector安装](#21-pgvector安装)
    - [2.2 启用扩展](#22-启用扩展)
    - [2.3 准备数据表](#23-准备数据表)
  - [3. 步骤](#3-步骤)
    - [3.1 索引类型选择](#31-索引类型选择)
    - [3.2 在线查询（结构化 + 向量）](#32-在线查询结构化--向量)
    - [3.3 全文 + 向量融合](#33-全文--向量融合)
  - [4. 运行参数](#4-运行参数)
    - [4.1 性能参数调优](#41-性能参数调优)
    - [4.2 性能监控](#42-性能监控)
    - [4.3 性能基准测试](#43-性能基准测试)
  - [5. 验证](#5-验证)
    - [5.1 召回率验证](#51-召回率验证)
    - [5.2 性能基线记录](#52-性能基线记录)
  - [6. 故障排查](#6-故障排查)
    - [6.1 常见问题](#61-常见问题)
  - [7. 最佳实践](#7-最佳实践)
...
- 安装 `pgvector`；准备 `docs(id, text, embedding vector(n))`；选择索引（HNSW/IVFFlat/PQ）并 ANALYZE。

### 2.1 pgvector安装

```bash
# 方法1: 使用apt安装（Ubuntu/Debian）
...
WITH (
    m = 32,              -- 每个节点的连接数（16-64）
    ef_construction = 200  -- 构建时的候选数（100-1000）
);

-- 查询时设置ef_search（影响召回率和性能）
SET ivfflat.probes = 100;  -- 注意：HNSW使用ef_search参数
```
...
WITH (
    lists = 100  -- 聚类中心数（建议：rows/1000）
);

-- 查询时设置probes
SET ivfflat.probes = 10;  -- 搜索的聚类数（1-lists）
```
...
| 场景 | 推荐索引 | 原因 |
|------|---------|------|
| 高维向量(>100维) | HNSW | 更好的召回率 |
| 大规模数据(>100万) | IVFFlat | 更快的构建速度 |
| 实时查询 | HNSW | 更低的延迟 |
| 批量查询 | IVFFlat | 更好的吞吐量 |
...
```sql
-- 1. 调整work_mem（向量查询需要更多内存）
SET work_mem = '256MB';

-- 2. 调整IVFFlat probes（召回率 vs 性能权衡）
SET ivfflat.probes = 10;   -- 默认值，快速但召回率较低
SET ivfflat.probes = 50;   -- 平衡模式
SET ivfflat.probes = 100;  -- 高召回率，但较慢

-- 3. 调整HNSW ef_search（仅HNSW索引）
-- 注意：pgvector的HNSW实现可能使用不同参数名
-- 查询时在ORDER BY子句中指定LIMIT会影响搜索范围
...
-- 解决方案：增加work_mem或减少查询范围
SET work_mem = '512MB';
```

## 7. 最佳实践

1. **索引选择**: 高维向量用HNSW，大规模数据用IVFFlat
2. **参数调优**: 根据召回率和性能需求平衡probes/ef_search
3. **混合查询**: 先结构化过滤，再向量搜索，最后融合排序
4. **监控指标**: 定期检查查询延迟、召回率、索引使用情况
5. **定期维护**: 定期VACUUM和ANALYZE，重建索引以优化性能

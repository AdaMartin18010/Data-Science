...
| 特性 | 行存储 | 列存储 |
|-----|--------|--------|
| **数据组织** | 按行存储，一行数据连续存储 | 按列存储，一列数据连续存储 |
| **查询模式** | 适合点查询、事务处理 | 适合分析查询、聚合操作 |
| **I/O效率** | 查询多列时效率高 | 查询少列时效率高 |
| **压缩率** | 10-30% | 70-90% |
| **更新性能** | 支持高效更新 | 更新性能差（通常只读） |
| **适用场景** | OLTP、点查询、事务处理 | OLAP、分析查询、数据仓库 |
...
查询模式
  ...
  │   └─ 行存储 ✅
  │
  ├─ 分析查询为主（SELECT col1, col2, SUM(col3) GROUP BY col1）
  │   ├─ 数据量 < 100GB
  │   │   └─ 行存储 + 索引 ✅
  │   │
  │   ├─ 数据量 100GB - 1TB

# PostgreSQL云原生与容器化部署完整指南

> **版本**: v3.0
> **最后更新**: 2025-01-15
> **版本覆盖**: PostgreSQL 18.x (推荐) ⭐ | 17.x (推荐) | 16.x (兼容)
> **难度**: ⭐⭐⭐⭐
> **应用场景**: 容器化部署、Kubernetes编排、云原生应用、微服务架构

---

## 📑 目录

- [PostgreSQL云原生与容器化部署完整指南](#postgresql云原生与容器化部署完整指南)
  - [📑 目录](#-目录)
  - [一、概述](#一概述)
    - [1.1 云原生概念](#11-云原生概念)
    - [1.2 容器化优势](#12-容器化优势)
    - [1.3 适用场景](#13-适用场景)
  - [二、Docker容器化部署](#二docker容器化部署)
    - [2.1 Docker基础部署](#21-docker基础部署)
    - [2.2 Docker Compose部署](#22-docker-compose部署)
    - [2.3 自定义镜像构建](#23-自定义镜像构建)
    - [2.4 数据持久化](#24-数据持久化)
    - [2.5 网络配置](#25-网络配置)
    - [2.6 健康检查](#26-健康检查)
  - [三、Kubernetes部署](#三kubernetes部署)
    - [3.1 StatefulSet部署](#31-statefulset部署)
    - [3.2 使用PostgreSQL Operator](#32-使用postgresql-operator)
    - [3.3 Helm Chart部署](#33-helm-chart部署)
    - [3.4 存储配置](#34-存储配置)
    - [3.5 服务发现](#35-服务发现)
  - [四、高可用配置](#四高可用配置)
    - [4.1 主从复制](#41-主从复制)
    - [4.2 Patroni高可用](#42-patroni高可用)
    - [4.3 自动故障转移](#43-自动故障转移)
  - [五、监控与日志](#五监控与日志)
    - [5.1 Prometheus监控](#51-prometheus监控)
    - [5.2 日志收集](#52-日志收集)
    - [5.3 告警配置](#53-告警配置)
  - [六、备份与恢复](#六备份与恢复)
    - [6.1 容器内备份](#61-容器内备份)
    - [6.2 持久化卷备份](#62-持久化卷备份)
    - [6.3 自动化备份](#63-自动化备份)
  - [七、安全配置](#七安全配置)
    - [7.1 镜像安全](#71-镜像安全)
    - [7.2 网络安全](#72-网络安全)
    - [7.3 密钥管理](#73-密钥管理)
  - [八、性能优化](#八性能优化)
    - [8.1 资源限制](#81-资源限制)
    - [8.2 存储优化](#82-存储优化)
    - [8.3 网络优化](#83-网络优化)
  - [九、知识矩阵对比](#九知识矩阵对比)
    - [9.1 部署方案对比](#91-部署方案对比)
    - [9.2 Operator对比](#92-operator对比)
    - [9.3 存储方案对比](#93-存储方案对比)
  - [十、实践案例](#十实践案例)
    - [10.1 开发环境部署](#101-开发环境部署)
    - [10.2 生产环境部署](#102-生产环境部署)
    - [10.3 多租户部署](#103-多租户部署)
  - [十一、故障排查](#十一故障排查)
    - [11.1 常见问题](#111-常见问题)
    - [11.2 诊断方法](#112-诊断方法)
    - [11.3 解决方案](#113-解决方案)
  - [十二、最佳实践](#十二最佳实践)
    - [12.1 部署最佳实践](#121-部署最佳实践)
    - [12.2 安全最佳实践](#122-安全最佳实践)
    - [12.3 运维最佳实践](#123-运维最佳实践)
  - [十三、参考资源](#十三参考资源)
    - [13.1 官方文档](#131-官方文档)
    - [13.2 网络资源](#132-网络资源)
    - [13.3 相关文档](#133-相关文档)

---

## 一、概述

### 1.1 云原生概念

云原生是一种构建和运行应用程序的方法，充分利用云计算交付模型的优势。PostgreSQL云原生部署包括：

- **容器化**：使用Docker等容器技术
- **编排**：使用Kubernetes等编排平台
- **微服务**：支持微服务架构
- **自动化**：自动化部署、扩展和管理
- **可观测性**：监控、日志和追踪

### 1.2 容器化优势

**容器化部署的优势**：

- ✅ **环境一致性**：开发、测试、生产环境一致
- ✅ **快速部署**：秒级启动和停止
- ✅ **资源隔离**：进程和资源隔离
- ✅ **易于扩展**：水平扩展和自动伸缩
- ✅ **版本管理**：轻松切换版本
- ✅ **简化运维**：统一的部署和管理方式

### 1.3 适用场景

**容器化部署适用场景**：

- **微服务架构**：每个服务独立数据库实例
- **开发测试环境**：快速搭建和销毁
- **CI/CD流水线**：自动化测试和部署
- **云原生应用**：Kubernetes集群部署
- **多租户应用**：隔离的数据库实例

---

## 二、Docker容器化部署

### 2.1 Docker基础部署

**拉取PostgreSQL镜像**：

```bash
# 拉取PostgreSQL 18官方镜像
docker pull postgres:18

# 查看可用标签
docker search postgres

# 查看镜像信息
docker inspect postgres:18
```

**运行PostgreSQL容器**：

```bash
# 基本运行
docker run --name postgres \
  -e POSTGRES_PASSWORD=mysecretpassword \
  -e POSTGRES_USER=postgres \
  -e POSTGRES_DB=mydb \
  -p 5432:5432 \
  -d postgres:18

# 查看运行状态
docker ps

# 查看日志
docker logs postgres
docker logs -f postgres  # 实时查看

# 停止容器
docker stop postgres

# 启动容器
docker start postgres

# 删除容器
docker rm postgres
```

**环境变量配置**：

```bash
# 完整环境变量配置
docker run --name postgres \
  -e POSTGRES_PASSWORD=mysecretpassword \
  -e POSTGRES_USER=postgres \
  -e POSTGRES_DB=mydb \
  -e POSTGRES_INITDB_ARGS="--encoding=UTF8 --locale=zh_CN.UTF-8" \
  -e POSTGRES_HOST_AUTH_METHOD=scram-sha-256 \
  -p 5432:5432 \
  -d postgres:18
```

**常用环境变量**：

- `POSTGRES_PASSWORD`：PostgreSQL超级用户密码（必需）
- `POSTGRES_USER`：PostgreSQL用户名（默认：postgres）
- `POSTGRES_DB`：初始数据库名（默认：postgres）
- `POSTGRES_INITDB_ARGS`：initdb参数
- `POSTGRES_HOST_AUTH_METHOD`：认证方法（md5/scram-sha-256）

### 2.2 Docker Compose部署

**基础docker-compose.yml**：

```yaml
version: '3.8'

services:
  postgres:
    image: postgres:18
    container_name: postgres
    environment:
      POSTGRES_USER: postgres
      POSTGRES_PASSWORD: mysecretpassword
      POSTGRES_DB: mydb
      POSTGRES_INITDB_ARGS: "--encoding=UTF8 --locale=zh_CN.UTF-8"
    ports:
      - "5432:5432"
    volumes:
      - postgres_data:/var/lib/postgresql/data
      - ./init.sql:/docker-entrypoint-initdb.d/init.sql
    networks:
      - postgres_network
    restart: unless-stopped
    healthcheck:
      test: ["CMD-SHELL", "pg_isready -U postgres"]
      interval: 10s
      timeout: 5s
      retries: 5

volumes:
  postgres_data:
    driver: local

networks:
  postgres_network:
    driver: bridge
```

**完整docker-compose.yml（生产级）**：

```yaml
version: '3.8'

services:
  postgres:
    image: postgres:18
    container_name: postgres
    environment:
      POSTGRES_USER: ${POSTGRES_USER:-postgres}
      POSTGRES_PASSWORD: ${POSTGRES_PASSWORD:-changeme}
      POSTGRES_DB: ${POSTGRES_DB:-mydb}
      POSTGRES_INITDB_ARGS: "--encoding=UTF8 --locale=zh_CN.UTF-8 --data-checksums"
      POSTGRES_HOST_AUTH_METHOD: scram-sha-256
    ports:
      - "${POSTGRES_PORT:-5432}:5432"
    volumes:
      - postgres_data:/var/lib/postgresql/data
      - postgres_config:/etc/postgresql
      - ./init:/docker-entrypoint-initdb.d
      - ./backups:/backups
    networks:
      - postgres_network
    restart: unless-stopped
    healthcheck:
      test: ["CMD-SHELL", "pg_isready -U ${POSTGRES_USER:-postgres} -d ${POSTGRES_DB:-mydb}"]
      interval: 10s
      timeout: 5s
      retries: 5
      start_period: 30s
    deploy:
      resources:
        limits:
          cpus: '2.0'
          memory: 4G
        reservations:
          cpus: '1.0'
          memory: 2G
    logging:
      driver: "json-file"
      options:
        max-size: "10m"
        max-file: "3"

  # pgAdmin（可选）
  pgadmin:
    image: dpage/pgadmin4:latest
    container_name: pgadmin
    environment:
      PGADMIN_DEFAULT_EMAIL: admin@example.com
      PGADMIN_DEFAULT_PASSWORD: admin
      PGADMIN_CONFIG_SERVER_MODE: 'False'
    ports:
      - "5050:80"
    volumes:
      - pgadmin_data:/var/lib/pgadmin
    networks:
      - postgres_network
    depends_on:
      - postgres
    restart: unless-stopped

volumes:
  postgres_data:
    driver: local
  postgres_config:
    driver: local
  pgadmin_data:
    driver: local

networks:
  postgres_network:
    driver: bridge
```

**使用Docker Compose**：

```bash
# 启动服务
docker-compose up -d

# 查看服务状态
docker-compose ps

# 查看日志
docker-compose logs -f postgres

# 停止服务
docker-compose stop

# 停止并删除容器
docker-compose down

# 停止并删除容器和卷
docker-compose down -v

# 重新构建并启动
docker-compose up -d --build
```

### 2.3 自定义镜像构建

**Dockerfile示例**：

```dockerfile
# 基于PostgreSQL 18官方镜像
FROM postgres:18

# 设置维护者
LABEL maintainer="your-email@example.com"

# 安装额外工具
RUN apt-get update && \
    apt-get install -y \
    postgresql-contrib-18 \
    postgresql-18-pgvector \
    vim \
    curl \
    && rm -rf /var/lib/apt/lists/*

# 复制自定义配置
COPY postgresql.conf /etc/postgresql/postgresql.conf
COPY pg_hba.conf /etc/postgresql/pg_hba.conf

# 复制初始化脚本
COPY init-scripts/ /docker-entrypoint-initdb.d/

# 设置权限
RUN chmod +x /docker-entrypoint-initdb.d/*.sh

# 暴露端口
EXPOSE 5432

# 使用自定义配置启动
CMD ["postgres", "-c", "config_file=/etc/postgresql/postgresql.conf"]
```

**构建和运行自定义镜像**：

```bash
# 构建镜像
docker build -t my-postgres:18 .

# 运行自定义镜像
docker run --name my-postgres \
  -e POSTGRES_PASSWORD=mysecretpassword \
  -p 5432:5432 \
  -d my-postgres:18

# 推送到镜像仓库
docker tag my-postgres:18 registry.example.com/my-postgres:18
docker push registry.example.com/my-postgres:18
```

**多阶段构建优化**：

```dockerfile
# 构建阶段
FROM postgres:18 AS builder

# 安装编译工具
RUN apt-get update && \
    apt-get install -y \
    build-essential \
    postgresql-server-dev-18 \
    && rm -rf /var/lib/apt/lists/*

# 编译扩展
WORKDIR /tmp
RUN git clone https://github.com/pgvector/pgvector.git && \
    cd pgvector && \
    make && \
    make install

# 运行阶段
FROM postgres:18

# 从构建阶段复制扩展
COPY --from=builder /usr/share/postgresql/18/extension/pgvector* /usr/share/postgresql/18/extension/
COPY --from=builder /usr/lib/postgresql/18/lib/vector.so /usr/lib/postgresql/18/lib/

# 复制配置和脚本
COPY postgresql.conf /etc/postgresql/postgresql.conf
COPY init-scripts/ /docker-entrypoint-initdb.d/

EXPOSE 5432
```

### 2.4 数据持久化

**使用命名卷（推荐）**：

```yaml
version: '3.8'

services:
  postgres:
    image: postgres:18
    volumes:
      - postgres_data:/var/lib/postgresql/data

volumes:
  postgres_data:
    driver: local
```

**使用绑定挂载**：

```yaml
version: '3.8'

services:
  postgres:
    image: postgres:18
    volumes:
      - ./data:/var/lib/postgresql/data
      - ./config:/etc/postgresql
```

**数据卷管理**：

```bash
# 查看数据卷
docker volume ls

# 查看数据卷详情
docker volume inspect postgres_data

# 备份数据卷
docker run --rm \
  -v postgres_data:/data \
  -v $(pwd):/backup \
  alpine tar czf /backup/postgres_backup.tar.gz /data

# 恢复数据卷
docker run --rm \
  -v postgres_data:/data \
  -v $(pwd):/backup \
  alpine tar xzf /backup/postgres_backup.tar.gz -C /
```

### 2.5 网络配置

**自定义网络**：

```bash
# 创建自定义网络
docker network create postgres_network

# 运行PostgreSQL容器
docker run --name postgres \
  --network postgres_network \
  -e POSTGRES_PASSWORD=mysecretpassword \
  -d postgres:18

# 运行应用容器（连接到同一网络）
docker run --name myapp \
  --network postgres_network \
  -e DATABASE_URL=postgresql://postgres:mysecretpassword@postgres:5432/mydb \
  -d myapp:latest
```

**Docker Compose网络配置**：

```yaml
version: '3.8'

services:
  postgres:
    image: postgres:18
    networks:
      - backend

  app:
    image: myapp:latest
    networks:
      - backend
    depends_on:
      - postgres

networks:
  backend:
    driver: bridge
    ipam:
      config:
        - subnet: 172.20.0.0/16
```

### 2.6 健康检查

**健康检查配置**：

```yaml
version: '3.8'

services:
  postgres:
    image: postgres:18
    healthcheck:
      test: ["CMD-SHELL", "pg_isready -U postgres -d mydb"]
      interval: 10s
      timeout: 5s
      retries: 5
      start_period: 30s
```

**自定义健康检查脚本**：

```bash
#!/bin/bash
# healthcheck.sh

set -e

# 检查PostgreSQL是否就绪
pg_isready -U postgres -d mydb

# 检查数据库连接
psql -U postgres -d mydb -c "SELECT 1" > /dev/null

# 检查WAL归档（如果启用）
# pg_archivecheck /path/to/wal

exit 0
```

---

## 三、Kubernetes部署

### 3.1 StatefulSet部署

**StatefulSet配置**：

```yaml
apiVersion: apps/v1
kind: StatefulSet
metadata:
  name: postgres
  namespace: postgres
spec:
  serviceName: postgres
  replicas: 1
  selector:
    matchLabels:
      app: postgres
  template:
    metadata:
      labels:
        app: postgres
    spec:
      containers:
      - name: postgres
        image: postgres:18
        env:
        - name: POSTGRES_PASSWORD
          valueFrom:
            secretKeyRef:
              name: postgres-secret
              key: postgres-password
        - name: POSTGRES_USER
          value: "postgres"
        - name: POSTGRES_DB
          value: "mydb"
        - name: PGDATA
          value: /var/lib/postgresql/data/pgdata
        ports:
        - containerPort: 5432
          name: postgres
        volumeMounts:
        - name: postgres-storage
          mountPath: /var/lib/postgresql/data
        - name: postgres-config
          mountPath: /etc/postgresql
        resources:
          requests:
            memory: "1Gi"
            cpu: "500m"
          limits:
            memory: "2Gi"
            cpu: "2000m"
        livenessProbe:
          exec:
            command:
            - pg_isready
            - -U
            - postgres
          initialDelaySeconds: 30
          periodSeconds: 10
        readinessProbe:
          exec:
            command:
            - pg_isready
            - -U
            - postgres
          initialDelaySeconds: 5
          periodSeconds: 5
      volumes:
      - name: postgres-config
        configMap:
          name: postgres-config
  volumeClaimTemplates:
  - metadata:
      name: postgres-storage
    spec:
      accessModes: [ "ReadWriteOnce" ]
      storageClassName: fast-ssd
      resources:
        requests:
          storage: 10Gi
```

**ConfigMap配置**：

```yaml
apiVersion: v1
kind: ConfigMap
metadata:
  name: postgres-config
  namespace: postgres
data:
  postgresql.conf: |
    # 连接设置
    listen_addresses = '*'
    port = 5432
    max_connections = 100

    # 内存设置
    shared_buffers = 256MB
    effective_cache_size = 1GB
    work_mem = 4MB
    maintenance_work_mem = 64MB

    # WAL设置
    wal_level = replica
    max_wal_size = 1GB
    min_wal_size = 80MB

    # 日志设置
    logging_collector = on
    log_directory = 'log'
    log_filename = 'postgresql-%Y-%m-%d.log'
    log_min_duration_statement = 1000
```

**Secret配置**：

```yaml
apiVersion: v1
kind: Secret
metadata:
  name: postgres-secret
  namespace: postgres
type: Opaque
stringData:
  postgres-password: mysecretpassword
  replication-password: replicationpassword
```

**Service配置**：

```yaml
apiVersion: v1
kind: Service
metadata:
  name: postgres
  namespace: postgres
spec:
  clusterIP: None
  selector:
    app: postgres
  ports:
  - port: 5432
    name: postgres
---
apiVersion: v1
kind: Service
metadata:
  name: postgres-read
  namespace: postgres
spec:
  selector:
    app: postgres
  ports:
  - port: 5432
    name: postgres
```

**部署命令**：

```bash
# 创建命名空间
kubectl create namespace postgres

# 创建Secret
kubectl apply -f postgres-secret.yaml

# 创建ConfigMap
kubectl apply -f postgres-configmap.yaml

# 创建StatefulSet
kubectl apply -f postgres-statefulset.yaml

# 创建Service
kubectl apply -f postgres-service.yaml

# 查看状态
kubectl get pods -n postgres
kubectl get svc -n postgres
kubectl get pvc -n postgres

# 查看日志
kubectl logs -f postgres-0 -n postgres

# 连接数据库
kubectl exec -it postgres-0 -n postgres -- psql -U postgres -d mydb
```

### 3.2 使用PostgreSQL Operator

**安装Crunchy PostgreSQL Operator**：

```bash
# 添加Helm仓库
helm repo add postgres-operator https://opensource.crunchydata.com/postgres-operator
helm repo update

# 安装Operator
helm install postgres-operator postgres-operator/postgres-operator \
  --namespace postgres-operator \
  --create-namespace \
  --set image.repository=registry.developers.crunchydata.com/crunchydata/postgres-operator \
  --set image.tag=ubi8-5.4.0-0

# 验证安装
kubectl get pods -n postgres-operator
```

**创建PostgreSQL集群**：

```yaml
apiVersion: postgres-operator.crunchydata.com/v1beta1
kind: PostgresCluster
metadata:
  name: postgres-cluster
  namespace: postgres
spec:
  image: registry.developers.crunchydata.com/crunchydata/crunchy-postgres:ubi8-18.0-0
  postgresVersion: 18
  instances:
    - name: instance1
      replicas: 3
      resources:
        requests:
          memory: 1Gi
          cpu: 500m
        limits:
          memory: 2Gi
          cpu: 2000m
      dataVolumeClaimSpec:
        accessModes:
        - ReadWriteOnce
        resources:
          requests:
            storage: 10Gi
  backups:
    pgbackrest:
      image: registry.developers.crunchydata.com/crunchydata/crunchy-pgbackrest:ubi8-2.48-0
      repos:
      - name: repo1
        volume:
          volumeClaimSpec:
            accessModes:
            - ReadWriteOnce
            resources:
              requests:
                storage: 20Gi
  proxy:
    pgBouncer:
      image: registry.developers.crunchydata.com/crunchydata/crunchy-pgbouncer:ubi8-1.19-0
      replicas: 2
      resources:
        requests:
          memory: 64Mi
          cpu: 100m
```

**应用配置**：

```bash
# 创建集群
kubectl apply -f postgres-cluster.yaml

# 查看集群状态
kubectl get postgresclusters -n postgres

# 查看Pod状态
kubectl get pods -n postgres -l postgres-operator.crunchydata.com/cluster=postgres-cluster

# 获取连接信息
kubectl get secrets postgres-cluster-pguser-postgres -n postgres -o jsonpath='{.data.password}' | base64 -d
```

### 3.3 Helm Chart部署

**使用Bitnami Helm Chart**：

```bash
# 添加Helm仓库
helm repo add bitnami https://charts.bitnami.com/bitnami
helm repo update

# 安装PostgreSQL
helm install postgres bitnami/postgresql \
  --namespace postgres \
  --create-namespace \
  --set auth.postgresPassword=mysecretpassword \
  --set auth.database=mydb \
  --set primary.persistence.size=10Gi \
  --set metrics.enabled=true

# 查看状态
helm list -n postgres
kubectl get pods -n postgres

# 获取连接信息
kubectl get secret postgres-postgresql -n postgres -o jsonpath='{.data.postgres-password}' | base64 -d
```

**自定义values.yaml**：

```yaml
# values.yaml
auth:
  postgresPassword: mysecretpassword
  database: mydb
  username: myuser
  password: myuserpassword

primary:
  persistence:
    enabled: true
    size: 20Gi
    storageClass: fast-ssd
  resources:
    requests:
      memory: 2Gi
      cpu: 1000m
    limits:
      memory: 4Gi
      cpu: 2000m

readReplicas:
  replicaCount: 2
  persistence:
    enabled: true
    size: 20Gi

metrics:
  enabled: true
  serviceMonitor:
    enabled: true

postgresql:
  shared_buffers: 256MB
  effective_cache_size: 1GB
  work_mem: 4MB
```

**使用自定义values部署**：

```bash
helm install postgres bitnami/postgresql \
  --namespace postgres \
  --create-namespace \
  -f values.yaml
```

### 3.4 存储配置

**StorageClass配置**：

```yaml
apiVersion: storage.k8s.io/v1
kind: StorageClass
metadata:
  name: fast-ssd
provisioner: kubernetes.io/aws-ebs
parameters:
  type: gp3
  fsType: ext4
  encrypted: "true"
volumeBindingMode: WaitForFirstConsumer
allowVolumeExpansion: true
```

**PersistentVolumeClaim配置**：

```yaml
apiVersion: v1
kind: PersistentVolumeClaim
metadata:
  name: postgres-pvc
  namespace: postgres
spec:
  accessModes:
    - ReadWriteOnce
  storageClassName: fast-ssd
  resources:
    requests:
      storage: 50Gi
```

**存储扩展**：

```bash
# 扩展PVC
kubectl patch pvc postgres-pvc -n postgres -p '{"spec":{"resources":{"requests":{"storage":"100Gi"}}}}'

# 在PostgreSQL中扩展文件系统
kubectl exec -it postgres-0 -n postgres -- psql -U postgres -c "SELECT pg_size_pretty(pg_database_size('mydb'));"
```

### 3.5 服务发现

**Headless Service**：

```yaml
apiVersion: v1
kind: Service
metadata:
  name: postgres-headless
  namespace: postgres
spec:
  clusterIP: None
  selector:
    app: postgres
  ports:
  - port: 5432
    name: postgres
```

**DNS服务发现**：

```bash
# 在Pod中通过DNS访问
# postgres-0.postgres-headless.postgres.svc.cluster.local

# 应用连接示例
DATABASE_URL=postgresql://postgres:password@postgres-headless.postgres.svc.cluster.local:5432/mydb
```

---

## 四、高可用配置

### 4.1 主从复制

**主库配置（StatefulSet）**：

```yaml
apiVersion: apps/v1
kind: StatefulSet
metadata:
  name: postgres-primary
  namespace: postgres
spec:
  serviceName: postgres-primary
  replicas: 1
  template:
    spec:
      containers:
      - name: postgres
        image: postgres:18
        env:
        - name: POSTGRES_REPLICATION_USER
          value: replicator
        - name: POSTGRES_REPLICATION_PASSWORD
          valueFrom:
            secretKeyRef:
              name: postgres-secret
              key: replication-password
        volumeMounts:
        - name: postgres-config
          mountPath: /etc/postgresql
      volumes:
      - name: postgres-config
        configMap:
          name: postgres-primary-config
```

**主库ConfigMap**：

```yaml
apiVersion: v1
kind: ConfigMap
metadata:
  name: postgres-primary-config
  namespace: postgres
data:
  postgresql.conf: |
    wal_level = replica
    max_wal_senders = 10
    max_replication_slots = 10
    wal_keep_size = 1GB
  pg_hba.conf: |
    host replication replicator 0.0.0.0/0 scram-sha-256
```

**从库配置**：

```yaml
apiVersion: apps/v1
kind: StatefulSet
metadata:
  name: postgres-replica
  namespace: postgres
spec:
  serviceName: postgres-replica
  replicas: 2
  template:
    spec:
      initContainers:
      - name: init-replica
        image: postgres:18
        command:
        - /bin/bash
        - -c
        - |
          pg_basebackup -h postgres-primary.postgres.svc.cluster.local \
            -U replicator \
            -D /var/lib/postgresql/data \
            -X stream \
            -R \
            -C \
            -S replica-$(hostname)
        volumeMounts:
        - name: postgres-storage
          mountPath: /var/lib/postgresql/data
        env:
        - name: PGPASSWORD
          valueFrom:
            secretKeyRef:
              name: postgres-secret
              key: replication-password
      containers:
      - name: postgres
        image: postgres:18
        volumeMounts:
        - name: postgres-storage
          mountPath: /var/lib/postgresql/data
  volumeClaimTemplates:
  - metadata:
      name: postgres-storage
    spec:
      accessModes: [ "ReadWriteOnce" ]
      resources:
        requests:
          storage: 10Gi
```

### 4.2 Patroni高可用

**Patroni ConfigMap**：

```yaml
apiVersion: v1
kind: ConfigMap
metadata:
  name: patroni-config
  namespace: postgres
data:
  patroni.yml: |
    scope: postgres
    namespace: /service/postgres
    name: ${POD_NAME}

    restapi:
      listen: 0.0.0.0:8008
      connect_address: ${POD_IP}:8008

    bootstrap:
      dcs:
        ttl: 30
        loop_wait: 10
        retry_timeout: 30
        maximum_lag_on_failover: 1048576
        postgresql:
          use_pg_rewind: true
          parameters:
            wal_level: replica
            hot_standby: on
            max_connections: 100
            max_wal_senders: 10
            max_replication_slots: 10
            wal_keep_size: 1GB

    postgresql:
      listen: 0.0.0.0:5432
      connect_address: ${POD_IP}:5432
      data_dir: /var/lib/postgresql/data/pgdata
      pgpass: /tmp/pgpass
      authentication:
        replication:
          username: replicator
          password: ${REPLICATION_PASSWORD}
        superuser:
          username: postgres
          password: ${POSTGRES_PASSWORD}
      parameters:
        unix_socket_directories: '/var/run/postgresql'

    consul:
      host: consul.postgres.svc.cluster.local:8500
```

**Patroni StatefulSet**：

```yaml
apiVersion: apps/v1
kind: StatefulSet
metadata:
  name: patroni
  namespace: postgres
spec:
  serviceName: patroni
  replicas: 3
  selector:
    matchLabels:
      app: patroni
  template:
    metadata:
      labels:
        app: patroni
    spec:
      containers:
      - name: patroni
        image: patroni/patroni:latest
        env:
        - name: POD_NAME
          valueFrom:
            fieldRef:
              fieldPath: metadata.name
        - name: POD_IP
          valueFrom:
            fieldRef:
              fieldPath: status.podIP
        - name: POSTGRES_PASSWORD
          valueFrom:
            secretKeyRef:
              name: postgres-secret
              key: postgres-password
        - name: REPLICATION_PASSWORD
          valueFrom:
            secretKeyRef:
              name: postgres-secret
              key: replication-password
        volumeMounts:
        - name: patroni-config
          mountPath: /etc/patroni
        - name: postgres-storage
          mountPath: /var/lib/postgresql/data
      volumes:
      - name: patroni-config
        configMap:
          name: patroni-config
  volumeClaimTemplates:
  - metadata:
      name: postgres-storage
    spec:
      accessModes: [ "ReadWriteOnce" ]
      resources:
        requests:
          storage: 10Gi
```

### 4.3 自动故障转移

**故障转移配置**：

```yaml
# Patroni故障转移配置
apiVersion: v1
kind: ConfigMap
metadata:
  name: patroni-config
data:
  patroni.yml: |
    dcs:
      ttl: 30
      loop_wait: 10
      retry_timeout: 30
      maximum_lag_on_failover: 1048576
      master_start_timeout: 300
      synchronous_mode: false
      synchronous_mode_strict: false
      synchronous_node_count: 1
```

**测试故障转移**：

```bash
# 查看当前主节点
kubectl exec -it patroni-0 -n postgres -- patronictl list

# 模拟主节点故障
kubectl delete pod patroni-0 -n postgres

# 观察故障转移
kubectl exec -it patroni-1 -n postgres -- patronictl list
```

---

## 五、监控与日志

### 5.1 Prometheus监控

**PostgreSQL Exporter部署**：

```yaml
apiVersion: apps/v1
kind: Deployment
metadata:
  name: postgres-exporter
  namespace: postgres
spec:
  replicas: 1
  selector:
    matchLabels:
      app: postgres-exporter
  template:
    metadata:
      labels:
        app: postgres-exporter
    spec:
      containers:
      - name: postgres-exporter
        image: prometheuscommunity/postgres-exporter:latest
        env:
        - name: DATA_SOURCE_NAME
          value: "postgresql://postgres:password@postgres.postgres.svc.cluster.local:5432/mydb?sslmode=disable"
        ports:
        - containerPort: 9187
          name: metrics
```

**ServiceMonitor配置**：

```yaml
apiVersion: monitoring.coreos.com/v1
kind: ServiceMonitor
metadata:
  name: postgres-exporter
  namespace: postgres
spec:
  selector:
    matchLabels:
      app: postgres-exporter
  endpoints:
  - port: metrics
    interval: 30s
    path: /metrics
```

### 5.2 日志收集

**Fluentd日志收集**：

```yaml
apiVersion: v1
kind: ConfigMap
metadata:
  name: fluentd-config
  namespace: postgres
data:
  fluent.conf: |
    <source>
      @type tail
      path /var/log/postgresql/*.log
      pos_file /var/log/fluentd-postgres.log.pos
      tag postgres.log
      <parse>
        @type none
      </parse>
    </source>

    <match postgres.log>
      @type elasticsearch
      host elasticsearch.logging.svc.cluster.local
      port 9200
      index_name postgres
      type_name _doc
    </match>
```

### 5.3 告警配置

**Prometheus告警规则**：

```yaml
apiVersion: monitoring.coreos.com/v1
kind: PrometheusRule
metadata:
  name: postgres-alerts
  namespace: postgres
spec:
  groups:
  - name: postgres
    rules:
    - alert: PostgresDown
      expr: pg_up == 0
      for: 1m
      labels:
        severity: critical
      annotations:
        summary: "PostgreSQL instance is down"

    - alert: PostgresTooManyConnections
      expr: pg_stat_database_numbackends / pg_settings_max_connections > 0.8
      for: 5m
      labels:
        severity: warning
      annotations:
        summary: "PostgreSQL has too many connections"

    - alert: PostgresSlowQueries
      expr: rate(pg_stat_statements_mean_exec_time[5m]) > 1
      for: 5m
      labels:
        severity: warning
      annotations:
        summary: "PostgreSQL has slow queries"
```

---

## 六、备份与恢复

### 6.1 容器内备份

**CronJob备份**：

```yaml
apiVersion: batch/v1
kind: CronJob
metadata:
  name: postgres-backup
  namespace: postgres
spec:
  schedule: "0 2 * * *"  # 每天凌晨2点
  jobTemplate:
    spec:
      template:
        spec:
          containers:
          - name: postgres-backup
            image: postgres:18
            command:
            - /bin/bash
            - -c
            - |
              pg_dump -h postgres.postgres.svc.cluster.local \
                -U postgres \
                -d mydb \
                -F c \
                -f /backup/mydb_$(date +%Y%m%d_%H%M%S).dump
            env:
            - name: PGPASSWORD
              valueFrom:
                secretKeyRef:
                  name: postgres-secret
                  key: postgres-password
            volumeMounts:
            - name: backup-storage
              mountPath: /backup
          volumes:
          - name: backup-storage
            persistentVolumeClaim:
              claimName: postgres-backup-pvc
          restartPolicy: OnFailure
```

### 6.2 持久化卷备份

**卷快照备份**：

```yaml
apiVersion: snapshot.storage.k8s.io/v1
kind: VolumeSnapshot
metadata:
  name: postgres-snapshot
  namespace: postgres
spec:
  source:
    persistentVolumeClaimName: postgres-storage-postgres-0
  volumeSnapshotClassName: csi-snapshotter
```

### 6.3 自动化备份

**使用pgBackRest**：

```yaml
apiVersion: batch/v1
kind: CronJob
metadata:
  name: pgbackrest-backup
  namespace: postgres
spec:
  schedule: "0 */6 * * *"  # 每6小时
  jobTemplate:
    spec:
      template:
        spec:
          containers:
          - name: pgbackrest
            image: pgbackrest/pgbackrest:latest
            command:
            - pgbackrest
            - --stanza=postgres
            - backup
            - --type=incr
            volumeMounts:
            - name: postgres-storage
              mountPath: /var/lib/postgresql/data
            - name: backup-storage
              mountPath: /backup
          volumes:
          - name: postgres-storage
            persistentVolumeClaim:
              claimName: postgres-storage-postgres-0
          - name: backup-storage
            persistentVolumeClaim:
              claimName: postgres-backup-pvc
          restartPolicy: OnFailure
```

---

## 七、安全配置

### 7.1 镜像安全

**使用官方镜像**：

```bash
# 使用官方PostgreSQL镜像
image: postgres:18

# 避免使用latest标签
image: postgres:18.0

# 定期更新镜像
docker pull postgres:18
```

**镜像扫描**：

```bash
# 使用Trivy扫描镜像
trivy image postgres:18

# 使用Snyk扫描
snyk test --docker postgres:18
```

### 7.2 网络安全

**NetworkPolicy配置**：

```yaml
apiVersion: networking.k8s.io/v1
kind: NetworkPolicy
metadata:
  name: postgres-network-policy
  namespace: postgres
spec:
  podSelector:
    matchLabels:
      app: postgres
  policyTypes:
  - Ingress
  - Egress
  ingress:
  - from:
    - namespaceSelector:
        matchLabels:
          name: app-namespace
    ports:
    - protocol: TCP
      port: 5432
  egress:
  - to:
    - namespaceSelector:
        matchLabels:
          name: app-namespace
    ports:
    - protocol: TCP
      port: 5432
```

### 7.3 密钥管理

**使用Sealed Secrets**：

```bash
# 安装Sealed Secrets
kubectl apply -f https://github.com/bitnami-labs/sealed-secrets/releases/download/v0.24.0/controller.yaml

# 创建Sealed Secret
kubectl create secret generic postgres-secret \
  --from-literal=postgres-password=mysecretpassword \
  --dry-run=client -o yaml | \
  kubectl seal -o yaml > postgres-sealed-secret.yaml
```

**使用External Secrets Operator**：

```yaml
apiVersion: external-secrets.io/v1beta1
kind: ExternalSecret
metadata:
  name: postgres-secret
  namespace: postgres
spec:
  secretStoreRef:
    name: vault-backend
    kind: SecretStore
  target:
    name: postgres-secret
  data:
  - secretKey: postgres-password
    remoteRef:
      key: postgres/credentials
      property: password
```

---

## 八、性能优化

### 8.1 资源限制

**资源请求和限制**：

```yaml
resources:
  requests:
    memory: "2Gi"
    cpu: "1000m"
  limits:
    memory: "4Gi"
    cpu: "2000m"
```

**HPA自动伸缩**：

```yaml
apiVersion: autoscaling/v2
kind: HorizontalPodAutoscaler
metadata:
  name: postgres-hpa
  namespace: postgres
spec:
  scaleTargetRef:
    apiVersion: apps/v1
    kind: StatefulSet
    name: postgres
  minReplicas: 1
  maxReplicas: 5
  metrics:
  - type: Resource
    resource:
      name: cpu
      target:
        type: Utilization
        averageUtilization: 70
  - type: Resource
    resource:
      name: memory
      target:
        type: Utilization
        averageUtilization: 80
```

### 8.2 存储优化

**使用本地SSD**：

```yaml
apiVersion: storage.k8s.io/v1
kind: StorageClass
metadata:
  name: local-ssd
provisioner: kubernetes.io/no-provisioner
volumeBindingMode: WaitForFirstConsumer
```

**存储性能优化**：

```yaml
# 使用高性能存储类
storageClassName: fast-ssd

# 启用存储扩展
allowVolumeExpansion: true
```

### 8.3 网络优化

**使用Service Mesh**：

```yaml
# Istio VirtualService
apiVersion: networking.istio.io/v1beta1
kind: VirtualService
metadata:
  name: postgres
  namespace: postgres
spec:
  hosts:
  - postgres.postgres.svc.cluster.local
  tcp:
  - match:
    - port: 5432
    route:
    - destination:
        host: postgres.postgres.svc.cluster.local
        port:
          number: 5432
```

---

## 九、知识矩阵对比

### 9.1 部署方案对比

| 方案 | 复杂度 | 自动化程度 | 高可用 | 适用场景 | 推荐度 |
|------|--------|-----------|--------|---------|--------|
| **Docker单容器** | ⭐ | ⭐⭐ | ⭐ | 开发测试 | ⭐⭐⭐ |
| **Docker Compose** | ⭐⭐ | ⭐⭐⭐ | ⭐⭐ | 开发测试、小型生产 | ⭐⭐⭐⭐ |
| **Kubernetes StatefulSet** | ⭐⭐⭐ | ⭐⭐⭐ | ⭐⭐⭐ | 生产环境 | ⭐⭐⭐⭐ |
| **PostgreSQL Operator** | ⭐⭐⭐⭐ | ⭐⭐⭐⭐⭐ | ⭐⭐⭐⭐⭐ | 生产环境 | ⭐⭐⭐⭐⭐ |
| **Helm Chart** | ⭐⭐⭐ | ⭐⭐⭐⭐ | ⭐⭐⭐ | 快速部署 | ⭐⭐⭐⭐ |

### 9.2 Operator对比

| Operator | 功能完整性 | 易用性 | 社区支持 | 推荐度 |
|---------|-----------|--------|---------|--------|
| **Crunchy PostgreSQL Operator** | ⭐⭐⭐⭐⭐ | ⭐⭐⭐⭐ | ⭐⭐⭐⭐ | ⭐⭐⭐⭐⭐ |
| **Zalando PostgreSQL Operator** | ⭐⭐⭐⭐ | ⭐⭐⭐ | ⭐⭐⭐ | ⭐⭐⭐⭐ |
| **CloudNativePG** | ⭐⭐⭐⭐ | ⭐⭐⭐⭐ | ⭐⭐⭐ | ⭐⭐⭐⭐ |

### 9.3 存储方案对比

| 存储方案 | 性能 | 可靠性 | 成本 | 适用场景 | 推荐度 |
|---------|------|--------|------|---------|--------|
| **本地存储** | ⭐⭐⭐⭐⭐ | ⭐⭐⭐ | ⭐⭐⭐⭐ | 高性能需求 | ⭐⭐⭐ |
| **网络存储（NFS）** | ⭐⭐⭐ | ⭐⭐⭐⭐ | ⭐⭐⭐⭐ | 共享存储 | ⭐⭐⭐ |
| **云存储（EBS/GCE PD）** | ⭐⭐⭐⭐ | ⭐⭐⭐⭐⭐ | ⭐⭐⭐ | 云环境 | ⭐⭐⭐⭐⭐ |
| **分布式存储（Ceph）** | ⭐⭐⭐⭐ | ⭐⭐⭐⭐⭐ | ⭐⭐ | 大规模部署 | ⭐⭐⭐⭐ |

---

## 十、实践案例

### 10.1 开发环境部署

**开发环境docker-compose.yml**：

```yaml
version: '3.8'

services:
  postgres:
    image: postgres:18
    environment:
      POSTGRES_PASSWORD: devpassword
      POSTGRES_DB: devdb
    ports:
      - "5432:5432"
    volumes:
      - postgres_dev_data:/var/lib/postgresql/data
    networks:
      - dev_network

  pgadmin:
    image: dpage/pgadmin4:latest
    environment:
      PGADMIN_DEFAULT_EMAIL: dev@example.com
      PGADMIN_DEFAULT_PASSWORD: admin
    ports:
      - "5050:80"
    networks:
      - dev_network
    depends_on:
      - postgres

volumes:
  postgres_dev_data:

networks:
  dev_network:
    driver: bridge
```

### 10.2 生产环境部署

**生产环境Kubernetes配置**：

```yaml
# 使用PostgreSQL Operator
apiVersion: postgres-operator.crunchydata.com/v1beta1
kind: PostgresCluster
metadata:
  name: postgres-prod
  namespace: production
spec:
  image: registry.developers.crunchydata.com/crunchydata/crunchy-postgres:ubi8-18.0-0
  postgresVersion: 18
  instances:
    - name: instance1
      replicas: 3
      resources:
        requests:
          memory: 4Gi
          cpu: 2000m
        limits:
          memory: 8Gi
          cpu: 4000m
      dataVolumeClaimSpec:
        accessModes:
        - ReadWriteOnce
        storageClassName: fast-ssd
        resources:
          requests:
            storage: 100Gi
  backups:
    pgbackrest:
      repos:
      - name: repo1
        volume:
          volumeClaimSpec:
            accessModes:
            - ReadWriteOnce
            storageClassName: fast-ssd
            resources:
              requests:
                storage: 200Gi
  proxy:
    pgBouncer:
      replicas: 3
      resources:
        requests:
          memory: 128Mi
          cpu: 200m
```

### 10.3 多租户部署

**多租户StatefulSet配置**：

```yaml
apiVersion: apps/v1
kind: StatefulSet
metadata:
  name: postgres-tenant-${TENANT_ID}
  namespace: tenants
spec:
  serviceName: postgres-tenant-${TENANT_ID}
  replicas: 1
  template:
    spec:
      containers:
      - name: postgres
        image: postgres:18
        env:
        - name: POSTGRES_DB
          value: "tenant_${TENANT_ID}"
        - name: POSTGRES_USER
          value: "tenant_${TENANT_ID}"
        - name: POSTGRES_PASSWORD
          valueFrom:
            secretKeyRef:
              name: tenant-${TENANT_ID}-secret
              key: password
  volumeClaimTemplates:
  - metadata:
      name: postgres-storage
    spec:
      accessModes: [ "ReadWriteOnce" ]
      resources:
        requests:
          storage: 10Gi
```

---

## 十一、故障排查

### 11.1 常见问题

**问题1：容器无法启动**

```bash
# 查看容器日志
docker logs postgres
kubectl logs postgres-0 -n postgres

# 检查配置
docker exec postgres cat /etc/postgresql/postgresql.conf
kubectl exec postgres-0 -n postgres -- cat /etc/postgresql/postgresql.conf

# 检查数据目录权限
docker exec postgres ls -la /var/lib/postgresql/data
```

**问题2：连接被拒绝**

```bash
# 检查服务状态
kubectl get svc -n postgres

# 检查网络策略
kubectl get networkpolicies -n postgres

# 测试连接
kubectl run -it --rm debug --image=postgres:18 --restart=Never -- \
  psql -h postgres.postgres.svc.cluster.local -U postgres
```

**问题3：存储问题**

```bash
# 检查PVC状态
kubectl get pvc -n postgres

# 检查PV状态
kubectl get pv

# 查看存储事件
kubectl describe pvc postgres-storage-postgres-0 -n postgres
```

### 11.2 诊断方法

**Pod诊断**：

```bash
# 查看Pod状态
kubectl describe pod postgres-0 -n postgres

# 查看Pod事件
kubectl get events -n postgres --sort-by='.lastTimestamp'

# 进入Pod调试
kubectl exec -it postgres-0 -n postgres -- /bin/bash
```

**网络诊断**：

```bash
# 测试DNS解析
kubectl run -it --rm debug --image=busybox --restart=Never -- \
  nslookup postgres.postgres.svc.cluster.local

# 测试端口连通性
kubectl run -it --rm debug --image=postgres:18 --restart=Never -- \
  pg_isready -h postgres.postgres.svc.cluster.local -p 5432
```

### 11.3 解决方案

**数据恢复**：

```bash
# 从备份恢复
kubectl exec -it postgres-0 -n postgres -- \
  pg_restore -h localhost -U postgres -d mydb /backup/mydb.dump

# 从卷快照恢复
kubectl create pvc postgres-restored \
  --from-snapshot=postgres-snapshot \
  -n postgres
```

**性能问题**：

```bash
# 查看资源使用
kubectl top pod postgres-0 -n postgres

# 调整资源限制
kubectl patch statefulset postgres -n postgres -p '
{
  "spec": {
    "template": {
      "spec": {
        "containers": [{
          "name": "postgres",
          "resources": {
            "limits": {"memory": "8Gi", "cpu": "4000m"}
          }
        }]
      }
    }
  }
}'
```

---

## 十二、最佳实践

### 12.1 部署最佳实践

1. **镜像选择**：
   - 使用官方镜像
   - 固定版本标签
   - 定期更新镜像

2. **资源配置**：
   - 设置合理的资源请求和限制
   - 监控资源使用情况
   - 使用HPA自动伸缩

3. **存储管理**：
   - 使用持久化存储
   - 定期备份
   - 启用存储扩展

### 12.2 安全最佳实践

1. **密钥管理**：
   - 使用Secret存储密码
   - 使用Sealed Secrets或External Secrets
   - 定期轮换密钥

2. **网络安全**：
   - 使用NetworkPolicy限制访问
   - 启用TLS/SSL
   - 使用Service Mesh

3. **镜像安全**：
   - 扫描镜像漏洞
   - 使用可信镜像源
   - 定期更新镜像

### 12.3 运维最佳实践

1. **监控告警**：
   - 配置Prometheus监控
   - 设置告警规则
   - 定期检查日志

2. **备份恢复**：
   - 自动化备份
   - 定期测试恢复
   - 多地点备份

3. **故障处理**：
   - 建立故障处理流程
   - 定期演练故障转移
   - 记录故障处理经验

---

## 十三、参考资源

### 13.1 官方文档

- [PostgreSQL Docker镜像](https://hub.docker.com/_/postgres)
- [Kubernetes StatefulSet](https://kubernetes.io/docs/concepts/workloads/controllers/statefulset/)
- [Crunchy PostgreSQL Operator](https://www.crunchydata.com/products/crunchy-postgresql-operator)

### 13.2 网络资源

- [PostgreSQL容器化最佳实践](https://www.postgresql.org/docs/current/)
- [Kubernetes部署指南](https://kubernetes.io/docs/)
- [Docker Compose文档](https://docs.docker.com/compose/)

### 13.3 相关文档

- `04-部署运维/04.01-单机部署与配置.md` - 单机部署指南
- `04-部署运维/04.02-集群部署与高可用.md` - 高可用部署
- `05-部署架构/容器化部署/05.12-Docker部署.md` - Docker详细指南
- `05-部署架构/容器化部署/05.13-Kubernetes部署.md` - Kubernetes详细指南

---

**维护者**: Data-Science Team
**最后更新**: 2025-01-15
**版本**: 3.0

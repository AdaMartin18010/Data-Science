...
- [06 落地案例（2025 精选）](#06-落地案例2025-精选)
  ...
  - [案例 5：政务社保大数据（行列混存 + 脱敏）](#案例-5政务社保大数据行列混存--脱敏)
    ...
    - [指标与效果](#指标与效果-4)
    - [风险与教训](#风险与教训-4)
  - [案例 6：教育智能推荐系统（pgvector + 虚拟生成列）](#案例-6教育智能推荐系统pgvector--虚拟生成列)
    - [场景与目标](#场景与目标-5)
    - [架构与组件](#架构与组件-5)
    - [关键技术实现](#关键技术实现-5)
      - [1. 虚拟生成列实现特征工程](#1-虚拟生成列实现特征工程)
      - [2. 推荐查询实现](#2-推荐查询实现)
    - [指标与效果](#指标与效果-5)
    - [风险与教训](#风险与教训-5)
  - [案例 7：物流路径优化（Apache AGE + 时序 + 向量）](#案例-7物流路径优化apache-age--时序--向量)
    - [场景与目标](#场景与目标-6)
    - [架构与组件](#架构与组件-6)
    - [关键技术实现](#关键技术实现-6)
      - [1. 多模态数据模型设计](#1-多模态数据模型设计)
      - [2. 图+时序+向量联合查询](#2-图时序向量联合查询)
    - [指标与效果](#指标与效果-6)
    - [风险与教训](#风险与教训-6)
  - [案例 8：内容 RAG 知识库（pgvector + Neon分支）](#案例-8内容-rag-知识库pgvector--neon分支)
    - [场景与目标](#场景与目标-7)
    - [架构与组件](#架构与组件-7)
    - [关键技术实现](#关键技术实现-7)
      - [1. 分支创建与管理](#1-分支创建与管理-1)
      - [2. A/B 测试不同 embedding 模型](#2-ab-测试不同-embedding-模型)
      - [3. 检索准确率评估](#3-检索准确率评估)
    - [指标与效果](#指标与效果-7)
    - [风险与教训](#风险与教训-7)
  - [📝 总结：案例共性](#-总结案例共性)
...
## 案例 8：内容 RAG 知识库（pgvector + Neon分支）

...
- [04 多模一体化（JSONB / 时序 / 图 / 向量）](#04-多模一体化jsonb--时序--图--向量)
  ...
  - [🎯 核心结论](#-核心结论)
  - [能力与边界](#能力与边界)
    - [JSONB（半结构化数据）](#jsonb半结构化数据)
    - [Timescale（时序数据）](#timescale时序数据)
    - [Apache AGE（图数据）](#apache-age图数据)
    - [pgvector（向量数据）](#pgvector向量数据)
  - [组合建模](#组合建模)
    - [1. 业务实体主表设计](#1-业务实体主表设计)
    - [2. 时序侧表设计](#2-时序侧表设计)
    - [3. 向量表设计](#3-向量表设计)
    - [4. 图侧设计](#4-图侧设计)
  - [示例 SQL 片段](#示例-sql-片段)
    - [1. JSONB 属性查询](#1-jsonb-属性查询)
    - [2. 时序 + 向量联合查询](#2-时序--向量联合查询)
    - [3. 图 + 向量联合查询](#3-图--向量联合查询)
    - [4. JSONB + 向量联合查询](#4-jsonb--向量联合查询)
  - [最佳实践](#最佳实践)
    - [1. 共分区/共簇策略](#1-共分区共簇策略)
    - [2. 冷热数据分层](#2-冷热数据分层)
    - [3. 混合查询索引优化](#3-混合查询索引优化)
  - [风险与缓解](#风险与缓解)
    - [1. 资源竞争](#1-资源竞争)
    - [2. 调优复杂](#2-调优复杂)
    - [3. 存储成本](#3-存储成本)

...
- [云原生与容器化](#云原生与容器化)
  ...
  - [3. 容器化要点](#3-容器化要点)
    ...
    - [3.2 参数注入](#32-参数注入)
      - [使用环境变量](#使用环境变量)
      - [使用ConfigMap（Kubernetes）](#使用configmapkubernetes)
    - [3.3 健康检查](#33-健康检查)
      - [Docker健康检查](#docker健康检查)
      - [Docker Compose健康检查](#docker-compose健康检查)
      - [Kubernetes健康检查](#kubernetes健康检查)
    - [3.4 日志管理](#34-日志管理)
      - [Docker日志配置](#docker日志配置)
      - [日志收集（Fluentd）](#日志收集fluentd)
  - [4. Kubernetes](#4-kubernetes)
    - [4.1 StatefulSet部署](#41-statefulset部署)
      - [基础StatefulSet](#基础statefulset)
    - [4.2 PersistentVolume和PersistentVolumeClaim](#42-persistentvolume和persistentvolumeclaim)
    ...
    - [4.5 自动扩缩容](#45-自动扩缩容)
      - [HPA配置](#hpa配置)
  - [5. 最佳实践](#5-最佳实践)
    - [5.1 资源请求和限制](#51-资源请求和限制)
    - [5.2 亲和性和反亲和性](#52-亲和性和反亲和性)
      - [Pod反亲和性（避免同一节点）](#pod反亲和性避免同一节点)
      - [节点亲和性（指定节点类型）](#节点亲和性指定节点类型)
    - [5.3 滚动升级](#53-滚动升级)
      - [更新策略](#更新策略)
      - [金丝雀发布](#金丝雀发布)
    - [5.4 网络策略](#54-网络策略)
    - [5.5 配置管理](#55-配置管理)
      - [使用ConfigMap和Secret](#使用configmap和secret)
    - [5.6 监控集成](#56-监控集成)
      - [Prometheus监控](#prometheus监控)
  - [6. 后续整合任务](#6-后续整合任务)
    - [6.1 Helm Chart示例](#61-helm-chart示例)
    - [6.2 Operator对比](#62-operator对比)
    - [6.3 选择建议](#63-选择建议)
...
| 特性 | Crunchy Operator | Zalando Operator | CloudNativePG |
|------|-----------------|-----------------|---------------|
| 高可用 | ✅ | ✅ | ✅ |
| 自动备份 | ✅ (pgBackRest) | ✅ | ✅ |
| 监控集成 | ✅ | ✅ | ✅ |
...
- **小型项目**: 使用StatefulSet + ConfigMap/Secret
- **中型项目**: 使用Helm Chart（如Bitnami）
- **大型项目**: 使用Operator（推荐Crunchy或CloudNativePG）
- **云环境**: 使用云服务商的托管服务

...
- [主从复制配置指南](#主从复制配置指南)
  ...
  - [6. 监控与诊断](#6-监控与诊断)
  - [7. 生产级复制配置](#7-生产级复制配置)
    - [7.1 完整生产环境配置](#71-完整生产环境配置)
    - [7.2 级联复制配置](#72-级联复制配置)
    - [7.3 复制槽管理最佳实践](#73-复制槽管理最佳实践)
    - [7.4 PostgreSQL 18新特性配置](#74-postgresql-18新特性配置)
  - [8. 故障排查和诊断](#8-故障排查和诊断)
    - [8.1 复制延迟问题诊断](#81-复制延迟问题诊断)
    - [8.2 复制中断问题排查](#82-复制中断问题排查)
    - [8.3 数据不一致问题](#83-数据不一致问题)
    - [8.4 性能问题诊断](#84-性能问题诊断)
  - [9. 监控和告警](#9-监控和告警)
    - [9.1 复制状态监控](#91-复制状态监控)
    - [9.2 延迟监控](#92-延迟监控)
    - [9.3 告警规则配置](#93-告警规则配置)
    - [9.4 自动化运维脚本](#94-自动化运维脚本)
  - [10. 故障处理](#10-故障处理)
    - [10.1 复制中断恢复](#101-复制中断恢复)
    - [10.2 从库提升为主库](#102-从库提升为主库)
    - [10.3 逻辑复制冲突处理](#103-逻辑复制冲突处理)
    - [10.4 主库故障切换](#104-主库故障切换)
  - [11. 交叉引用](#11-交叉引用)
...
- **物理复制（Physical Replication）**: 基于 WAL 的块级复制，适用于主从高可用场景
- **逻辑复制（Logical Replication）**: 基于逻辑变更的复制，适用于跨版本升级、部分表复制等场景

### 选择建议

| 场景 | 推荐方式 | 说明 |
|------|---------|------|
| 高可用主从 | 物理复制 | 延迟低、数据一致性强 |
| 跨版本升级 | 逻辑复制 | 支持不同版本间复制 |
| 部分表复制 | 逻辑复制 | 可选择特定表 |
| 读写分离 | 物理复制 | 从库可读，延迟低 |
| 多主复制 | 逻辑复制 | 支持双向复制 |
...
```text
# WAL 级别
wal_level = replica  # PostgreSQL 18 推荐使用 replica
...
# WAL 保留大小（避免复制延迟时 WAL 被回收）
wal_keep_size = '1GB'  # PostgreSQL 13+ 推荐使用 wal_keep_size
# 或使用 max_slot_wal_keep_size 控制复制槽保留的 WAL
...
```text
# 同步提交级别
synchronous_commit = remote_write  # 或 remote_apply
...
|------|------|------|--------|
| `local` | 本地提交 | 最快 | 弱 |
| `remote_write` | 远程写入 WAL | 快 | 中 |
| `remote_apply` | 远程应用 WAL | 慢 | 强 |
| `on` | 等同于 `remote_write` | 中 | 中 |
| `off` | 异步提交 | 最快 | 最弱 |
...
- **强同步（remote_apply）**: RPO = 0，但写延迟增加 2-5 倍
- **弱同步（remote_write）**: RPO ≈ 0，写延迟增加 1-2 倍
- **异步**: RPO > 0（可能丢失几秒数据），但写延迟最低
...
**主库完整配置（postgresql.conf）**:

```conf
# ============================================
# WAL配置（复制必需）
# ============================================
wal_level = replica                    # 物理复制使用replica，逻辑复制使用logical
max_wal_senders = 10                   # 最大WAL发送进程数
max_replication_slots = 10             # 最大复制槽数
wal_keep_size = 1GB                    # WAL保留大小（PostgreSQL 13+）
# max_slot_wal_keep_size = 2GB         # 复制槽保留的WAL大小（PostgreSQL 13+）

# ============================================
# 同步复制配置
# ============================================
synchronous_commit = remote_write      # 同步提交级别
synchronous_standby_names = 'ANY 1 (replica1, replica2)'  # 同步备用服务器

# ============================================
# 性能优化（PostgreSQL 18）
# ============================================
# 异步I/O提升复制性能
io_method = aio                        # PostgreSQL 18: 异步I/O
io_combine_limit = 128                 # PostgreSQL 18: I/O合并限制
maintenance_io_workers = 4             # PostgreSQL 18: 维护I/O工作进程
...
max_standby_streaming_delay = 30s      # 最大流式延迟
max_standby_archive_delay = 300s       # 最大归档延迟
...
# ============================================
# 从库查询性能优化
max_connections = 500                  # 从库可以支持更多连接（只读）
shared_buffers = 2GB
...
```bash
# 创建standby.signal文件（标识为备用服务器）
touch /var/lib/postgresql/data/standby.signal
```

**从库postgresql.auto.conf配置**:

```conf
# 自动生成的复制配置（pg_basebackup -R自动创建）
...
```
主库 (Primary)
  ├── 从库1 (Standby 1) - 直接从主库复制
  │     └── 从库2 (Standby 2) - 从Standby 1复制（级联）
  └── 从库3 (Standby 3) - 直接从主库复制
```
...
```bash
# 1. 配置Standby 1作为级联主库
# 在Standby 1的postgresql.conf中
wal_level = replica
max_wal_senders = 5
hot_standby = on

# 2. 在Standby 1的pg_hba.conf中允许复制连接
host    replication     repl            192.168.1.0/24          scram-sha-256

# 3. 从Standby 2连接到Standby 1
pg_basebackup \
  -h standby1_host \
  -U repl \
  -D /var/lib/postgresql/data \
...
io_combine_limit = 128                 # I/O请求合并限制
maintenance_io_workers = 4             # 维护I/O工作进程
max_io_workers = 10                    # 最大I/O工作进程
...
```conf
# postgresql.conf
# PostgreSQL 18: 逻辑复制性能提升
wal_level = logical
max_logical_replication_workers = 8    # 增加逻辑复制工作进程
max_sync_workers_per_subscription = 4  # 增加同步工作进程
...
# 问题3: 主库WAL生成过快
# 解决: 优化主库写操作，使用异步I/O（PostgreSQL 18）
io_method = aio
...
- ⭐⭐⭐ [集群部署与高可用](./05.04-集群部署与高可用.md) - 高可用架构
- ⭐⭐⭐ [读写分离](./05.06-读写分离.md) - 读写分离配置
...
- ⭐⭐⭐ [监控与诊断](../../06-运维实践/监控与诊断/06.01-监控与诊断.md) - 复制监控
...
- [PostgreSQL 官方文档 - 复制](https://www.postgresql.org/docs/current/high-availability.html)

...
- [读写分离配置指南](#读写分离配置指南)
  ...
  - [2. 架构设计](#2-架构设计)
    ...
    - [2.2 中间件架构](#22-中间件架构)
    - [2.3 应用层路由](#23-应用层路由)
  - [3. 使用 pgpool-II](#3-使用-pgpool-ii)
    - [3.1 安装配置](#31-安装配置)
      - [安装](#安装)
      - [基本配置（pgpool.conf）](#基本配置pgpoolconf)
    - [3.2 启动服务](#32-启动服务)
    - [3.3 连接测试](#33-连接测试)
  - [4. 使用 pgbouncer](#4-使用-pgbouncer)
    - [4.1 配置（pgbouncer.ini）](#41-配置pgbouncerini)
    - [4.2 应用层路由](#42-应用层路由)
  - [5. 应用层路由](#5-应用层路由)
    - [5.1 Django 配置](#51-django-配置)
    - [5.2 Spring Boot 配置](#52-spring-boot-配置)
  - [6. 负载均衡](#6-负载均衡)
    - [6.1 轮询（Round Robin）](#61-轮询round-robin)
    - [6.2 加权轮询](#62-加权轮询)
    - [6.3 基于延迟的路由](#63-基于延迟的路由)
  - [7. 监控与诊断](#7-监控与诊断)
    - [7.1 pgpool-II 监控](#71-pgpool-ii-监控)
    - [7.2 复制延迟监控](#72-复制延迟监控)
    - [7.3 读写分离效果监控](#73-读写分离效果监控)
  - [7. pgpool-II详细配置](#7-pgpool-ii详细配置)
    - [7.1 生产级配置](#71-生产级配置)
    - [7.2 负载均衡配置](#72-负载均衡配置)
    - [7.3 故障转移配置](#73-故障转移配置)
    - [7.4 性能优化配置](#74-性能优化配置)
  - [8. 应用层路由详细实现](#8-应用层路由详细实现)
    - [8.1 Django完整配置示例](#81-django完整配置示例)
    - [8.2 Spring Boot完整配置示例](#82-spring-boot完整配置示例)
    - [8.3 Node.js配置示例](#83-nodejs配置示例)
    - [8.4 Python连接池配置](#84-python连接池配置)
  - [9. 故障排查和诊断](#9-故障排查和诊断)
    - [9.1 读写分离问题诊断](#91-读写分离问题诊断)
    - [9.2 负载均衡问题](#92-负载均衡问题)
    - [9.3 连接池问题](#93-连接池问题)
    - [9.4 性能问题诊断](#94-性能问题诊断)
  - [10. 监控和告警](#10-监控和告警)
    - [10.1 读写分离效果监控](#101-读写分离效果监控)
    - [10.2 延迟监控](#102-延迟监控)
    - [10.3 告警规则配置](#103-告警规则配置)
    - [10.4 性能指标监控](#104-性能指标监控)
  - [11. 交叉引用](#11-交叉引用)
    - [相关文档](#相关文档)
      - [部署架构](#部署架构)
      - [核心课程](#核心课程)
...
读写分离（Read-Write Splitting）是将读操作和写操作分发到不同的数据库实例，以提高系统整体性能和可扩展性。
...
child_max_connections = 0              # 子进程最大连接数（0=无限制）
connection_life_time = 0               # 连接生命周期（0=无限制）
client_idle_limit = 0                  # 客户端空闲限制（0=无限制）

# ============================================
# 负载均衡配置
# ============================================
load_balance_mode = on                 # 启用负载均衡
ignore_leading_white_space = on       # 忽略前导空格
...
failover_on_backend_error = on         # 后端错误时故障转移

# ============================================
# 健康检查配置
# ============================================
health_check_period = 30               # 健康检查周期（秒）
health_check_timeout = 10              # 健康检查超时（秒）
health_check_user = 'postgres'         # 健康检查用户
health_check_password = 'password'     # 健康检查密码
health_check_database = 'postgres'     # 健康检查数据库
health_check_max_retries = 3           # 最大重试次数
health_check_retry_delay = 1           # 重试延迟（秒）
...
# 性能优化
# ============================================
memory_cache_enabled = on              # 启用内存缓存
memqcache_method = 'memcached'         # 使用Memcached
...
# 负载均衡权重（根据从库性能调整）
backend_weight1 = 2                    # 高性能从库权重更高
backend_weight2 = 1                    # 普通从库权重较低

# 基于语句的负载均衡
statement_level_load_balance = on      # 语句级负载均衡
...
# 参数: $1=失败节点主机名 $2=失败节点端口 $3=失败节点数据库 $4=新主库主机名 $5=新主库端口 $6=旧主库数据目录 $7=新主库数据目录
...
# 故障转移超时
failover_timeout = 60                  # 故障转移超时（秒）

# 自动故障恢复
failback_on_backend_error = off       # 不自动故障恢复（推荐手动恢复）
```
...
# pgpool.conf
# 连接池大小优化
num_init_children = 64                # 根据并发连接数调整
max_pool = 4                          # 每个子进程的最大连接数
child_life_time = 300                  # 子进程生命周期

# 连接复用
connection_life_time = 0               # 连接生命周期（0=无限制）
client_idle_limit = 0                  # 客户端空闲限制
...
# 查询重写
rewrite_query = on                     # 启用查询重写
rewrite_query_list = 'SELECT'          # 重写SELECT查询

# 并行查询
parallel_mode = off                    # 禁用并行模式（推荐）
```
...
def get_user_list(request):
    """读操作自动路由到从库"""
    users = User.objects.all()  # 自动路由到从库
    return render(request, 'users.html', {'users': users})

def create_user(request):
    """写操作自动路由到主库"""
    user = User.objects.create(  # 自动路由到主库
...
# pgpool.conf
# 根据从库性能调整权重
backend_weight1 = 2                    # 高性能从库
backend_weight2 = 1                    # 普通从库
...
- ⭐⭐⭐ [监控与诊断](../../06-运维实践/监控与诊断/06.01-监控与诊断.md) - 读写分离监控
- ⭐⭐ [备份与恢复](../../06-运维实践/备份与恢复/06.06-备份与恢复.md) - 备份恢复

### 外部资源

- [pgpool-II 官方文档](https://www.pgpool.net/docs/)

...
**PostgreSQL版本**: 18.x (推荐) ⭐ | 17.x (推荐) | 16.x, 15.x (兼容)
**难度**: ⭐⭐⭐⭐
**最后更新**: 2025-11-12

> 🆕 **PostgreSQL 18 + pgvector 2.0 性能优化要点** ⭐⭐⭐
>
> PostgreSQL 18 + pgvector 2.0 带来多项性能改进，调优时应重点关注：
>
> - ✅ **异步 I/O 子系统**: 向量检索 I/O 性能提升 2-3 倍 ⭐⭐⭐
> - ✅ **虚拟生成列**: 动态计算优化，查询性能提升 15-25% ⭐⭐
> - ✅ **并行查询增强**: 更智能的并行执行计划，建议调整max_parallel_workers
> - ✅ **索引优化**: B-tree去重优化，BRIN性能提升15-20%
> - ✅ **内存管理**: 动态共享内存，可减少预分配内存
> - ✅ **向量操作**: pgvector 2.0 SIMD优化，性能提升35-45%
...
- [性能调优实践](#性能调优实践)
  ...
  - [2. 合并来源与映射](#2-合并来源与映射)
  - [3. 基线与变更治理](#3-基线与变更治理)
    - [3.1 基线参数建立](#31-基线参数建立)
    - [3.2 SLA目标定义](#32-sla目标定义)
    - [3.3 指标阈值设置](#33-指标阈值设置)
    - [3.4 变更票据模板](#34-变更票据模板)
    - [3.5 预生产压测](#35-预生产压测)
  - [4. 调优流程](#4-调优流程)
    - [4.1 步骤1: 入口指征识别](#41-步骤1-入口指征识别)
    - [4.2 步骤2: 快速体检](#42-步骤2-快速体检)
    - [4.3 步骤3: TopN分析](#43-步骤3-topn分析)
    - [4.4 步骤4: 计划核查](#44-步骤4-计划核查)
    - [4.5 步骤5: 策略选择](#45-步骤5-策略选择)
    - [4.6 步骤6: 小步快跑](#46-步骤6-小步快跑)
    - [4.7 步骤7: 回归守护](#47-步骤7-回归守护)
  - [5. 典型场景](#5-典型场景)
  - [6. 工具与方法](#6-工具与方法)
  - [7. 参数调优指南（按子系统）](#7-参数调优指南按子系统)
    - [7.1 连接与并发参数](#71-连接与并发参数)
    - [7.2 缓冲与缓存参数](#72-缓冲与缓存参数)
    - [7.3 并行与JIT参数](#73-并行与jit参数)
    - [7.4 I/O与存储参数](#74-io与存储参数)
    - [7.5 WAL与持久化参数](#75-wal与持久化参数)
    - [7.6 维护与自清理参数](#76-维护与自清理参数)
  - [8. 查询层优化策略](#8-查询层优化策略)
    - [8.1 代价对齐优化](#81-代价对齐优化)
    - [8.2 访问路径优化](#82-访问路径优化)
    - [8.3 连接策略优化](#83-连接策略优化)
    - [8.4 查询重写优化](#84-查询重写优化)
  - [9. 存储与表设计](#9-存储与表设计)
  - [10. 典型工作负载基线](#10-典型工作负载基线)
  - [11. 验证与回归](#11-验证与回归)
  - [12. PostgreSQL 18 新特性应用](#12-postgresql-18-新特性应用)
    - [异步 I/O 优化](#异步-io-优化)
    - [虚拟生成列优化](#虚拟生成列优化)
  - [13. 实战案例](#13-实战案例)
    - [案例1: OLTP系统调优](#案例1-oltp系统调优)
    - [案例2: OLAP分析系统调优](#案例2-olap分析系统调优)
    - [案例3: 混合负载调优](#案例3-混合负载调优)
  - [14. 深入阅读](#14-深入阅读)
    - [相关文档](#相关文档)
      - [查询与优化](#查询与优化)
      - [核心课程](#核心课程)
...
- 1.1.16-性能调优与监控.md
- 03-查询与优化/02.04-执行计划与性能调优.md（交叉）
...
('P95延迟', 'p95_latency', 100, 'ms'),
('P99延迟', 'p99_latency', 500, 'ms'),
('吞吐量', 'tps', 1000, 'transactions/sec'),
('缓存命中率', 'cache_hit_ratio', 95, 'percent'),
('错误率', 'error_rate', 0.1, 'percent');
```
...
# 使用pgbench进行压测
# 1. 初始化测试数据
pgbench -i -s 100 mydb  # -s: 缩放因子，100表示1000万行
...
```sql
-- 1. 检查P99延迟（需要pg_stat_statements）
SELECT
    queryid,
...
-- 3. 检查基数估算误差
-- 如果实际行数与估算行数差异大，需要更新统计信息
ANALYZE users;

-- 4. 检查并行度
-- 查看计划中是否使用了并行工作进程
-- 如果应该并行但没有，检查max_parallel_workers_per_gather
...
('p95_latency', 100, 200, false),  -- 毫秒
('error_rate', 0.1, 1.0, true),    -- 百分比
('cache_hit_ratio', 90, 85, false); -- 百分比
...
```sql
-- 1. max_connections
-- 计算：考虑应用连接数 + 管理连接 + 复制连接
-- 建议：100-300（使用连接池时）
-- 内存影响：每个连接约消耗 10MB（work_mem等）
...
-- 如果使用pgbouncer，可以降低到100-150

-- 2. work_mem
-- 计算：work_mem * max_connections < 总内存的25%
-- 建议：4-64MB，根据查询复杂度调整
...
```sql
-- 1. shared_buffers
-- 建议：系统内存的25%（Linux），Windows可以更高
-- 64GB内存：16GB shared_buffers
...
-- 配置建议
ALTER SYSTEM SET shared_buffers = '16GB';  -- 64GB系统
...
-- max_parallel_workers: 系统最大并行工作进程总数
...
-- 3. 检查点参数
-- checkpoint_timeout: 检查点间隔（10-30分钟）
-- max_wal_size: 最大WAL大小（2-20GB）
-- checkpoint_completion_target: 检查点完成目标（0.7-0.9）
...
```sql
-- 1. wal_compression
-- 启用压缩可以减少WAL大小，但增加CPU使用
ALTER SYSTEM SET wal_compression = on;

-- 2. synchronous_commit
-- local: 本地提交即可（最快，可能丢数据）
-- remote_write: 等待写入从库OS缓存（推荐）
-- remote_apply: 等待从库应用（最严格，最慢）
-- off: 等同于local（已废弃）
...
-- autovacuum_analyze_scale_factor: 触发ANALYZE的变更比例
...
```sql
-- 1. 调整统计目标
-- default_statistics_target: 默认100，可以提高到200-500
...
FROM users u
JOIN orders o ON u.id::text = o.user_id;  -- 类型转换导致Seq Scan

-- 正确示例：类型一致
SELECT u.id, o.user_id
FROM users u
JOIN orders o ON u.id = o.user_id::int;  -- 或修改表结构使类型一致

-- 3. 使用合适的连接算法
-- Hash Join: 适合大表JOIN
-- Nested Loop: 适合小表JOIN
-- Merge Join: 适合已排序的数据
...
-- 4. 连接顺序优化
-- 使用JOIN_COLLAPSE_LIMIT控制连接顺序
SET join_collapse_limit = 8;  -- 默认8，可以调整
```
...
## 12. PostgreSQL 18 新特性应用

### 异步 I/O 优化

PostgreSQL 18 的异步 I/O 子系统可以显著提升I/O密集型操作的性能：

```sql
-- PostgreSQL 18: 异步 I/O 自动启用
-- 无需额外配置，系统自动优化I/O操作
-- 特别适用于：
-- - 大规模向量检索
-- - 批量数据导入
...
PostgreSQL 18 的虚拟生成列可以优化计算密集型查询：

```sql
-- PostgreSQL 18: 使用虚拟生成列优化查询
...
**场景**: 电商订单系统，高并发写入，低延迟查询

**调优方案**:

```sql
-- 连接池配置
-- pgbouncer transaction模式
...
**效果**: 复杂查询性能提升60%，并行利用率提升50%

### 案例3: 混合负载调优

**场景**: 读写混合，需要平衡OLTP和OLAP性能
...
- ⭐⭐ [存储管理与数据持久化](../../01-核心课程/01.06-存储管理与数据持久化.md) - 存储优化（列存储架构、列压缩技术、异步I/O）🆕

#### 运维实践

- ⭐⭐⭐ [监控与诊断](../../06-运维实践/监控与诊断/06.01-监控与诊断.md) - 性能监控
- ⭐⭐ [性能调优变更闭环](../../06-运维实践/监控与诊断/06.03-性能调优变更闭环.md) - 变更管理
- ⭐⭐ [性能问题-案例库](../../09-应用设计/行业案例/性能问题-案例库.md) - 性能问题案例

#### 前沿技术

- ⭐⭐ [向量检索性能调优](../../07-前沿技术/05.05-向量检索性能调优指南.md) - 向量检索优化

#### 版本特性

- ⭐⭐ [PostgreSQL 18新特性](../../02-版本特性/02.01-PostgreSQL-18-新特性.md) - PostgreSQL 18性能优化
...
- [PostgreSQL配置参数](https://www.postgresql.org/docs/current/runtime-config.html)
...
2. PostgreSQL官方文档 - [性能调优](https://www.postgresql.org/docs/current/performance-tips.html)
3. PostgreSQL官方文档 - [配置参数](https://www.postgresql.org/docs/current/runtime-config.html)
4. PostgreSQL官方文档 - [pg_stat_statements](https://www.postgresql.org/docs/current/pgstatstatements.html)
5. PostgreSQL官方文档 - [EXPLAIN](https://www.postgresql.org/docs/current/sql-explain.html)
6. TimescaleDB - [性能调优最佳实践](https://docs.timescale.com/timescaledb/latest/how-to-guides/performance/)

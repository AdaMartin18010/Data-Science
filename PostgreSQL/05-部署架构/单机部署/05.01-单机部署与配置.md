...
- [单机部署与配置](#单机部署与配置)
  ...
  - [2. 合并来源与映射](#2-合并来源与映射)
  - [3. 安装与初始化](#3-安装与初始化)
    - [3.1 包管理器安装](#31-包管理器安装)
      - [Ubuntu/Debian](#ubuntudebian)
      - [CentOS/RHEL](#centosrhel)
      - [macOS](#macos)
    - [3.2 源码编译安装](#32-源码编译安装)
    - [3.3 数据库初始化](#33-数据库初始化)
    - [3.4 服务管理](#34-服务管理)
    - [3.5 验证安装](#35-验证安装)
  - [4. 配置要点](#4-配置要点)
    - [4.1 postgresql.conf基础配置](#41-postgresqlconf基础配置)
      - [连接设置](#连接设置)
      - [内存设置](#内存设置)
      - [I/O设置](#io设置)
      - [WAL设置](#wal设置)
      - [日志设置](#日志设置)
    - [4.2 pg\_hba.conf认证配置](#42-pg_hbaconf认证配置)
      - [认证规则示例](#认证规则示例)
      - [认证方法说明](#认证方法说明)
    - [4.3 系统参数优化](#43-系统参数优化)
      - [Linux内核参数](#linux内核参数)
      - [资源限制](#资源限制)
    - [4.4 配置验证](#44-配置验证)
  - [5. 安全与合规](#5-安全与合规)
    - [5.1 用户和权限管理](#51-用户和权限管理)
    - [5.2 SSL/TLS加密](#52-ssltls加密)
    - [5.3 审计日志](#53-审计日志)
    - [5.4 行级安全策略](#54-行级安全策略)
    - [5.5 数据加密](#55-数据加密)
    - [5.6 安全最佳实践](#56-安全最佳实践)
  - [6. 后续整合任务](#6-后续整合任务)
    - [6.1 不同操作系统差异](#61-不同操作系统差异)
      - [Ubuntu/Debian](#ubuntudebian-1)
      - [CentOS/RHEL](#centosrhel-1)
      - [macOS (Homebrew)](#macos-homebrew)
    - [6.2 系统参数推荐表](#62-系统参数推荐表)
    - [6.3 性能调优检查清单](#63-性能调优检查清单)
...
```conf
# 随机页面成本（SSD建议1.1，HDD建议4.0）
random_page_cost = 1.1

# 有效I/O并发数（SSD建议200，HDD建议2）
effective_io_concurrency = 200

# 异步I/O（PostgreSQL 18新特性）
...
# SSL连接使用scram-sha-256认证
hostssl all             all             0.0.0.0/0               scram-sha-256
```

#### 认证方法说明

- **trust**: 无条件允许连接（仅用于本地开发）
- **md5**: 使用MD5加密密码
- **scram-sha-256**: 使用SCRAM-SHA-256加密密码（推荐）
...
# 2. 限制网络访问
# 在pg_hba.conf中仅允许必要的IP访问

# 3. 使用强密码策略
# 在应用层或使用passwordcheck扩展

# 4. 定期备份
# 配置自动备份脚本

# 5. 监控异常访问
# 使用pg_stat_statements和日志分析
```
...
| 系统内存 | shared_buffers | effective_cache_size | work_mem | maintenance_work_mem |
|---------|----------------|---------------------|----------|---------------------|
...
- ⭐⭐ [安全机制与访问控制](../../04-高级特性/03.02-安全机制与访问控制.md) - 安全最佳实践

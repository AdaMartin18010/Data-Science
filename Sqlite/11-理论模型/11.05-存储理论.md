...
- [存储理论：页结构与B-Tree存储模型](#存储理论页结构与b-tree存储模型)
  ...
  - [4. 存储模型理论](#4-存储模型理论)
    ...
    - [4.5. 列存储理论](#45-列存储理论)
      - [📊 列存储理论知识体系](#-列存储理论知识体系)
      - [📊 列存储 vs 行存储理论对比矩阵](#-列存储-vs-行存储理论对比矩阵)
      - [📊 列存储理论证明流程图](#-列存储理论证明流程图)
      - [📊 列存储I/O理论分析图](#-列存储io理论分析图)
    - [4.6. 列压缩理论](#46-列压缩理论)
      - [📊 列压缩理论知识体系](#-列压缩理论知识体系)
...
| 理论维度 | 行存储（SQLite） | 列存储 | 理论优势 | 数学证明 |
|---------|----------------|--------|---------|---------|
| **信息熵** | H_row = -Σ P(row)log₂P(row) | H_column = -Σ P(col)log₂P(col) | 列存储 | H_column < H_row |
| **压缩下限** | ≥ n × H_row | ≥ n × H_column | 列存储 | 香农熵定理 |
| **I/O复杂度** | O(n × RowSize) | O(Σ Sᵢ × (1-CRᵢ)) | 列存储 | k << TotalCols时显著 |
| **访问模式** | 随机访问整行 | 顺序访问单列 | 列存储 | 列扫描O(n)，行扫描O(n×m) |
| **缓存效率** | 低（混合数据） | 高（同质数据） | 列存储 | 局部性原理 |
| **压缩率** | 10-30% | 70-90% | 列存储 | 熵差异导致 |
| **查询复杂度** | O(n) 全表扫描 | O(k×n/k) 列扫描 | 列存储 | k列时I/O减少k倍 |
...
flowchart TD
    ...
    B --> C[列存储熵<br/>H_column = -Σ P col log P col]
    B --> D[行存储熵<br/>H_row = -Σ P row log P row]

    C --> E[同列数据类型相同<br/>分布集中]
    D --> F[行数据混合<br/>分布分散]

    E --> G[H_column < H_row]
    F --> G

    G --> H[压缩下限<br/>Compressed ≥ n × H]

    H --> I[列存储压缩下限更低]
    I --> J[列存储压缩率更高<br/>CompressionRatio_column < CompressionRatio_row]
...
graph LR
    subgraph "行存储I/O"
        A1[查询Q需要k列] --> A2[读取整行数据]
        A2 --> A3[I/O = n × RowSize]
        A3 --> A4[即使只需要k列<br/>也要读取所有列]
    end

    subgraph "列存储I/O"
        B1[查询Q需要k列] --> B2[只读取k列数据]
        B2 --> B3[I/O = Σ Sᵢ × 1-CRᵢ]
        B3 --> B4[只读取需要的列<br/>I/O减少50-90%]
    end

    A4 --> C[I/O节省]
    B4 --> C
    C --> D[IOSaving = 1 - ΣSᵢ / n×RowSize]
...
| 压缩算法 | 理论压缩率 | 适用条件 | 最优场景 | 复杂度 | 信息论基础 | 数学证明 |
|---------|-----------|---------|---------|--------|-----------|---------|
| **字典编码** | 40-60% | log₂\|V\| < AvgValueSize | 低基数列 | O(n) | 香农熵 | 熵下限定理 |
| **游程编码** | 50-90% | k < n (游程数<数据量) | 重复值多 | O(n) | 游程分布 | 最优k=1 |
| **增量编码** | 50-70% | Avg(VarInt(δ)) < ValueSize | 有序数值列 | O(n) | 差值分布 | 单调递增最优 |
| **位图压缩** | 25-75% | k×⌈n/8⌉ < n×ValueSize | 布尔/低基数列 | O(n) | 位图理论 | 布尔列最优 |
| **通用压缩** | 30-80% | 通用场景 | 高熵数据 | O(n log n) | 通用信息论 | 香农熵下限 |
...
flowchart TD
    ...
    B -->|布尔列| C[位图压缩 ✅<br/>压缩率25%]
    B -->|低基数列| D{基数大小}
    B -->|有序数值列| E[增量编码 ✅<br/>压缩率50-70%]
    B -->|高基数列| F[通用压缩 ✅<br/>压缩率30-80%]

    D -->|基数 < 100| G[字典编码 ✅<br/>压缩率40-60%]
    D -->|基数 < 1000| H{重复模式}
    D -->|基数 > 1000| F

    H -->|连续重复| I[游程编码 ✅<br/>压缩率50-90%]
    H -->|随机分布| G

    C --> J[理论证明:<br/>k=2时最优]
    G --> K[理论证明:<br/>log₂V < AvgSize]
    I --> L[理论证明:<br/>k < n时有效]
    E --> M[理论证明:<br/>VarInt δ < ValueSize]
    F --> N[理论证明:<br/>香农熵下限]
...
graph TB
    subgraph "信息论基础"
        A[香农熵定理] --> B[H X = -Σ P x log₂ P x]
        B --> C[压缩下限<br/>CompressedSize ≥ n × H X]
    end

    subgraph "列存储压缩"
        D[列存储熵<br/>H_column] --> E[同列数据类型相同<br/>分布集中]
        E --> F[H_column < H_row]
        F --> G[压缩下限更低<br/>CompressedSize ≥ n × H_column]
    end

    subgraph "行存储压缩"
        H[行存储熵<br/>H_row] --> I[行数据混合<br/>分布分散]
        I --> J[压缩下限更高<br/>CompressedSize ≥ n × H_row]
    end

    G --> K[列存储压缩优势]
    J --> K
    K --> L[CompressionRatio_column < CompressionRatio_row]

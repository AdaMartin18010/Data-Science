...
- [SQLite事务隔离级别形式化证明](#sqlite事务隔离级别形式化证明)
  - [📑 目录](#-目录)
  - [SQLite隔离级别总览](#sqlite隔离级别总览)
  - [形式化定义](#形式化定义)
    - [事务与操作](#事务与操作)
    - [历史与调度](#历史与调度)
  - [SQLite默认模式（Rollback Journal）](#sqlite默认模式rollback-journal)
    - [锁状态机](#锁状态机)
    - [定理1.1：Rollback模式不允许脏读](#定理11rollback模式不允许脏读)
    - [定理1.2：Rollback模式允许不可重复读](#定理12rollback模式允许不可重复读)
  - [WAL模式形式化](#wal模式形式化)
    - [MVCC快照](#mvcc快照)
    - [定理2.1：WAL模式实现快照隔离](#定理21wal模式实现快照隔离)
    - [定理2.2：WAL模式防止幻读](#定理22wal模式防止幻读)
  - [写偏序分析](#写偏序分析)
    - [定义3.1 写偏序](#定义31-写偏序)
    - [定理3.1：WAL模式允许写偏序](#定理31wal模式允许写偏序)
    - [解决方案：显式锁](#解决方案显式锁)
  - [总结定理](#总结定理)
    - [定理4.1：SQLite隔离级别分类](#定理41sqlite隔离级别分类)
  - [实验验证](#实验验证)
    - [实验1：不可重复读（Rollback模式）](#实验1不可重复读rollback模式)
    - [实验2：快照隔离（WAL模式）](#实验2快照隔离wal模式)

...
虽然SQLite原生不支持列存储，但可以通过装饰器模式模拟列存储功能：

```python
# 列存储装饰器：为表添加列存储功能
class ColumnarStorageDecorator:
    """列存储装饰器：为行存储表添加列存储能力"""
    ...
    def create_column_tables(self):
        ...
        for col in self.table.columns:
            col_table = f"{self.table.name}_{col}_column"
    ...
    def query_column(self, column):
        """装饰：列存储查询"""
        # 使用列存储表查询
        return self.column_tables[column].query()

    def query_row(self, row_id):
        """原始：行存储查询"""
        return self.table.query_row(row_id)

# 使用装饰器
table = RowStorageTable('logs')
decorated_table = ColumnarStorageDecorator(table)
decorated_table.create_column_tables()

# 可以使用列存储查询
results = decorated_table.query_column('action')  # 列存储
row = decorated_table.query_row(123)  # 行存储
```
...
```python
# 列压缩装饰器：为列添加压缩功能
class ColumnCompressionDecorator:
    """列压缩装饰器：为列添加压缩能力"""

    def __init__(self, column, compression_method='dictionary'):
        self.column = column
    ...
    def _create_compressor(self):
        """创建压缩器"""
        if self.compression_method == 'dictionary':
            return DictionaryEncoder()
        elif self.compression_method == 'rle':
            return RunLengthEncoder()
        elif self.compression_method == 'delta':
            return DeltaEncoder()

    def store(self, values):
        """装饰：压缩存储"""
        compressed = self.compressor.compress(values)
        return self.column.store(compressed)

    def retrieve(self):
        """装饰：解压检索"""
        compressed = self.column.retrieve()
        return self.compressor.decompress(compressed)

# 使用装饰器
column = Column('action')
compressed_column = ColumnCompressionDecorator(column, 'dictionary')
compressed_column.store(['login', 'logout', 'login', ...])  # 自动压缩
values = compressed_column.retrieve()  # 自动解压
```
...
- [12.04 设计决策](./12.04-设计决策.md) - 行存储vs列存储决策 🆕

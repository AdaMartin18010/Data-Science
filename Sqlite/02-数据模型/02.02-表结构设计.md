...
- [表结构设计：范式化与反范式化决策](#表结构设计范式化与反范式化决策)
  - [📋 概述](#-概述)
  - [📑 目录](#-目录)
  - [一、范式化 vs 反范式化决策矩阵](#一范式化-vs-反范式化决策矩阵)
    - [1.1 决策因素](#11-决策因素)
    - [1.2 决策流程图](#12-决策流程图)
  - [二、移动应用案例：反范式化设计](#二移动应用案例反范式化设计)
    - [2.1 微信聊天记录案例](#21-微信聊天记录案例)
    - [2.2 数据一致性维护](#22-数据一致性维护)
  - [三、嵌入式系统案例：范式化设计](#三嵌入式系统案例范式化设计)
    - [3.1 车载诊断系统案例](#31-车载诊断系统案例)
  - [四、索引策略形式化决策树](#四索引策略形式化决策树)
    - [4.1 索引收益函数](#41-索引收益函数)
    - [4.2 索引选择性](#42-索引选择性)
    - [4.3 Android官方最佳实践](#43-android官方最佳实践)
  - [五、索引设计最佳实践](#五索引设计最佳实践)
    - [5.1 主键设计](#51-主键设计)
    - [5.2 复合索引设计](#52-复合索引设计)
    - [5.3 部分索引优化](#53-部分索引优化)
  - [六、表结构设计模式](#六表结构设计模式)
    - [6.1 时间序列数据模式](#61-时间序列数据模式)
    - [6.2 树形结构模式](#62-树形结构模式)
    - [6.3 多对多关系模式](#63-多对多关系模式)
  - [七、表结构设计多维对比矩阵](#七表结构设计多维对比矩阵)
    - [7.1 范式化 vs 反范式化多维对比矩阵](#71-范式化-vs-反范式化多维对比矩阵)
    - [7.2 索引类型多维对比矩阵](#72-索引类型多维对比矩阵)
    - [7.3 表结构设计模式对比矩阵](#73-表结构设计模式对比矩阵)
    - [7.4 设计模式对比总结](#74-设计模式对比总结)
    - [7.4.1 范式化 vs 反范式化](#741-范式化-vs-反范式化)
    - [7.2 索引策略总结](#72-索引策略总结)
  - [🔗 相关资源](#-相关资源)
  - [🔗 交叉引用](#-交叉引用)
    - [理论模型 🆕](#理论模型-)
    - [设计模型 🆕](#设计模型-)
...
| 维度 | 时间序列模式 | 树形结构模式 | 多对多关系模式 | 键值对模式 |
|------|------------|------------|--------------|-----------|
| **查询性能** | ⭐⭐⭐⭐ | ⭐⭐⭐ | ⭐⭐⭐⭐ | ⭐⭐⭐⭐⭐ |
| **写入性能** | ⭐⭐⭐⭐⭐ | ⭐⭐⭐ | ⭐⭐⭐ | ⭐⭐⭐⭐⭐ |
| **存储效率** | ⭐⭐⭐⭐⭐ | ⭐⭐⭐⭐ | ⭐⭐⭐ | ⭐⭐⭐⭐ |
| **设计复杂度** | ⭐⭐⭐⭐ | ⭐⭐⭐ | ⭐⭐⭐ | ⭐⭐⭐⭐⭐ |
| **适用场景** | 日志、监控 | 组织架构、分类 | 用户-角色 | 配置、缓存 |
| **索引需求** | 时间索引 | 路径索引 | 关联表索引 | 键索引 |

...
- [PRAGMA配置：性能与安全调优](#pragma配置性能与安全调优)
  ...
  - [9.5. PRAGMA配置决策树](#95-pragma配置决策树)
  ...
  - [12. 🔗 交叉引用](#12--交叉引用)
    ...
    - [12.4. 编程实践文档 🆕](#124-编程实践文档-)
    - [12.5. 知识图谱与导航 🆕](#125-知识图谱与导航-)
    - [12.6. 相关概念链接 🆕](#126-相关概念链接-)
      - [12.6.1. PRAGMA配置概念](#1261-pragma配置概念)
  - [13. 📚 参考资料](#13--参考资料)
...
问题：如何配置PRAGMA？
    ...
    │
    ├─ 缓存配置？
    │   ├─ 小数据库（< 100MB）？ → cache_size = -2000（2MB）
    │   ├─ 中等数据库（100MB - 1GB）？ → cache_size = -16000（16MB）
    │   ├─ 大数据库（> 1GB）？ → cache_size = -64000（64MB）或更大
    │   │
    │   ├─ 临时表存储？
    │   │   ├─ 内存充足？ → temp_store = MEMORY
    │   │   └─ 内存有限？ → temp_store = FILE
    │   │
    │   └─ 内存映射？
    │       ├─ 大文件读取？ → mmap_size = 268435456（256MB）
    │       └─ 小文件读取？ → mmap_size = 0（禁用）
    │
    ├─ 外键约束？
    │   ├─ 需要引用完整性？ → foreign_keys = ON
    │   │   ├─ 保证数据一致性
    │   │   └─ 性能略低
    │   │
    │   └─ 不需要引用完整性？ → foreign_keys = OFF
    │       ├─ 性能略高
    │       └─ 应用层维护
    │
    └─ WAL模式配置？
        ├─ 自动Checkpoint？
        │   ├─ 自动管理？ → wal_autocheckpoint = 1000（默认）
        │   └─ 手动管理？ → wal_autocheckpoint = 0
        │
        └─ 手动Checkpoint？
            ├─ 定期执行 → PRAGMA wal_checkpoint(TRUNCATE)
            └─ 完全Checkpoint → PRAGMA wal_checkpoint(FULL)

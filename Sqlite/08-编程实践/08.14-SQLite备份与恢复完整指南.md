# SQLite备份与恢复完整指南

> **创建日期**: 2025-12-05
> **版本**: SQLite 3.47.x
> **难度**: ⭐⭐⭐⭐⭐
> **适用场景**: 生产环境备份、灾难恢复、数据迁移

---

## 📑 目录

- [SQLite备份与恢复完整指南](#sqlite备份与恢复完整指南)
  - [📑 目录](#-目录)
  - [备份策略总览](#备份策略总览)
  - [1. 备份方法](#1-备份方法)
    - [1.1 在线备份（Online Backup API）](#11-在线备份online-backup-api)
      - [Python实现](#python实现)
      - [C语言实现](#c语言实现)
    - [1.2 文件复制备份](#12-文件复制备份)
    - [1.3 SQL转储备份](#13-sql转储备份)
    - [1.4 增量备份](#14-增量备份)
  - [2. 备份实现](#2-备份实现)
    - [2.1 Python备份实现](#21-python备份实现)
    - [2.2 C语言备份实现](#22-c语言备份实现)
    - [2.3 Shell脚本备份](#23-shell脚本备份)
  - [3. 恢复方法](#3-恢复方法)
    - [3.1 完整恢复](#31-完整恢复)
    - [3.2 部分恢复](#32-部分恢复)
    - [3.3 时间点恢复](#33-时间点恢复)
  - [4. WAL模式备份](#4-wal模式备份)
    - [4.1 WAL模式备份策略](#41-wal模式备份策略)
    - [4.2 检查点与备份](#42-检查点与备份)
  - [5. 备份验证](#5-备份验证)
    - [5.1 完整性验证](#51-完整性验证)
    - [5.2 一致性验证](#52-一致性验证)
  - [6. 自动化备份](#6-自动化备份)
    - [6.1 定时备份](#61-定时备份)
    - [6.2 事件驱动备份](#62-事件驱动备份)
  - [7. 灾难恢复计划](#7-灾难恢复计划)
    - [7.1 RTO与RPO](#71-rto与rpo)
    - [7.2 恢复流程](#72-恢复流程)
  - [7.3. 备份恢复决策树](#73-备份恢复决策树)
  - [8. 备份最佳实践](#8-备份最佳实践)
    - [8.1 备份策略](#81-备份策略)
    - [8.2 恢复测试](#82-恢复测试)
  - [9. 🔗 相关资源](#9--相关资源)
    - [内部资源](#内部资源)
    - [外部资源](#外部资源)
  - [🔗 交叉引用](#-交叉引用)
    - [编程实践文档](#编程实践文档)
    - [核心架构文档 🆕](#核心架构文档-)
    - [形式化理论文档 🆕](#形式化理论文档-)
    - [知识图谱与导航 🆕](#知识图谱与导航-)
    - [相关概念链接 🆕](#相关概念链接-)
      - [备份恢复概念](#备份恢复概念)

---

## 备份策略总览

```text
SQLite备份策略矩阵
══════════════════════════════════════════════════════════════════════════════

┌─────────────────────┬──────────────┬──────────────┬──────────────┐
│ 备份方法             │ 适用场景     │ 优点          │ 缺点         │
├─────────────────────┼──────────────┼──────────────┼──────────────┤
│ Online Backup API   │ 生产环境      │ 在线备份      │ 需要C API    │
│                     │ 大型数据库    │ 一致性保证    │ 实现复杂      │
│                     │              │ 性能好        │              │
├─────────────────────┼──────────────┼──────────────┼──────────────┤
│ 文件复制             │ 小型数据库    │ 简单快速     │ 需要停机      │
│                     │ 维护窗口      │ 完整备份     │ 一致性风险    │
├─────────────────────┼──────────────┼──────────────┼──────────────┤
│ SQL转储             │ 跨平台迁移    │ 可读性强      │ 速度慢       │
│                     │ 版本兼容     │ 可编辑        │ 大文件处理    │
├─────────────────────┼──────────────┼──────────────┼──────────────┤
│ 增量备份             │ 频繁备份      │ 节省空间      │ 实现复杂     │
│                     │ 网络备份      │ 快速备份      │ 恢复复杂     │
└─────────────────────┴──────────────┴──────────────┴──────────────┘
```

---

## 1. 备份方法

### 1.1 在线备份（Online Backup API）

在线备份API是SQLite推荐的备份方法，支持在数据库运行时进行备份。

#### Python实现

```python
import sqlite3
import shutil
from pathlib import Path
from contextlib import contextmanager

class OnlineBackup:
    def __init__(self, source_db, backup_db):
        self.source_db = source_db
        self.backup_db = backup_db

    def backup(self, progress_callback=None):
        """使用Online Backup API进行备份"""
        source_conn = sqlite3.connect(self.source_db)
        backup_conn = sqlite3.connect(self.backup_db)

        try:
            # 获取备份对象
            backup = source_conn.backup(backup_conn)

            # 执行备份
            if progress_callback:
                while not backup.done:
                    backup.step(100)  # 每次备份100页
                    progress = backup.progress
                    total = backup.pagecount
                    progress_callback(progress, total)
            else:
                backup.step(-1)  # 一次性备份所有页面

            # 完成备份
            backup.finish()

            return True
        except Exception as e:
            print(f"备份失败: {e}")
            return False
        finally:
            source_conn.close()
            backup_conn.close()

    def backup_with_progress(self):
        """带进度显示的备份"""
        def progress(remaining, total):
            percent = ((total - remaining) / total) * 100
            print(f"备份进度: {percent:.1f}% ({total - remaining}/{total} 页)")

        return self.backup(progress_callback=progress)
```

#### C语言实现

```c
#include <sqlite3.h>
#include <stdio.h>

int backup_database(const char* source_db, const char* backup_db) {
    sqlite3* source;
    sqlite3* backup;
    sqlite3_backup* backup_handle;
    int rc;

    // 打开源数据库
    rc = sqlite3_open(source_db, &source);
    if (rc != SQLITE_OK) {
        fprintf(stderr, "无法打开源数据库: %s\n", sqlite3_errmsg(source));
        return rc;
    }

    // 打开备份数据库
    rc = sqlite3_open(backup_db, &backup);
    if (rc != SQLITE_OK) {
        fprintf(stderr, "无法打开备份数据库: %s\n", sqlite3_errmsg(backup));
        sqlite3_close(source);
        return rc;
    }

    // 初始化备份对象
    backup_handle = sqlite3_backup_init(backup, "main", source, "main");
    if (backup_handle == NULL) {
        fprintf(stderr, "备份初始化失败: %s\n", sqlite3_errmsg(backup));
        sqlite3_close(backup);
        sqlite3_close(source);
        return SQLITE_ERROR;
    }

    // 执行备份（每次100页）
    do {
        rc = sqlite3_backup_step(backup_handle, 100);
        if (rc == SQLITE_OK || rc == SQLITE_BUSY || rc == SQLITE_LOCKED) {
            int remaining = sqlite3_backup_remaining(backup_handle);
            int page_count = sqlite3_backup_pagecount(backup_handle);
            printf("备份进度: %d/%d 页\n", page_count - remaining, page_count);
        }

        if (rc == SQLITE_BUSY || rc == SQLITE_LOCKED) {
            sqlite3_sleep(250);  // 等待250ms后重试
        }
    } while (rc == SQLITE_OK || rc == SQLITE_BUSY || rc == SQLITE_LOCKED);

    // 完成备份
    sqlite3_backup_finish(backup_handle);

    if (rc == SQLITE_DONE) {
        printf("备份完成\n");
    } else {
        fprintf(stderr, "备份失败: %s\n", sqlite3_errmsg(backup));
    }

    sqlite3_close(backup);
    sqlite3_close(source);

    return rc;
}
```

### 1.2 文件复制备份

文件复制是最简单的备份方法，但需要确保数据库处于一致状态。

```python
import shutil
import sqlite3
from pathlib import Path
from datetime import datetime

def file_copy_backup(db_path, backup_dir):
    """文件复制备份"""
    db_path = Path(db_path)
    backup_dir = Path(backup_dir)
    backup_dir.mkdir(parents=True, exist_ok=True)

    # 生成备份文件名
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    backup_path = backup_dir / f"{db_path.stem}_{timestamp}.db"

    # 检查WAL模式
    conn = sqlite3.connect(str(db_path))
    cursor = conn.cursor()
    cursor.execute("PRAGMA journal_mode")
    journal_mode = cursor.fetchone()[0]
    conn.close()

    if journal_mode == "wal":
        # WAL模式：需要先执行检查点
        conn = sqlite3.connect(str(db_path))
        cursor = conn.cursor()
        cursor.execute("PRAGMA wal_checkpoint(TRUNCATE)")
        conn.close()

    # 复制数据库文件
    shutil.copy2(db_path, backup_path)

    # 如果存在WAL文件，也复制
    wal_path = db_path.parent / f"{db_path.name}-wal"
    if wal_path.exists():
        backup_wal_path = backup_dir / f"{db_path.stem}_{timestamp}.db-wal"
        shutil.copy2(wal_path, backup_wal_path)

    # 如果存在SHM文件，也复制
    shm_path = db_path.parent / f"{db_path.name}-shm"
    if shm_path.exists():
        backup_shm_path = backup_dir / f"{db_path.stem}_{timestamp}.db-shm"
        shutil.copy2(shm_path, backup_shm_path)

    return backup_path
```

### 1.3 SQL转储备份

SQL转储生成可读的SQL脚本，适合跨平台迁移和版本兼容。

```python
import sqlite3
import subprocess
from pathlib import Path

def sql_dump_backup(db_path, dump_file):
    """SQL转储备份"""
    db_path = Path(db_path)
    dump_file = Path(dump_file)

    # 使用sqlite3命令行工具
    cmd = ['sqlite3', str(db_path), '.dump']

    with open(dump_file, 'w', encoding='utf-8') as f:
        result = subprocess.run(cmd, stdout=f, stderr=subprocess.PIPE)

    if result.returncode != 0:
        raise Exception(f"SQL转储失败: {result.stderr.decode()}")

    return dump_file

def sql_dump_backup_python(db_path, dump_file):
    """使用Python实现SQL转储"""
    conn = sqlite3.connect(db_path)
    dump_file = Path(dump_file)

    with open(dump_file, 'w', encoding='utf-8') as f:
        # 写入头部
        f.write("-- SQLite数据库转储\n")
        f.write(f"-- 源数据库: {db_path}\n")
        f.write(f"-- 生成时间: {datetime.now().isoformat()}\n\n")

        # 转储所有表
        cursor = conn.cursor()

        # 获取所有表
        cursor.execute("""
            SELECT name
            FROM sqlite_master
            WHERE type='table'
            AND name NOT LIKE 'sqlite_%'
        """)

        tables = cursor.fetchall()

        for (table_name,) in tables:
            # 获取表结构
            cursor.execute(f"SELECT sql FROM sqlite_master WHERE type='table' AND name='{table_name}'")
            create_sql = cursor.fetchone()
            if create_sql:
                f.write(f"{create_sql[0]};\n\n")

            # 获取表数据
            cursor.execute(f"SELECT * FROM {table_name}")
            rows = cursor.fetchall()

            # 获取列名
            cursor.execute(f"PRAGMA table_info({table_name})")
            columns = [row[1] for row in cursor.fetchall()]

            # 写入数据
            for row in rows:
                values = []
                for value in row:
                    if value is None:
                        values.append('NULL')
                    elif isinstance(value, str):
                        values.append(f"'{value.replace("'", "''")}'")
                    else:
                        values.append(str(value))

                f.write(f"INSERT INTO {table_name} ({', '.join(columns)}) VALUES ({', '.join(values)});\n")

            f.write("\n")

        # 转储索引
        cursor.execute("""
            SELECT sql
            FROM sqlite_master
            WHERE type='index'
            AND name NOT LIKE 'sqlite_%'
        """)

        indexes = cursor.fetchall()
        for (index_sql,) in indexes:
            if index_sql:
                f.write(f"{index_sql};\n\n")

    conn.close()
    return dump_file
```

### 1.4 增量备份

增量备份只备份自上次备份以来的变更。

```python
class IncrementalBackup:
    def __init__(self, db_path, backup_dir):
        self.db_path = db_path
        self.backup_dir = Path(backup_dir)
        self.backup_dir.mkdir(parents=True, exist_ok=True)
        self.last_backup_file = self.backup_dir / "last_backup.txt"

    def get_last_backup_time(self):
        """获取上次备份时间"""
        if self.last_backup_file.exists():
            with open(self.last_backup_file, 'r') as f:
                return datetime.fromisoformat(f.read().strip())
        return None

    def save_backup_time(self, backup_time):
        """保存备份时间"""
        with open(self.last_backup_file, 'w') as f:
            f.write(backup_time.isoformat())

    def incremental_backup(self):
        """执行增量备份"""
        last_backup = self.get_last_backup_time()
        current_time = datetime.now()

        # 获取自上次备份以来的变更
        conn = sqlite3.connect(self.db_path)
        cursor = conn.cursor()

        # 检查是否有变更（需要应用层支持）
        # 这里简化处理，实际应该检查WAL文件或变更日志

        # 执行备份
        backup_path = self.backup_dir / f"incremental_{current_time.strftime('%Y%m%d_%H%M%S')}.db"

        online_backup = OnlineBackup(self.db_path, str(backup_path))
        success = online_backup.backup()

        if success:
            self.save_backup_time(current_time)

        conn.close()
        return backup_path if success else None
```

---

## 2. 备份实现

### 2.1 Python备份实现

```python
class SQLiteBackupManager:
    def __init__(self, db_path, backup_dir, retention_days=30):
        self.db_path = Path(db_path)
        self.backup_dir = Path(backup_dir)
        self.backup_dir.mkdir(parents=True, exist_ok=True)
        self.retention_days = retention_days

    def full_backup(self, method='online'):
        """完整备份"""
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        backup_path = self.backup_dir / f"full_{timestamp}.db"

        if method == 'online':
            backup = OnlineBackup(str(self.db_path), str(backup_path))
            success = backup.backup()
        elif method == 'file_copy':
            backup_path = file_copy_backup(self.db_path, self.backup_dir)
            success = True
        elif method == 'sql_dump':
            backup_path = self.backup_dir / f"full_{timestamp}.sql"
            backup_path = sql_dump_backup(self.db_path, backup_path)
            success = True
        else:
            raise ValueError(f"未知的备份方法: {method}")

        if success:
            # 验证备份
            if self.verify_backup(backup_path):
                # 清理旧备份
                self.cleanup_old_backups()
                return backup_path
            else:
                backup_path.unlink()
                raise Exception("备份验证失败")

        return None

    def verify_backup(self, backup_path):
        """验证备份"""
        try:
            conn = sqlite3.connect(str(backup_path))
            cursor = conn.cursor()

            # 快速完整性检查
            cursor.execute("PRAGMA quick_check")
            result = cursor.fetchone()[0]

            conn.close()
            return result == 'ok'
        except Exception as e:
            print(f"备份验证失败: {e}")
            return False

    def cleanup_old_backups(self):
        """清理旧备份"""
        cutoff_date = datetime.now() - timedelta(days=self.retention_days)

        for backup_file in self.backup_dir.glob("*.db"):
            if backup_file.stat().st_mtime < cutoff_date.timestamp():
                backup_file.unlink()
                print(f"删除旧备份: {backup_file}")
```

### 2.2 C语言备份实现

见1.1节C语言实现部分。

### 2.3 Shell脚本备份

```bash
#!/bin/bash
# SQLite备份脚本

DB_PATH="$1"
BACKUP_DIR="${2:-./backups}"
RETENTION_DAYS="${3:-30}"

if [ -z "$DB_PATH" ]; then
    echo "用法: $0 <database_path> [backup_dir] [retention_days]"
    exit 1
fi

mkdir -p "$BACKUP_DIR"

# 生成备份文件名
TIMESTAMP=$(date +%Y%m%d_%H%M%S)
BACKUP_FILE="$BACKUP_DIR/backup_${TIMESTAMP}.db"

# 执行备份
sqlite3 "$DB_PATH" ".backup '$BACKUP_FILE'"

if [ $? -eq 0 ]; then
    echo "备份成功: $BACKUP_FILE"

    # 验证备份
    sqlite3 "$BACKUP_FILE" "PRAGMA quick_check" | grep -q "ok"
    if [ $? -eq 0 ]; then
        echo "备份验证通过"
    else
        echo "警告: 备份验证失败"
        rm "$BACKUP_FILE"
        exit 1
    fi

    # 清理旧备份
    find "$BACKUP_DIR" -name "backup_*.db" -mtime +$RETENTION_DAYS -delete
    echo "已清理 $RETENTION_DAYS 天前的备份"
else
    echo "备份失败"
    exit 1
fi
```

---

## 3. 恢复方法

### 3.1 完整恢复

```python
def restore_from_backup(backup_path, target_path):
    """从备份恢复数据库"""
    backup_path = Path(backup_path)
    target_path = Path(target_path)

    # 如果目标数据库存在，先备份
    if target_path.exists():
        old_backup = target_path.parent / f"{target_path.stem}_old_{datetime.now().strftime('%Y%m%d_%H%M%S')}.db"
        shutil.copy2(target_path, old_backup)
        print(f"已备份现有数据库到: {old_backup}")

    # 恢复数据库
    if backup_path.suffix == '.sql':
        # SQL转储恢复
        conn = sqlite3.connect(str(target_path))
        with open(backup_path, 'r', encoding='utf-8') as f:
            sql_script = f.read()
            conn.executescript(sql_script)
        conn.close()
    else:
        # 文件复制恢复
        shutil.copy2(backup_path, target_path)

    # 验证恢复
    conn = sqlite3.connect(str(target_path))
    cursor = conn.cursor()
    cursor.execute("PRAGMA quick_check")
    result = cursor.fetchone()[0]
    conn.close()

    if result == 'ok':
        print("恢复成功并验证通过")
        return True
    else:
        print(f"恢复验证失败: {result}")
        return False
```

### 3.2 部分恢复

```python
def partial_restore(backup_path, target_path, tables=None):
    """部分恢复（只恢复指定表）"""
    backup_conn = sqlite3.connect(str(backup_path))
    target_conn = sqlite3.connect(str(target_path))

    if tables is None:
        # 获取所有表
        cursor = backup_conn.cursor()
        cursor.execute("""
            SELECT name
            FROM sqlite_master
            WHERE type='table'
            AND name NOT LIKE 'sqlite_%'
        """)
        tables = [row[0] for row in cursor.fetchall()]

    for table in tables:
        # 删除目标表中的数据
        target_conn.execute(f"DELETE FROM {table}")

        # 从备份复制数据
        cursor = backup_conn.cursor()
        cursor.execute(f"SELECT * FROM {table}")
        rows = cursor.fetchall()

        # 获取列名
        cursor.execute(f"PRAGMA table_info({table})")
        columns = [row[1] for row in cursor.fetchall()]

        # 插入数据
        placeholders = ', '.join(['?'] * len(columns))
        insert_sql = f"INSERT INTO {table} ({', '.join(columns)}) VALUES ({placeholders})"
        target_conn.executemany(insert_sql, rows)

    target_conn.commit()
    backup_conn.close()
    target_conn.close()
```

### 3.3 时间点恢复

时间点恢复需要结合WAL日志和检查点。

```python
def point_in_time_restore(base_backup, wal_files, target_path, target_time):
    """时间点恢复"""
    # 恢复基础备份
    shutil.copy2(base_backup, target_path)

    # 应用WAL文件直到目标时间
    conn = sqlite3.connect(str(target_path))
    cursor = conn.cursor()

    for wal_file in sorted(wal_files):
        # 检查WAL文件时间戳
        wal_time = datetime.fromtimestamp(Path(wal_file).stat().st_mtime)
        if wal_time > target_time:
            break

        # 应用WAL文件
        # 注意：这需要SQLite的WAL恢复机制支持
        # 实际实现可能需要使用sqlite3命令行工具或C API

    conn.close()
```

---

## 4. WAL模式备份

### 4.1 WAL模式备份策略

```python
def wal_mode_backup(db_path, backup_path):
    """WAL模式下的备份"""
    conn = sqlite3.connect(str(db_path))
    cursor = conn.cursor()

    # 1. 执行检查点，将WAL内容合并到数据库
    cursor.execute("PRAGMA wal_checkpoint(TRUNCATE)")

    # 2. 等待检查点完成
    while True:
        cursor.execute("PRAGMA wal_checkpoint(PASSIVE)")
        result = cursor.fetchone()
        if result[0] == 0 and result[1] == 0:  # 检查点完成
            break
        time.sleep(0.1)

    # 3. 执行备份
    backup = OnlineBackup(str(db_path), str(backup_path))
    success = backup.backup()

    conn.close()
    return success
```

### 4.2 检查点与备份

```python
def checkpoint_and_backup(db_path, backup_path):
    """执行检查点后备份"""
    conn = sqlite3.connect(str(db_path))
    cursor = conn.cursor()

    # 获取当前WAL模式
    cursor.execute("PRAGMA journal_mode")
    journal_mode = cursor.fetchone()[0]

    if journal_mode == 'wal':
        # WAL模式：执行检查点
        cursor.execute("PRAGMA wal_checkpoint(TRUNCATE)")

        # 验证检查点完成
        cursor.execute("PRAGMA wal_checkpoint(PASSIVE)")
        checkpoint_result = cursor.fetchone()

        if checkpoint_result[0] != 0 or checkpoint_result[1] != 0:
            print("警告: 检查点未完全完成")

    conn.close()

    # 执行备份
    return wal_mode_backup(db_path, backup_path)
```

---

## 5. 备份验证

### 5.1 完整性验证

```python
def verify_backup_integrity(backup_path):
    """验证备份完整性"""
    conn = sqlite3.connect(str(backup_path))
    cursor = conn.cursor()

    # 快速检查
    cursor.execute("PRAGMA quick_check")
    quick_result = cursor.fetchone()[0]

    if quick_result != 'ok':
        conn.close()
        return False, f"快速检查失败: {quick_result}"

    # 完整检查
    cursor.execute("PRAGMA integrity_check")
    integrity_results = cursor.fetchall()

    conn.close()

    # 检查是否有错误
    for result in integrity_results:
        if result[0] != 'ok':
            return False, f"完整性检查失败: {result[0]}"

    return True, "验证通过"
```

### 5.2 一致性验证

```python
def verify_backup_consistency(original_path, backup_path):
    """验证备份一致性"""
    orig_conn = sqlite3.connect(str(original_path))
    backup_conn = sqlite3.connect(str(backup_path))

    orig_cursor = orig_conn.cursor()
    backup_cursor = backup_conn.cursor()

    # 获取所有表
    orig_cursor.execute("""
        SELECT name
        FROM sqlite_master
        WHERE type='table'
        AND name NOT LIKE 'sqlite_%'
    """)
    tables = [row[0] for row in orig_cursor.fetchall()]

    inconsistencies = []

    for table in tables:
        # 比较行数
        orig_cursor.execute(f"SELECT COUNT(*) FROM {table}")
        orig_count = orig_cursor.fetchone()[0]

        backup_cursor.execute(f"SELECT COUNT(*) FROM {table}")
        backup_count = backup_cursor.fetchone()[0]

        if orig_count != backup_count:
            inconsistencies.append(f"表 {table}: 行数不匹配 ({orig_count} vs {backup_count})")

    orig_conn.close()
    backup_conn.close()

    return len(inconsistencies) == 0, inconsistencies
```

---

## 6. 自动化备份

### 6.1 定时备份

```python
import schedule
import time

class AutomatedBackup:
    def __init__(self, db_path, backup_dir):
        self.backup_manager = SQLiteBackupManager(db_path, backup_dir)

    def daily_backup(self):
        """每日备份"""
        print(f"开始每日备份: {datetime.now()}")
        backup_path = self.backup_manager.full_backup()
        if backup_path:
            print(f"备份完成: {backup_path}")
        else:
            print("备份失败")

    def hourly_backup(self):
        """每小时备份"""
        print(f"开始每小时备份: {datetime.now()}")
        backup_path = self.backup_manager.full_backup()
        if backup_path:
            print(f"备份完成: {backup_path}")

    def start_scheduler(self):
        """启动定时备份"""
        # 每天凌晨2点备份
        schedule.every().day.at("02:00").do(self.daily_backup)

        # 每小时备份（可选）
        # schedule.every().hour.do(self.hourly_backup)

        print("定时备份已启动")
        while True:
            schedule.run_pending()
            time.sleep(60)
```

### 6.2 事件驱动备份

```python
class EventDrivenBackup:
    def __init__(self, db_path, backup_dir):
        self.db_path = db_path
        self.backup_manager = SQLiteBackupManager(db_path, backup_dir)
        self.last_backup_size = 0

    def check_and_backup(self):
        """检查变更并备份"""
        current_size = Path(self.db_path).stat().st_size

        # 如果数据库大小变化超过阈值，执行备份
        if abs(current_size - self.last_backup_size) > 1024 * 1024:  # 1MB
            print("检测到数据库变更，执行备份")
            backup_path = self.backup_manager.full_backup()
            if backup_path:
                self.last_backup_size = current_size
                return backup_path

        return None
```

---

## 7. 灾难恢复计划

### 7.1 RTO与RPO

```text
灾难恢复指标
══════════════════════════════════════════════════════════════════════════════

RTO (Recovery Time Objective) - 恢复时间目标
  • 目标: < 1小时
  • 措施: 自动化恢复脚本、预测试恢复流程

RPO (Recovery Point Objective) - 恢复点目标
  • 目标: < 1小时数据丢失
  • 措施: 每小时增量备份、实时WAL复制
```

### 7.2 恢复流程

```python
class DisasterRecovery:
    def __init__(self, backup_dir):
        self.backup_dir = Path(backup_dir)

    def list_available_backups(self):
        """列出可用备份"""
        backups = []
        for backup_file in sorted(self.backup_dir.glob("*.db"), reverse=True):
            backups.append({
                'path': backup_file,
                'time': datetime.fromtimestamp(backup_file.stat().st_mtime),
                'size': backup_file.stat().st_size
            })
        return backups

    def recover_to_latest(self, target_path):
        """恢复到最新备份"""
        backups = self.list_available_backups()
        if not backups:
            raise Exception("没有可用的备份")

        latest_backup = backups[0]
        print(f"恢复到备份: {latest_backup['path']} ({latest_backup['time']})")

        return restore_from_backup(latest_backup['path'], target_path)

    def recover_to_point(self, target_path, target_time):
        """恢复到指定时间点"""
        backups = self.list_available_backups()

        # 找到最接近目标时间的备份
        best_backup = None
        min_diff = float('inf')

        for backup in backups:
            diff = abs((backup['time'] - target_time).total_seconds())
            if diff < min_diff:
                min_diff = diff
                best_backup = backup

        if best_backup:
            print(f"恢复到备份: {best_backup['path']} ({best_backup['time']})")
            return restore_from_backup(best_backup['path'], target_path)
        else:
            raise Exception("未找到合适的备份")
```

---

## 7.3. 备份恢复决策树

```text
备份恢复决策树
══════════════════════════════════════════════════════════════════════════════

问题：如何选择备份恢复策略？
    │
    ├─ 备份场景？
    │   ├─ 生产环境在线备份？ → Online Backup API（推荐）
    │   │   ├─ 在线备份
    │   │   ├─ 一致性保证
    │   │   ├─ 性能好
    │   │   └─ 适合大型数据库
    │   │
    │   ├─ 小型数据库快速备份？ → 文件复制
    │   │   ├─ 简单快速
    │   │   ├─ 完整备份
    │   │   ├─ 需要停机
    │   │   └─ 适合维护窗口
    │   │
    │   ├─ 跨平台迁移？ → SQL转储
    │   │   ├─ 可读性强
    │   │   ├─ 可编辑
    │   │   ├─ 速度慢
    │   │   └─ 适合版本兼容
    │   │
    │   └─ 频繁备份需求？ → 增量备份
    │       ├─ 节省空间
    │       ├─ 快速备份
    │       ├─ 实现复杂
    │       └─ 适合网络备份
    │
    ├─ WAL模式？
    │   ├─ 使用WAL模式？ → WAL模式备份策略
    │   │   ├─ 检查点后备份
    │   │   ├─ 备份主文件+WAL文件
    │   │   └─ 一致性保证
    │   │
    │   └─ 不使用WAL模式？ → 标准备份
    │       ├─ 直接备份主文件
    │       └─ 需要停机
    │
    ├─ 备份频率？
    │   ├─ 每天备份？ → 定时备份
    │   │   ├─ cron任务
    │   │   ├─ 定期执行
    │   │   └─ 自动清理旧备份
    │   │
    │   ├─ 实时备份？ → 事件驱动备份
    │   │   ├─ 数据变更触发
    │   │   ├─ 实时同步
    │   │   └─ 实现复杂
    │   │
    │   └─ 按需备份？ → 手动备份
    │       ├─ 手动执行
    │       └─ 灵活控制
    │
    └─ 恢复需求？
        ├─ 完整恢复？ → 完整恢复流程
        │   ├─ 从备份文件恢复
        │   ├─ 验证完整性
        │   └─ 测试恢复
        │
        ├─ 部分恢复？ → 部分恢复流程
        │   ├─ 选择性恢复
        │   ├─ 表级恢复
        │   └─ 数据迁移
        │
        └─ 时间点恢复？ → 时间点恢复流程
            ├─ 增量备份链
            ├─ 日志回放
            └─ 复杂实现

推荐备份策略:
1. 生产环境（推荐）:
   - Online Backup API
   - WAL模式
   - 每天备份
   - 保留30天

2. 小型数据库:
   - 文件复制
   - 每天备份
   - 保留7天

3. 跨平台迁移:
   - SQL转储
   - 按需备份
   - 版本兼容
```

---

## 8. 备份最佳实践

### 8.1 备份策略

1. **3-2-1规则**
   - 3份备份副本
   - 2种不同介质
   - 1份异地备份

2. **备份频率**
   - 生产环境：每小时增量备份 + 每日完整备份
   - 开发环境：每日完整备份

3. **备份验证**
   - 每次备份后立即验证
   - 定期测试恢复流程

4. **备份存储**
   - 本地备份：快速恢复
   - 远程备份：灾难恢复
   - 版本控制：保留多个版本

### 8.2 恢复测试

```python
def test_recovery_process(backup_dir, test_db_path):
    """测试恢复流程"""
    dr = DisasterRecovery(backup_dir)

    # 列出备份
    backups = dr.list_available_backups()
    print(f"可用备份数量: {len(backups)}")

    # 测试恢复到最新
    test_path = Path(test_db_path)
    if test_path.exists():
        test_path.unlink()

    success = dr.recover_to_latest(test_db_path)

    if success:
        # 验证恢复的数据库
        conn = sqlite3.connect(test_db_path)
        cursor = conn.cursor()
        cursor.execute("PRAGMA quick_check")
        result = cursor.fetchone()[0]
        conn.close()

        if result == 'ok':
            print("恢复测试成功")
            return True

    print("恢复测试失败")
    return False
```

---

## 9. 🔗 相关资源

### 内部资源

- [生产环境监控与诊断](./08.13-SQLite生产环境监控与诊断.md) - 监控和诊断工具
- [错误处理](./08.05-错误处理.md) - 错误处理和恢复
- [PRAGMA配置](./08.04-PRAGMA配置.md) - WAL模式配置
- [事务管理](./08.02-事务管理.md) - 事务和一致性

### 外部资源

- [SQLite官方文档 - Online Backup API](https://www.sqlite.org/backup.html)
- [SQLite官方文档 - WAL模式](https://www.sqlite.org/wal.html)
- [SQLite官方文档 - 检查点](https://www.sqlite.org/wal.html#checkpointing)

---

## 🔗 交叉引用

### 编程实践文档

- ⭐⭐⭐ [生产环境监控与诊断](./08.13-SQLite生产环境监控与诊断.md) - 监控和诊断工具（含交叉引用）
- ⭐⭐ [错误处理](./08.05-错误处理.md) - 错误处理和恢复（含交叉引用）
- ⭐⭐ [安全与加密](./08.15-SQLite安全与加密完整指南.md) - 备份加密（含交叉引用）

### 核心架构文档 🆕

- ⭐⭐⭐ [事务与并发控制](../01-核心架构/01.02-事务与并发控制.md) - WAL模式备份基础（含论证脉络）
- ⭐⭐ [存储引擎](../01-核心架构/01.03-存储引擎.md) - 存储备份基础

### 形式化理论文档 🆕

- ⭐⭐ [WAL模式形式化验证](../06-形式化理论/06.04-WAL模式形式化验证.md) - WAL恢复理论（含交叉引用）
- ⭐⭐ [ACID原子性证明](../06-形式化理论/06.01-ACID原子性证明.md) - 恢复原子性理论

### 知识图谱与导航 🆕

- ⭐⭐⭐ [知识图谱与概念关系网络](../09-最新特性/09.03-SQLite知识图谱与概念关系网络.md) - 备份恢复概念关系（400+概念）
- ⭐⭐ [文档依赖关系图](../00-项目导航/06-文档依赖关系图.md) - 备份恢复文档依赖
- ⭐⭐ [术语标准化词典](../00-项目导航/03-术语词典/SQLite术语标准化词典.md) - 备份恢复术语索引

### 相关概念链接 🆕

#### 备份恢复概念

- **备份恢复** → [知识图谱：开发实践本体](../09-最新特性/09.03-SQLite知识图谱与概念关系网络.md#开发实践本体)
- **WAL备份** → [知识图谱：WAL机制](../09-最新特性/09.03-SQLite知识图谱与概念关系网络.md#核心本体wal机制)
- **崩溃恢复** → [知识图谱：崩溃恢复概念](../09-最新特性/09.03-SQLite知识图谱与概念关系网络.md#核心本体wal机制)

---

**最后更新**: 2025-12-05
**维护者**: Data-Science Team

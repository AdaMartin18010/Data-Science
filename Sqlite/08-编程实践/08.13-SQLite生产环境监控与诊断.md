# SQLite生产环境监控与诊断

> **创建日期**: 2025-12-05
> **版本**: SQLite 3.47.x
> **难度**: ⭐⭐⭐⭐⭐
> **适用场景**: 生产环境运维、性能调优、故障排查

---

## 📑 目录

- [SQLite生产环境监控与诊断](#sqlite生产环境监控与诊断)
  - [📑 目录](#-目录)
  - [监控体系总览](#监控体系总览)
  - [1. 性能监控](#1-性能监控)
    - [1.1 查询性能监控](#11-查询性能监控)
      - [慢查询检测](#慢查询检测)
      - [查询计划分析](#查询计划分析)
    - [1.2 事务性能监控](#12-事务性能监控)
    - [1.3 连接池监控](#13-连接池监控)
    - [1.4 缓存命中率监控](#14-缓存命中率监控)
  - [2. 资源监控](#2-资源监控)
    - [2.1 数据库文件大小监控](#21-数据库文件大小监控)
    - [2.2 磁盘空间监控](#22-磁盘空间监控)
    - [2.3 内存使用监控](#23-内存使用监控)
    - [2.4 WAL文件监控](#24-wal文件监控)
  - [3. 健康检查](#3-健康检查)
    - [3.1 数据库完整性检查](#31-数据库完整性检查)
    - [3.2 索引有效性检查](#32-索引有效性检查)
    - [3.3 统计信息更新](#33-统计信息更新)
    - [3.4 碎片检查](#34-碎片检查)
  - [4. 诊断工具](#4-诊断工具)
    - [4.1 PRAGMA诊断命令](#41-pragma诊断命令)
    - [4.2 EXPLAIN QUERY PLAN分析](#42-explain-query-plan分析)
    - [4.3 日志分析](#43-日志分析)
    - [4.4 性能分析工具](#44-性能分析工具)
  - [5. 故障排查](#5-故障排查)
    - [5.1 锁定问题排查](#51-锁定问题排查)
    - [5.2 性能下降排查](#52-性能下降排查)
    - [5.3 数据库损坏排查](#53-数据库损坏排查)
    - [5.4 内存泄漏排查](#54-内存泄漏排查)
  - [6. 监控指标矩阵](#6-监控指标矩阵)
  - [7. 告警策略](#7-告警策略)
  - [8. 监控工具实现](#8-监控工具实现)
    - [8.1 Python监控脚本](#81-python监控脚本)
    - [8.2 Shell监控脚本](#82-shell监控脚本)
  - [8.3. 监控诊断决策树](#83-监控诊断决策树)
  - [9. 最佳实践](#9-最佳实践)
    - [9.1 监控最佳实践](#91-监控最佳实践)
    - [9.2 诊断最佳实践](#92-诊断最佳实践)
  - [10. 🔗 相关资源](#10--相关资源)
    - [内部资源](#内部资源)
    - [外部资源](#外部资源)
  - [🔗 交叉引用](#-交叉引用)
    - [编程实践文档](#编程实践文档)
    - [核心架构文档 🆕](#核心架构文档-)
    - [性能优化文档 🆕](#性能优化文档-)
    - [知识图谱与导航 🆕](#知识图谱与导航-)
    - [相关概念链接 🆕](#相关概念链接-)
      - [监控诊断概念](#监控诊断概念)

---

## 监控体系总览

```text
SQLite生产环境监控体系
══════════════════════════════════════════════════════════════════════════════

┌─────────────────────────────────────────────────────────────────────────┐
│                        监控维度                                          │
├─────────────────────────────────────────────────────────────────────────┤
│ 1. 性能监控                                                              │
│    • 查询执行时间                                                        │
│    • 事务吞吐量                                                          │
│    • 连接池状态                                                          │
│    • 缓存命中率                                                          │
│                                                                         │
│ 2. 资源监控                                                              │
│    • 数据库文件大小                                                      │
│    • 磁盘空间使用                                                        │
│    • 内存占用                                                            │
│    • WAL文件大小                                                         │
│                                                                         │
│ 3. 健康检查                                                              │
│    • 数据库完整性                                                        │
│    • 索引有效性                                                          │
│    • 统计信息准确性                                                      │
│    • 碎片程度                                                            │
│                                                                         │
│ 4. 故障诊断                                                              │
│    • 锁定问题                                                            │
│    • 性能瓶颈                                                            │
│    • 数据损坏                                                            │
│    • 内存泄漏                                                            │
└─────────────────────────────────────────────────────────────────────────┘
```

---

## 1. 性能监控

### 1.1 查询性能监控

#### 慢查询检测

```python
import sqlite3
import time
import logging
from contextlib import contextmanager

class QueryMonitor:
    def __init__(self, slow_query_threshold=1.0):
        self.slow_query_threshold = slow_query_threshold
        self.logger = logging.getLogger(__name__)

    @contextmanager
    def monitor_query(self, query, params=None):
        """监控查询执行时间"""
        start_time = time.time()
        try:
            yield
        finally:
            elapsed = time.time() - start_time
            if elapsed > self.slow_query_threshold:
                self.logger.warning(
                    f"慢查询检测: {elapsed:.3f}s\n"
                    f"SQL: {query}\n"
                    f"参数: {params}"
                )

    def get_query_stats(self, conn):
        """获取查询统计信息"""
        cursor = conn.cursor()

        # 获取查询计划统计
        stats = {}

        # 检查是否有活跃的长时间运行查询
        cursor.execute("""
            SELECT
                sql,
                COUNT(*) as count,
                AVG(time) as avg_time
            FROM sqlite_stat1
            GROUP BY sql
        """)

        return stats
```

#### 查询计划分析

```sql
-- 分析查询计划
EXPLAIN QUERY PLAN
SELECT * FROM users WHERE email = ?;

-- 获取查询计划统计
PRAGMA query_only = 1;
SELECT * FROM users WHERE email = ?;
PRAGMA query_only = 0;

-- 检查索引使用情况
SELECT
    name,
    tbl_name,
    sql
FROM sqlite_master
WHERE type = 'index'
ORDER BY name;
```

### 1.2 事务性能监控

```python
class TransactionMonitor:
    def __init__(self):
        self.transaction_stats = {
            'total': 0,
            'committed': 0,
            'rolled_back': 0,
            'avg_duration': 0.0,
            'max_duration': 0.0
        }

    @contextmanager
    def monitor_transaction(self, conn):
        """监控事务执行"""
        start_time = time.time()
        self.transaction_stats['total'] += 1

        try:
            yield
            conn.commit()
            self.transaction_stats['committed'] += 1
        except Exception as e:
            conn.rollback()
            self.transaction_stats['rolled_back'] += 1
            raise
        finally:
            duration = time.time() - start_time
            self.transaction_stats['avg_duration'] = (
                (self.transaction_stats['avg_duration'] *
                 (self.transaction_stats['total'] - 1) + duration) /
                self.transaction_stats['total']
            )
            self.transaction_stats['max_duration'] = max(
                self.transaction_stats['max_duration'],
                duration
            )

    def get_stats(self):
        """获取事务统计"""
        return self.transaction_stats.copy()
```

### 1.3 连接池监控

```python
class ConnectionPoolMonitor:
    def __init__(self, pool):
        self.pool = pool

    def get_pool_stats(self):
        """获取连接池统计"""
        return {
            'total_connections': self.pool._size,
            'active_connections': len(self.pool._connections),
            'idle_connections': self.pool._size - len(self.pool._connections),
            'max_connections': self.pool._maxsize,
            'wait_count': getattr(self.pool, '_wait_count', 0)
        }

    def check_connection_health(self, conn):
        """检查连接健康状态"""
        try:
            cursor = conn.cursor()
            cursor.execute("SELECT 1")
            cursor.fetchone()
            return True
        except Exception as e:
            return False
```

### 1.4 缓存命中率监控

```sql
-- 获取页面缓存统计
PRAGMA cache_size;
PRAGMA page_count;
PRAGMA page_size;

-- 计算缓存命中率（需要应用层统计）
-- 命中率 = (总查询 - 磁盘读取) / 总查询
```

```python
class CacheMonitor:
    def __init__(self):
        self.cache_stats = {
            'hits': 0,
            'misses': 0,
            'total': 0
        }

    def record_hit(self):
        self.cache_stats['hits'] += 1
        self.cache_stats['total'] += 1

    def record_miss(self):
        self.cache_stats['misses'] += 1
        self.cache_stats['total'] += 1

    def get_hit_rate(self):
        if self.cache_stats['total'] == 0:
            return 0.0
        return self.cache_stats['hits'] / self.cache_stats['total']
```

---

## 2. 资源监控

### 2.1 数据库文件大小监控

```python
import os
from pathlib import Path

def get_database_size(db_path):
    """获取数据库文件大小"""
    db_file = Path(db_path)
    if not db_file.exists():
        return None

    size_bytes = db_file.stat().st_size
    return {
        'bytes': size_bytes,
        'kb': size_bytes / 1024,
        'mb': size_bytes / (1024 * 1024),
        'gb': size_bytes / (1024 * 1024 * 1024)
    }

def get_database_info(conn):
    """获取数据库详细信息"""
    cursor = conn.cursor()

    # 页面信息
    cursor.execute("PRAGMA page_count")
    page_count = cursor.fetchone()[0]

    cursor.execute("PRAGMA page_size")
    page_size = cursor.fetchone()[0]

    # 表数量
    cursor.execute("""
        SELECT COUNT(*)
        FROM sqlite_master
        WHERE type = 'table'
    """)
    table_count = cursor.fetchone()[0]

    # 索引数量
    cursor.execute("""
        SELECT COUNT(*)
        FROM sqlite_master
        WHERE type = 'index'
    """)
    index_count = cursor.fetchone()[0]

    return {
        'page_count': page_count,
        'page_size': page_size,
        'calculated_size': page_count * page_size,
        'table_count': table_count,
        'index_count': index_count
    }
```

### 2.2 磁盘空间监控

```python
import shutil

def check_disk_space(db_path):
    """检查磁盘空间"""
    db_dir = Path(db_path).parent
    stat = shutil.disk_usage(db_dir)

    return {
        'total': stat.total,
        'used': stat.used,
        'free': stat.free,
        'percent_used': (stat.used / stat.total) * 100,
        'percent_free': (stat.free / stat.total) * 100
    }

def check_wal_size(db_path):
    """检查WAL文件大小"""
    wal_path = Path(f"{db_path}-wal")
    if wal_path.exists():
        return wal_path.stat().st_size
    return 0
```

### 2.3 内存使用监控

```sql
-- 获取内存配置
PRAGMA cache_size;        -- 页面缓存大小（页数）
PRAGMA mmap_size;         -- 内存映射大小
PRAGMA temp_store;        -- 临时存储位置

-- 获取当前内存使用（需要应用层监控）
```

```python
import psutil
import os

def get_sqlite_memory_usage(process_id=None):
    """获取SQLite进程内存使用"""
    if process_id is None:
        process_id = os.getpid()

    process = psutil.Process(process_id)
    memory_info = process.memory_info()

    return {
        'rss': memory_info.rss,  # 物理内存
        'vms': memory_info.vms,  # 虚拟内存
        'percent': process.memory_percent()
    }
```

### 2.4 WAL文件监控

```python
def monitor_wal_file(db_path):
    """监控WAL文件状态"""
    wal_path = Path(f"{db_path}-wal")
    shm_path = Path(f"{db_path}-shm")

    wal_info = {}

    if wal_path.exists():
        wal_info['size'] = wal_path.stat().st_size
        wal_info['exists'] = True
    else:
        wal_info['exists'] = False
        wal_info['size'] = 0

    if shm_path.exists():
        wal_info['shm_size'] = shm_path.stat().st_size
        wal_info['shm_exists'] = True
    else:
        wal_info['shm_exists'] = False
        wal_info['shm_size'] = 0

    # 检查WAL模式
    conn = sqlite3.connect(db_path)
    cursor = conn.cursor()
    cursor.execute("PRAGMA journal_mode")
    wal_info['journal_mode'] = cursor.fetchone()[0]
    conn.close()

    return wal_info
```

---

## 3. 健康检查

### 3.1 数据库完整性检查

```python
def integrity_check(conn):
    """完整性检查"""
    cursor = conn.cursor()

    # 快速完整性检查
    cursor.execute("PRAGMA quick_check")
    quick_result = cursor.fetchone()[0]

    # 完整完整性检查
    cursor.execute("PRAGMA integrity_check")
    integrity_result = cursor.fetchall()

    return {
        'quick_check': quick_result,
        'integrity_check': integrity_result,
        'is_healthy': quick_result == 'ok'
    }
```

### 3.2 索引有效性检查

```python
def check_indexes(conn):
    """检查索引有效性"""
    cursor = conn.cursor()

    # 获取所有索引
    cursor.execute("""
        SELECT
            name,
            tbl_name,
            sql
        FROM sqlite_master
        WHERE type = 'index'
        AND name NOT LIKE 'sqlite_%'
    """)

    indexes = cursor.fetchall()

    # 检查每个索引的使用情况
    index_stats = []
    for index_name, table_name, sql in indexes:
        cursor.execute(f"""
            SELECT COUNT(*)
            FROM sqlite_stat1
            WHERE tbl = '{table_name}' AND idx = '{index_name}'
        """)
        has_stats = cursor.fetchone()[0] > 0

        index_stats.append({
            'name': index_name,
            'table': table_name,
            'has_statistics': has_stats
        })

    return index_stats
```

### 3.3 统计信息更新

```python
def update_statistics(conn):
    """更新统计信息"""
    cursor = conn.cursor()

    # 更新查询优化器统计
    cursor.execute("ANALYZE")

    # 检查统计信息
    cursor.execute("""
        SELECT
            tbl,
            idx,
            stat
        FROM sqlite_stat1
    """)

    return cursor.fetchall()
```

### 3.4 碎片检查

```python
def check_fragmentation(conn, db_path):
    """检查数据库碎片"""
    cursor = conn.cursor()

    # 获取页面信息
    cursor.execute("PRAGMA page_count")
    page_count = cursor.fetchone()[0]

    cursor.execute("PRAGMA freelist_count")
    freelist_count = cursor.fetchone()[0]

    # 计算碎片率
    if page_count > 0:
        fragmentation = (freelist_count / page_count) * 100
    else:
        fragmentation = 0

    # 获取实际文件大小
    file_size = Path(db_path).stat().st_size
    cursor.execute("PRAGMA page_size")
    page_size = cursor.fetchone()[0]
    expected_size = page_count * page_size

    return {
        'page_count': page_count,
        'freelist_count': freelist_count,
        'fragmentation_percent': fragmentation,
        'file_size': file_size,
        'expected_size': expected_size,
        'overhead': file_size - expected_size
    }
```

---

## 4. 诊断工具

### 4.1 PRAGMA诊断命令

```sql
-- 数据库配置诊断
PRAGMA database_list;           -- 列出所有附加数据库
PRAGMA table_info(table_name);  -- 表结构信息
PRAGMA index_info(index_name);  -- 索引信息
PRAGMA index_list(table_name);  -- 表的所有索引

-- 性能诊断
PRAGMA cache_size;              -- 缓存大小
PRAGMA page_size;               -- 页面大小
PRAGMA page_count;              -- 页面数量
PRAGMA freelist_count;          -- 空闲页面数

-- WAL诊断
PRAGMA journal_mode;            -- 日志模式
PRAGMA wal_checkpoint;          -- 检查点
PRAGMA wal_autocheckpoint;      -- 自动检查点设置

-- 完整性诊断
PRAGMA quick_check;             -- 快速完整性检查
PRAGMA integrity_check;         -- 完整完整性检查
PRAGMA foreign_key_check;       -- 外键检查

-- 优化诊断
PRAGMA optimize;                -- 运行优化（SQLite 3.18+）
PRAGMA optimize(0x03);          -- 指定优化选项
```

### 4.2 EXPLAIN QUERY PLAN分析

```python
def analyze_query_plan(conn, query, params=None):
    """分析查询计划"""
    cursor = conn.cursor()

    # 获取查询计划
    cursor.execute(f"EXPLAIN QUERY PLAN {query}", params or [])
    plan = cursor.fetchall()

    # 解析查询计划
    plan_info = []
    for row in plan:
        plan_info.append({
            'selectid': row[0],
            'order': row[1],
            'from': row[2],
            'detail': row[3]
        })

    return plan_info

# 使用示例
conn = sqlite3.connect('example.db')
plan = analyze_query_plan(
    conn,
    "SELECT * FROM users WHERE email = ?",
    ('user@example.com',)
)
print(plan)
```

### 4.3 日志分析

```python
import logging
from datetime import datetime

class SQLiteLogger:
    def __init__(self, log_file='sqlite_monitor.log'):
        self.logger = logging.getLogger('sqlite_monitor')
        self.logger.setLevel(logging.INFO)

        handler = logging.FileHandler(log_file)
        formatter = logging.Formatter(
            '%(asctime)s - %(name)s - %(levelname)s - %(message)s'
        )
        handler.setFormatter(formatter)
        self.logger.addHandler(handler)

    def log_slow_query(self, query, duration, params=None):
        """记录慢查询"""
        self.logger.warning(
            f"慢查询: {duration:.3f}s\n"
            f"SQL: {query}\n"
            f"参数: {params}"
        )

    def log_error(self, error, context=None):
        """记录错误"""
        self.logger.error(
            f"错误: {error}\n"
            f"上下文: {context}"
        )

    def log_health_check(self, result):
        """记录健康检查结果"""
        self.logger.info(f"健康检查: {result}")
```

### 4.4 性能分析工具

```python
import cProfile
import pstats
from io import StringIO

def profile_query(conn, query, params=None, output_file=None):
    """性能分析查询"""
    profiler = cProfile.Profile()

    def execute_query():
        cursor = conn.cursor()
        cursor.execute(query, params or [])
        return cursor.fetchall()

    profiler.enable()
    result = execute_query()
    profiler.disable()

    # 生成报告
    s = StringIO()
    ps = pstats.Stats(profiler, stream=s)
    ps.sort_stats('cumulative')
    ps.print_stats()

    report = s.getvalue()

    if output_file:
        with open(output_file, 'w') as f:
            f.write(report)

    return result, report
```

---

## 5. 故障排查

### 5.1 锁定问题排查

```python
def diagnose_locking_issues(db_path):
    """诊断锁定问题"""
    diagnostics = {}

    # 检查WAL模式
    conn = sqlite3.connect(db_path)
    cursor = conn.cursor()
    cursor.execute("PRAGMA journal_mode")
    journal_mode = cursor.fetchone()[0]
    diagnostics['journal_mode'] = journal_mode

    # 检查是否有锁文件
    lock_files = [
        f"{db_path}-wal",
        f"{db_path}-shm",
        f"{db_path}-journal"
    ]

    for lock_file in lock_files:
        path = Path(lock_file)
        if path.exists():
            diagnostics[f"{path.name}_exists"] = True
            diagnostics[f"{path.name}_size"] = path.stat().st_size
        else:
            diagnostics[f"{path.name}_exists"] = False

    # 尝试获取锁（非阻塞）
    try:
        cursor.execute("BEGIN IMMEDIATE")
        cursor.execute("ROLLBACK")
        diagnostics['can_acquire_lock'] = True
    except sqlite3.OperationalError as e:
        diagnostics['can_acquire_lock'] = False
        diagnostics['lock_error'] = str(e)

    conn.close()
    return diagnostics
```

### 5.2 性能下降排查

```python
def diagnose_performance_issues(conn):
    """诊断性能问题"""
    diagnostics = {}
    cursor = conn.cursor()

    # 检查统计信息
    cursor.execute("SELECT COUNT(*) FROM sqlite_stat1")
    stats_count = cursor.fetchone()[0]
    diagnostics['has_statistics'] = stats_count > 0

    # 检查索引
    cursor.execute("""
        SELECT COUNT(*)
        FROM sqlite_master
        WHERE type = 'index'
    """)
    index_count = cursor.fetchone()[0]
    diagnostics['index_count'] = index_count

    # 检查缓存配置
    cursor.execute("PRAGMA cache_size")
    cache_size = cursor.fetchone()[0]
    diagnostics['cache_size'] = cache_size

    # 检查页面大小
    cursor.execute("PRAGMA page_size")
    page_size = cursor.fetchone()[0]
    diagnostics['page_size'] = page_size

    # 检查碎片
    cursor.execute("PRAGMA page_count")
    page_count = cursor.fetchone()[0]
    cursor.execute("PRAGMA freelist_count")
    freelist_count = cursor.fetchone()[0]
    if page_count > 0:
        diagnostics['fragmentation'] = (freelist_count / page_count) * 100
    else:
        diagnostics['fragmentation'] = 0

    return diagnostics
```

### 5.3 数据库损坏排查

```python
def diagnose_corruption(db_path):
    """诊断数据库损坏"""
    diagnostics = {}

    conn = sqlite3.connect(db_path)
    cursor = conn.cursor()

    # 快速检查
    try:
        cursor.execute("PRAGMA quick_check")
        quick_result = cursor.fetchone()[0]
        diagnostics['quick_check'] = quick_check
        diagnostics['is_corrupted'] = quick_result != 'ok'
    except Exception as e:
        diagnostics['quick_check'] = f"错误: {e}"
        diagnostics['is_corrupted'] = True

    # 完整检查
    if not diagnostics.get('is_corrupted'):
        try:
            cursor.execute("PRAGMA integrity_check")
            integrity_result = cursor.fetchall()
            diagnostics['integrity_check'] = integrity_result
        except Exception as e:
            diagnostics['integrity_check'] = f"错误: {e}"
            diagnostics['is_corrupted'] = True

    conn.close()
    return diagnostics
```

### 5.4 内存泄漏排查

```python
import tracemalloc

class MemoryLeakDetector:
    def __init__(self):
        self.snapshots = []

    def start_tracing(self):
        """开始内存跟踪"""
        tracemalloc.start()

    def take_snapshot(self, label=''):
        """拍摄内存快照"""
        snapshot = tracemalloc.take_snapshot()
        self.snapshots.append({
            'label': label,
            'snapshot': snapshot,
            'timestamp': time.time()
        })
        return snapshot

    def compare_snapshots(self, index1, index2):
        """比较两个快照"""
        if index1 >= len(self.snapshots) or index2 >= len(self.snapshots):
            return None

        snapshot1 = self.snapshots[index1]['snapshot']
        snapshot2 = self.snapshots[index2]['snapshot']

        top_stats = snapshot2.compare_to(snapshot1, 'lineno')

        return top_stats

    def get_memory_stats(self):
        """获取内存统计"""
        current, peak = tracemalloc.get_traced_memory()
        return {
            'current_mb': current / 1024 / 1024,
            'peak_mb': peak / 1024 / 1024
        }
```

---

## 6. 监控指标矩阵

```text
SQLite监控指标矩阵
══════════════════════════════════════════════════════════════════════════════

┌─────────────────────┬──────────┬──────────┬──────────┬──────────┐
│ 监控指标            │ 正常范围 │ 警告阈值 │ 危险阈值 │ 检查频率 │
├─────────────────────┼──────────┼──────────┼──────────┼──────────┤
│ 查询执行时间        │ < 100ms  │ 100-500ms│ > 500ms  │ 实时     │
│ 事务吞吐量          │ > 1000/s │ 500-1000 │ < 500/s  │ 每分钟   │
│ 连接池使用率        │ < 70%    │ 70-90%   │ > 90%    │ 每分钟   │
│ 缓存命中率          │ > 90%    │ 80-90%   │ < 80%    │ 每分钟   │
│ 数据库文件大小      │ 动态     │ 增长>10% │ 增长>50% │ 每小时   │
│ 磁盘空间使用率      │ < 70%    │ 70-85%   │ > 85%    │ 每小时   │
│ WAL文件大小         │ < 100MB  │ 100-500MB│ > 500MB  │ 每小时   │
│ 碎片率              │ < 5%     │ 5-10%    │ > 10%    │ 每天     │
│ 完整性检查          │ OK       │ -        │ 失败     │ 每天     │
│ 锁定等待时间        │ < 1s     │ 1-5s     │ > 5s     │ 实时     │
└─────────────────────┴──────────┴──────────┴──────────┴──────────┘
```

---

## 7. 告警策略

```python
class AlertManager:
    def __init__(self):
        self.alert_rules = {
            'slow_query': {
                'threshold': 1.0,  # 秒
                'action': self.alert_slow_query
            },
            'high_fragmentation': {
                'threshold': 10.0,  # 百分比
                'action': self.alert_fragmentation
            },
            'low_disk_space': {
                'threshold': 15.0,  # 百分比剩余
                'action': self.alert_disk_space
            },
            'corruption': {
                'threshold': None,  # 任何损坏
                'action': self.alert_corruption
            }
        }

    def check_and_alert(self, metric_name, value):
        """检查指标并触发告警"""
        if metric_name in self.alert_rules:
            rule = self.alert_rules[metric_name]
            if rule['threshold'] is None or value > rule['threshold']:
                rule['action'](metric_name, value)

    def alert_slow_query(self, metric, value):
        """慢查询告警"""
        print(f"⚠️ 告警: 慢查询检测 - {value:.3f}秒")

    def alert_fragmentation(self, metric, value):
        """碎片告警"""
        print(f"⚠️ 告警: 高碎片率 - {value:.2f}%")

    def alert_disk_space(self, metric, value):
        """磁盘空间告警"""
        print(f"⚠️ 告警: 磁盘空间不足 - 剩余{value:.2f}%")

    def alert_corruption(self, metric, value):
        """损坏告警"""
        print(f"🚨 严重告警: 数据库损坏检测到！")
```

---

## 8. 监控工具实现

### 8.1 Python监控脚本

```python
#!/usr/bin/env python3
"""
SQLite生产环境监控脚本
"""

import sqlite3
import time
import json
from pathlib import Path
from datetime import datetime

class SQLiteMonitor:
    def __init__(self, db_path, config=None):
        self.db_path = db_path
        self.config = config or {}
        self.metrics = {}

    def collect_all_metrics(self):
        """收集所有监控指标"""
        conn = sqlite3.connect(self.db_path)

        try:
            metrics = {
                'timestamp': datetime.now().isoformat(),
                'database': str(self.db_path),
                'performance': self._collect_performance_metrics(conn),
                'resources': self._collect_resource_metrics(conn),
                'health': self._collect_health_metrics(conn)
            }

            return metrics
        finally:
            conn.close()

    def _collect_performance_metrics(self, conn):
        """收集性能指标"""
        cursor = conn.cursor()

        cursor.execute("PRAGMA cache_size")
        cache_size = cursor.fetchone()[0]

        cursor.execute("PRAGMA page_count")
        page_count = cursor.fetchone()[0]

        return {
            'cache_size': cache_size,
            'page_count': page_count
        }

    def _collect_resource_metrics(self, conn):
        """收集资源指标"""
        db_file = Path(self.db_path)
        file_size = db_file.stat().st_size if db_file.exists() else 0

        wal_file = Path(f"{self.db_path}-wal")
        wal_size = wal_file.stat().st_size if wal_file.exists() else 0

        return {
            'database_size_mb': file_size / (1024 * 1024),
            'wal_size_mb': wal_size / (1024 * 1024)
        }

    def _collect_health_metrics(self, conn):
        """收集健康指标"""
        cursor = conn.cursor()

        cursor.execute("PRAGMA quick_check")
        quick_check = cursor.fetchone()[0]

        cursor.execute("PRAGMA page_count")
        page_count = cursor.fetchone()[0]

        cursor.execute("PRAGMA freelist_count")
        freelist_count = cursor.fetchone()[0]

        fragmentation = (freelist_count / page_count * 100) if page_count > 0 else 0

        return {
            'integrity': quick_check == 'ok',
            'fragmentation_percent': fragmentation
        }

    def generate_report(self, output_file=None):
        """生成监控报告"""
        metrics = self.collect_all_metrics()

        report = {
            'summary': {
                'database': metrics['database'],
                'timestamp': metrics['timestamp'],
                'status': 'healthy' if metrics['health']['integrity'] else 'unhealthy'
            },
            'details': metrics
        }

        if output_file:
            with open(output_file, 'w') as f:
                json.dump(report, f, indent=2)

        return report

if __name__ == '__main__':
    import sys

    if len(sys.argv) < 2:
        print("用法: python monitor.py <database_path> [output_file]")
        sys.exit(1)

    db_path = sys.argv[1]
    output_file = sys.argv[2] if len(sys.argv) > 2 else None

    monitor = SQLiteMonitor(db_path)
    report = monitor.generate_report(output_file)

    print(json.dumps(report, indent=2))
```

### 8.2 Shell监控脚本

```bash
#!/bin/bash
# SQLite监控脚本

DB_PATH="$1"
OUTPUT_FILE="${2:-monitor_report.json}"

if [ -z "$DB_PATH" ]; then
    echo "用法: $0 <database_path> [output_file]"
    exit 1
fi

# 收集指标
TIMESTAMP=$(date -Iseconds)
DB_SIZE=$(stat -f%z "$DB_PATH" 2>/dev/null || stat -c%s "$DB_PATH" 2>/dev/null)
WAL_SIZE=$(stat -f%z "${DB_PATH}-wal" 2>/dev/null || stat -c%s "${DB_PATH}-wal" 2>/dev/null || echo "0")

# 生成报告
cat > "$OUTPUT_FILE" <<EOF
{
  "timestamp": "$TIMESTAMP",
  "database": "$DB_PATH",
  "metrics": {
    "database_size_bytes": $DB_SIZE,
    "wal_size_bytes": $WAL_SIZE
  }
}
EOF

echo "监控报告已生成: $OUTPUT_FILE"
```

---

## 8.3. 监控诊断决策树

```text
监控诊断决策树
══════════════════════════════════════════════════════════════════════════════

问题：如何进行SQLite生产环境监控与诊断？
    │
    ├─ 监控维度？
    │   ├─ 性能监控？
    │   │   ├─ 是 → 性能监控
    │   │   │   ├─ 查询性能监控
    │   │   │   │   ├─ 慢查询检测
    │   │   │   │   └─ 查询计划分析
    │   │   │   │
    │   │   │   ├─ 事务性能监控
    │   │   │   │   ├─ 事务吞吐量
    │   │   │   │   └─ 事务延迟
    │   │   │   │
    │   │   │   ├─ 连接池监控
    │   │   │   │   ├─ 连接数
    │   │   │   │   └─ 连接使用率
    │   │   │   │
    │   │   │   └─ 缓存命中率监控
    │   │   │       ├─ 页缓存命中率
    │   │   │       └─ 查询缓存命中率
    │   │   │
    │   │   └─ 否 → 检查其他维度
    │   │
    │   ├─ 资源监控？
    │   │   ├─ 是 → 资源监控
    │   │   │   ├─ 数据库文件大小监控
    │   │   │   ├─ 磁盘空间监控
    │   │   │   ├─ 内存使用监控
    │   │   │   └─ WAL文件监控
    │   │   │
    │   │   └─ 否 → 检查其他维度
    │   │
    │   └─ 健康检查？
    │       ├─ 是 → 健康检查
    │       │   ├─ 数据库完整性检查
    │       │   ├─ 索引有效性检查
    │       │   ├─ 统计信息更新
    │       │   └─ 碎片检查
    │       │
    │       └─ 否 → 故障诊断
    │
    ├─ 故障类型？
    │   ├─ 锁定问题？
    │   │   ├─ 是 → 锁定问题排查
    │   │   │   ├─ 检查锁状态
    │   │   │   ├─ 检查WAL模式
    │   │   │   └─ 检查连接数
    │   │   │
    │   │   └─ 否 → 检查其他故障
    │   │
    │   ├─ 性能下降？
    │   │   ├─ 是 → 性能下降排查
    │   │   │   ├─ 慢查询分析
    │   │   │   ├─ 索引使用分析
    │   │   │   └─ 统计信息更新
    │   │   │
    │   │   └─ 否 → 检查其他故障
    │   │
    │   ├─ 数据库损坏？
    │   │   ├─ 是 → 数据库损坏排查
    │   │   │   ├─ 完整性检查
    │   │   │   ├─ 备份恢复
    │   │   │   └─ 数据恢复
    │   │   │
    │   │   └─ 否 → 检查其他故障
    │   │
    │   └─ 内存泄漏？
    │       ├─ 是 → 内存泄漏排查
    │       │   ├─ 内存使用监控
    │       │   ├─ 连接泄漏检查
    │       │   └─ 缓存配置检查
    │       │
    │       └─ 否 → 正常监控
    │
    └─ 告警策略？
        ├─ 需要告警？ → 告警策略
        │   ├─ 性能告警
        │   │   ├─ 慢查询告警（> 1秒）
        │   │   ├─ 事务延迟告警（> 100ms）
        │   │   └─ 缓存命中率告警（< 80%）
        │   │
        │   ├─ 资源告警
        │   │   ├─ 磁盘空间告警（< 10%）
        │   │   ├─ 内存使用告警（> 80%）
        │   │   └─ WAL文件告警（> 100MB）
        │   │
        │   └─ 健康告警
        │       ├─ 完整性检查失败
        │       ├─ 索引失效
        │       └─ 碎片过多（> 20%）
        │
        └─ 不需要告警？ → 定期监控
            └─ 定期检查报告

推荐监控策略:
1. 性能监控: 实时监控（每5分钟）
2. 资源监控: 定期监控（每小时）
3. 健康检查: 定期检查（每天）
4. 故障诊断: 按需诊断
5. 告警策略: 关键指标实时告警
```

---

## 9. 最佳实践

### 9.1 监控最佳实践

1. **定期健康检查**
   - 每天执行完整性检查
   - 每周更新统计信息
   - 每月检查碎片情况

2. **性能监控**
   - 记录所有慢查询（> 1秒）
   - 监控事务吞吐量
   - 跟踪连接池使用情况

3. **资源监控**
   - 监控数据库文件大小增长
   - 检查磁盘空间使用
   - 跟踪WAL文件大小

4. **告警设置**
   - 设置合理的告警阈值
   - 及时响应严重告警
   - 定期审查告警规则

### 9.2 诊断最佳实践

1. **问题定位**
   - 使用EXPLAIN QUERY PLAN分析慢查询
   - 检查索引使用情况
   - 分析查询计划统计

2. **性能优化**
   - 定期运行ANALYZE更新统计
   - 优化索引设计
   - 调整PRAGMA配置

3. **故障恢复**
   - 保持数据库备份
   - 测试恢复流程
   - 文档化故障处理流程

---

## 10. 🔗 相关资源

### 内部资源

- [错误处理](./08.05-错误处理.md) - 错误码体系和异常恢复
- [PRAGMA配置](./08.04-PRAGMA配置.md) - PRAGMA命令详解
- [性能优化](../03-性能优化/) - 性能优化策略
- [多线程并发模式](./08.12-SQLite多线程并发模式最佳实践.md) - 并发模式最佳实践

### 外部资源

- [SQLite官方文档 - PRAGMA](https://www.sqlite.org/pragma.html)
- [SQLite官方文档 - EXPLAIN QUERY PLAN](https://www.sqlite.org/eqp.html)
- [SQLite官方文档 - 完整性检查](https://www.sqlite.org/pragma.html#pragma_integrity_check)

---

## 🔗 交叉引用

### 编程实践文档

- ⭐⭐⭐ [备份与恢复](./08.14-SQLite备份与恢复完整指南.md) - 备份恢复实践（含交叉引用）
- ⭐⭐ [错误处理](./08.05-错误处理.md) - 错误处理实践（含交叉引用）
- ⭐⭐ [PRAGMA配置](./08.04-PRAGMA配置.md) - 配置优化（含交叉引用）

### 核心架构文档 🆕

- ⭐⭐ [事务与并发控制](../01-核心架构/01.02-事务与并发控制.md) - 并发监控基础
- ⭐⭐ [存储引擎](../01-核心架构/01.03-存储引擎.md) - 存储监控基础

### 性能优化文档 🆕

- ⭐⭐ [性能特征分析](../03-性能优化/03.01-性能特征分析.md) - 性能监控分析（含决策树）
- ⭐⭐ [查询优化](./08.03-查询优化.md) - 查询性能诊断（含交叉引用）

### 知识图谱与导航 🆕

- ⭐⭐⭐ [知识图谱与概念关系网络](../09-最新特性/09.03-SQLite知识图谱与概念关系网络.md) - 监控诊断概念关系（400+概念）
- ⭐⭐ [文档依赖关系图](../00-项目导航/06-文档依赖关系图.md) - 监控诊断文档依赖
- ⭐⭐ [术语标准化词典](../00-项目导航/03-术语词典/SQLite术语标准化词典.md) - 监控诊断术语索引

### 相关概念链接 🆕

#### 监控诊断概念

- **生产环境监控** → [知识图谱：开发实践本体](../09-最新特性/09.03-SQLite知识图谱与概念关系网络.md#开发实践本体)
- **性能诊断** → [知识图谱：性能优化概念](../09-最新特性/09.03-SQLite知识图谱与概念关系网络.md#核心本体b-tree存储)
- **健康检查** → [知识图谱：监控概念](../09-最新特性/09.03-SQLite知识图谱与概念关系网络.md#开发实践本体)

---

**最后更新**: 2025-12-05
**维护者**: Data-Science Team

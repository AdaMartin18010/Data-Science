# SQLite数据库版本控制工具完整指南

> **创建日期**: 2025-12-05
> **版本**: SQLite 3.47.x
> **难度**: ⭐⭐⭐⭐⭐
> **适用场景**: 数据库版本管理、Schema迁移、多环境部署、版本控制工具集成

---

## 📑 目录

- [SQLite数据库版本控制工具完整指南](#sqlite数据库版本控制工具完整指南)
  - [📑 目录](#-目录)
  - [版本控制工具总览](#版本控制工具总览)
  - [1. Flyway集成](#1-flyway集成)
    - [1.1 Flyway基础](#11-flyway基础)
    - [1.2 Flyway配置](#12-flyway配置)
    - [1.3 Flyway迁移脚本](#13-flyway迁移脚本)
  - [2. Liquibase集成](#2-liquibase集成)
    - [2.1 Liquibase基础](#21-liquibase基础)
    - [2.2 Liquibase配置](#22-liquibase配置)
    - [2.3 Liquibase变更集](#23-liquibase变更集)
  - [3. Alembic集成](#3-alembic集成)
    - [3.1 Alembic基础](#31-alembic基础)
    - [3.2 Alembic配置](#32-alembic配置)
    - [3.3 Alembic迁移脚本](#33-alembic迁移脚本)
  - [4. 自定义版本控制](#4-自定义版本控制)
    - [4.1 简单版本控制系统](#41-简单版本控制系统)
    - [4.2 高级版本控制功能](#42-高级版本控制功能)
  - [5. 多环境部署策略](#5-多环境部署策略)
    - [5.1 环境配置管理](#51-环境配置管理)
    - [5.2 环境特定迁移](#52-环境特定迁移)
    - [5.3 部署流程](#53-部署流程)
  - [6. 工具对比与选型](#6-工具对比与选型)
    - [6.1 工具对比矩阵](#61-工具对比矩阵)
    - [6.2 选型建议](#62-选型建议)
  - [7. 最佳实践](#7-最佳实践)
    - [7.1 版本控制最佳实践](#71-版本控制最佳实践)
    - [7.2 多环境部署最佳实践](#72-多环境部署最佳实践)
  - [8. 🔗 相关资源](#8--相关资源)
    - [内部资源](#内部资源)
    - [外部资源](#外部资源)

---

## 版本控制工具总览

```text
数据库版本控制工具
══════════════════════════════════════════════════════════════════════════════

┌─────────────────────────────────────────────────────────────────────────┐
│                     主流工具对比                                         │
├─────────────────────────────────────────────────────────────────────────┤
│ 1. Flyway                                                               │
│    • 简单易用                                                            │
│    • SQL优先                                                             │
│    • 版本号命名                                                          │
│                                                                          │
│ 2. Liquibase                                                             │
│    • 多格式支持（XML/YAML/JSON/SQL）                                     │
│    • 变更集管理                                                          │
│    • 回滚支持                                                            │
│                                                                          │
│ 3. Alembic                                                               │
│    • Python生态                                                          │
│    • SQLAlchemy集成                                                      │
│    • 自动生成迁移                                                        │
│                                                                          │
│ 4. 自定义工具                                                             │
│    • 完全控制                                                            │
│    • 轻量级                                                              │
│    • 定制化需求                                                          │
└─────────────────────────────────────────────────────────────────────────┘
```

---

## 1. Flyway集成

### 1.1 Flyway基础

```python
# Flyway Python集成示例
# 需要安装: pip install flyway

from flyway import Flyway
import sqlite3

class FlywayManager:
    def __init__(self, db_path: str, migrations_dir: str = 'migrations'):
        self.db_path = db_path
        self.migrations_dir = migrations_dir
        self.flyway = Flyway(
            url=f'jdbc:sqlite:{db_path}',
            locations=[f'filesystem:{migrations_dir}']
        )

    def migrate(self):
        """执行迁移"""
        return self.flyway.migrate()

    def info(self):
        """查看迁移信息"""
        return self.flyway.info()

    def validate(self):
        """验证迁移"""
        return self.flyway.validate()

    def baseline(self, version: str = '1'):
        """基线化数据库"""
        return self.flyway.baseline(version=version)
```

### 1.2 Flyway配置

```properties
# flyway.conf
flyway.url=jdbc:sqlite:./database.db
flyway.user=
flyway.password=
flyway.locations=filesystem:migrations
flyway.sqlMigrationPrefix=V
flyway.sqlMigrationSeparator=__
flyway.sqlMigrationSuffixes=.sql
flyway.validateOnMigrate=true
flyway.cleanOnValidationError=false
```

### 1.3 Flyway迁移脚本

```sql
-- migrations/V1__Initial_schema.sql
CREATE TABLE users (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    name TEXT NOT NULL,
    email TEXT UNIQUE NOT NULL,
    created_at INTEGER NOT NULL
);

CREATE INDEX idx_users_email ON users(email);

-- migrations/V2__Add_user_profile.sql
ALTER TABLE users ADD COLUMN profile TEXT;

CREATE TABLE user_profiles (
    user_id INTEGER PRIMARY KEY,
    bio TEXT,
    avatar_url TEXT,
    FOREIGN KEY (user_id) REFERENCES users(id)
);

-- migrations/V3__Add_indexes.sql
CREATE INDEX idx_users_created ON users(created_at);
CREATE INDEX idx_profiles_user ON user_profiles(user_id);
```

---

## 2. Liquibase集成

### 2.1 Liquibase基础

```python
# Liquibase Python集成示例
# 需要安装: pip install liquibase

from liquibase import Liquibase
import sqlite3

class LiquibaseManager:
    def __init__(self, db_path: str, changelog_file: str = 'changelog.xml'):
        self.db_path = db_path
        self.changelog_file = changelog_file
        self.liquibase = Liquibase(
            url=f'jdbc:sqlite:{db_path}',
            changeLogFile=changelog_file
        )

    def update(self):
        """执行更新"""
        return self.liquibase.update()

    def status(self):
        """查看状态"""
        return self.liquibase.status()

    def rollback(self, count: int = 1):
        """回滚"""
        return self.liquibase.rollback(count=count)

    def rollback_to_tag(self, tag: str):
        """回滚到标签"""
        return self.liquibase.rollback(tag=tag)
```

### 2.2 Liquibase配置

```xml
<!-- changelog.xml -->
<?xml version="1.0" encoding="UTF-8"?>
<databaseChangeLog
    xmlns="http://www.liquibase.org/xml/ns/dbchangelog"
    xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
    xsi:schemaLocation="http://www.liquibase.org/xml/ns/dbchangelog
        http://www.liquibase.org/xml/ns/dbchangelog/dbchangelog-4.20.xsd">

    <changeSet id="1" author="developer">
        <createTable tableName="users">
            <column name="id" type="INTEGER" autoIncrement="true">
                <constraints primaryKey="true" nullable="false"/>
            </column>
            <column name="name" type="TEXT">
                <constraints nullable="false"/>
            </column>
            <column name="email" type="TEXT">
                <constraints nullable="false" unique="true"/>
            </column>
            <column name="created_at" type="INTEGER">
                <constraints nullable="false"/>
            </column>
        </createTable>
    </changeSet>

    <changeSet id="2" author="developer">
        <addColumn tableName="users">
            <column name="profile" type="TEXT"/>
        </addColumn>
    </changeSet>

    <changeSet id="3" author="developer">
        <createTable tableName="user_profiles">
            <column name="user_id" type="INTEGER">
                <constraints primaryKey="true" nullable="false"/>
            </column>
            <column name="bio" type="TEXT"/>
            <column name="avatar_url" type="TEXT"/>
        </createTable>
        <addForeignKeyConstraint
            baseTableName="user_profiles"
            baseColumnNames="user_id"
            constraintName="fk_user_profiles_user_id"
            referencedTableName="users"
            referencedColumnNames="id"/>
    </changeSet>

</databaseChangeLog>
```

### 2.3 Liquibase变更集

```yaml
# changelog.yaml
databaseChangeLog:
  - changeSet:
      id: 1
      author: developer
      changes:
        - createTable:
            tableName: users
            columns:
              - column:
                  name: id
                  type: INTEGER
                  autoIncrement: true
                  constraints:
                    primaryKey: true
                    nullable: false
              - column:
                  name: name
                  type: TEXT
                  constraints:
                    nullable: false
              - column:
                  name: email
                  type: TEXT
                  constraints:
                    nullable: false
                    unique: true
              - column:
                  name: created_at
                  type: INTEGER
                  constraints:
                    nullable: false

  - changeSet:
      id: 2
      author: developer
      changes:
        - addColumn:
            tableName: users
            columns:
              - column:
                  name: profile
                  type: TEXT
```

---

## 3. Alembic集成

### 3.1 Alembic基础

```python
# Alembic配置和使用
# 需要安装: pip install alembic sqlalchemy

from alembic import config, script
from alembic.runtime import migration
from sqlalchemy import create_engine
import sqlite3

class AlembicManager:
    def __init__(self, db_path: str, alembic_cfg_path: str = 'alembic.ini'):
        self.db_path = db_path
        self.alembic_cfg = config.Config(alembic_cfg_path)
        self.alembic_cfg.set_main_option('sqlalchemy.url', f'sqlite:///{db_path}')

    def upgrade(self, revision: str = 'head'):
        """升级到指定版本"""
        from alembic import command
        command.upgrade(self.alembic_cfg, revision)

    def downgrade(self, revision: str = '-1'):
        """降级到指定版本"""
        from alembic import command
        command.downgrade(self.alembic_cfg, revision)

    def current(self):
        """查看当前版本"""
        from alembic import command
        return command.current(self.alembic_cfg)

    def history(self):
        """查看历史"""
        from alembic import command
        return command.history(self.alembic_cfg)

    def revision(self, message: str, autogenerate: bool = True):
        """创建新迁移"""
        from alembic import command
        command.revision(
            self.alembic_cfg,
            message=message,
            autogenerate=autogenerate
        )
```

### 3.2 Alembic配置

```ini
# alembic.ini
[alembic]
script_location = alembic
sqlalchemy.url = sqlite:///./database.db

[loggers]
keys = root,sqlalchemy,alembic

[handlers]
keys = console

[formatters]
keys = generic

[logger_root]
level = WARN
handlers = console
qualname =

[logger_sqlalchemy]
level = WARN
handlers =
qualname = sqlalchemy.engine

[logger_alembic]
level = INFO
handlers =
qualname = alembic

[handler_console]
class = StreamHandler
args = (sys.stderr,)
level = NOTSET
formatter = generic

[formatter_generic]
format = %(levelname)-5.5s [%(name)s] %(message)s
datefmt = %H:%M:%S
```

### 3.3 Alembic迁移脚本

```python
# alembic/versions/001_initial_schema.py
"""Initial schema

Revision ID: 001
Revises:
Create Date: 2025-12-05 10:00:00.000000

"""
from alembic import op
import sqlalchemy as sa

# revision identifiers, used by Alembic.
revision = '001'
down_revision = None
branch_labels = None
depends_on = None

def upgrade():
    op.create_table('users',
        sa.Column('id', sa.Integer(), autoincrement=True, nullable=False),
        sa.Column('name', sa.Text(), nullable=False),
        sa.Column('email', sa.Text(), nullable=False),
        sa.Column('created_at', sa.Integer(), nullable=False),
        sa.PrimaryKeyConstraint('id'),
        sa.UniqueConstraint('email')
    )
    op.create_index('idx_users_email', 'users', ['email'])

def downgrade():
    op.drop_index('idx_users_email', table_name='users')
    op.drop_table('users')

# alembic/versions/002_add_user_profile.py
"""Add user profile

Revision ID: 002
Revises: 001
Create Date: 2025-12-05 11:00:00.000000

"""
from alembic import op
import sqlalchemy as sa

revision = '002'
down_revision = '001'
branch_labels = None
depends_on = None

def upgrade():
    op.add_column('users', sa.Column('profile', sa.Text(), nullable=True))
    op.create_table('user_profiles',
        sa.Column('user_id', sa.Integer(), nullable=False),
        sa.Column('bio', sa.Text(), nullable=True),
        sa.Column('avatar_url', sa.Text(), nullable=True),
        sa.ForeignKeyConstraint(['user_id'], ['users.id']),
        sa.PrimaryKeyConstraint('user_id')
    )

def downgrade():
    op.drop_table('user_profiles')
    op.drop_column('users', 'profile')
```

---

## 4. 自定义版本控制

### 4.1 简单版本控制系统

```python
import sqlite3
import hashlib
from pathlib import Path
from typing import List, Dict
from datetime import datetime

class SimpleVersionControl:
    def __init__(self, db_path: str, migrations_dir: str = 'migrations'):
        self.db_path = db_path
        self.migrations_dir = Path(migrations_dir)
        self.conn = sqlite3.connect(db_path)
        self.setup_version_table()

    def setup_version_table(self):
        """设置版本表"""
        cursor = self.conn.cursor()
        cursor.execute("""
            CREATE TABLE IF NOT EXISTS schema_version (
                version INTEGER PRIMARY KEY,
                name TEXT NOT NULL,
                checksum TEXT NOT NULL,
                applied_at INTEGER NOT NULL
            )
        """)
        self.conn.commit()

    def get_current_version(self) -> int:
        """获取当前版本"""
        cursor = self.conn.cursor()
        cursor.execute("SELECT MAX(version) FROM schema_version")
        result = cursor.fetchone()[0]
        return result if result else 0

    def load_migrations(self) -> List[Dict]:
        """加载迁移脚本"""
        migrations = []

        for migration_file in sorted(self.migrations_dir.glob("*.sql")):
            # 文件名格式: 001_initial_schema.sql
            parts = migration_file.stem.split('_', 1)
            version = int(parts[0])
            name = parts[1] if len(parts) > 1 else migration_file.stem

            with open(migration_file, 'r', encoding='utf-8') as f:
                sql = f.read()

            checksum = hashlib.md5(sql.encode()).hexdigest()

            migrations.append({
                'version': version,
                'name': name,
                'file': migration_file,
                'sql': sql,
                'checksum': checksum
            })

        return sorted(migrations, key=lambda x: x['version'])

    def apply_migration(self, migration: Dict):
        """应用迁移"""
        cursor = self.conn.cursor()

        # 检查是否已应用
        cursor.execute("""
            SELECT version FROM schema_version WHERE version = ?
        """, (migration['version'],))
        if cursor.fetchone():
            print(f"迁移 {migration['version']} 已应用，跳过")
            return

        # 执行迁移
        try:
            cursor.executescript(migration['sql'])
            cursor.execute("""
                INSERT INTO schema_version (version, name, checksum, applied_at)
                VALUES (?, ?, ?, strftime('%s', 'now'))
            """, (migration['version'], migration['name'], migration['checksum']))
            self.conn.commit()
            print(f"迁移 {migration['version']} 应用成功")
        except Exception as e:
            self.conn.rollback()
            print(f"迁移 {migration['version']} 应用失败: {e}")
            raise

    def migrate(self):
        """执行所有未应用的迁移"""
        current_version = self.get_current_version()
        migrations = self.load_migrations()

        for migration in migrations:
            if migration['version'] > current_version:
                self.apply_migration(migration)

    def rollback(self, target_version: int):
        """回滚到指定版本"""
        # 注意：需要维护回滚脚本
        current_version = self.get_current_version()

        if target_version >= current_version:
            raise ValueError(f"目标版本 {target_version} 必须小于当前版本 {current_version}")

        cursor = self.conn.cursor()
        cursor.execute("""
            SELECT version FROM schema_version
            WHERE version > ?
            ORDER BY version DESC
        """, (target_version,))

        versions_to_rollback = [row[0] for row in cursor.fetchall()]

        # 执行回滚（需要回滚脚本）
        for version in versions_to_rollback:
            rollback_file = self.migrations_dir / f"{version:03d}_rollback.sql"
            if rollback_file.exists():
                with open(rollback_file, 'r') as f:
                    rollback_sql = f.read()
                cursor.executescript(rollback_sql)
                cursor.execute("DELETE FROM schema_version WHERE version = ?", (version,))

        self.conn.commit()
```

### 4.2 高级版本控制功能

```python
class AdvancedVersionControl(SimpleVersionControl):
    def __init__(self, db_path: str, migrations_dir: str = 'migrations'):
        super().__init__(db_path, migrations_dir)
        self.setup_advanced_tables()

    def setup_advanced_tables(self):
        """设置高级功能表"""
        cursor = self.conn.cursor()

        # 迁移历史详情
        cursor.execute("""
            CREATE TABLE IF NOT EXISTS migration_history (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                version INTEGER NOT NULL,
                name TEXT NOT NULL,
                checksum TEXT NOT NULL,
                applied_at INTEGER NOT NULL,
                execution_time REAL,
                success INTEGER,
                error_message TEXT,
                FOREIGN KEY (version) REFERENCES schema_version(version)
            )
        """)

        # 环境信息
        cursor.execute("""
            CREATE TABLE IF NOT EXISTS environment_info (
                environment TEXT PRIMARY KEY,
                current_version INTEGER,
                last_migration_at INTEGER,
                updated_at INTEGER NOT NULL
            )
        """)

        self.conn.commit()

    def record_migration_history(self, migration: Dict, execution_time: float,
                                 success: bool, error_message: str = None):
        """记录迁移历史"""
        cursor = self.conn.cursor()
        cursor.execute("""
            INSERT INTO migration_history
            (version, name, checksum, applied_at, execution_time, success, error_message)
            VALUES (?, ?, ?, strftime('%s', 'now'), ?, ?, ?)
        """, (
            migration['version'],
            migration['name'],
            migration['checksum'],
            execution_time,
            1 if success else 0,
            error_message
        ))
        self.conn.commit()

    def get_migration_history(self, limit: int = 100) -> List[Dict]:
        """获取迁移历史"""
        cursor = self.conn.cursor()
        cursor.execute("""
            SELECT * FROM migration_history
            ORDER BY applied_at DESC
            LIMIT ?
        """, (limit,))

        return [dict(row) for row in cursor.fetchall()]

    def validate_migrations(self) -> Dict:
        """验证迁移"""
        cursor = self.conn.cursor()

        # 检查已应用的迁移
        cursor.execute("SELECT version, checksum FROM schema_version")
        applied = {row[0]: row[1] for row in cursor.fetchall()}

        # 检查文件系统中的迁移
        migrations = self.load_migrations()
        file_checksums = {m['version']: m['checksum'] for m in migrations}

        # 比较
        issues = []
        for version, checksum in applied.items():
            if version in file_checksums:
                if file_checksums[version] != checksum:
                    issues.append({
                        'version': version,
                        'issue': 'checksum_mismatch',
                        'message': f'迁移 {version} 的校验和不匹配'
                    })
            else:
                issues.append({
                    'version': version,
                    'issue': 'file_missing',
                    'message': f'迁移 {version} 的文件不存在'
                })

        return {
            'valid': len(issues) == 0,
            'issues': issues
        }
```

---

## 5. 多环境部署策略

### 5.1 环境配置管理

```python
import os
from typing import Dict

class EnvironmentConfig:
    def __init__(self):
        self.environments = {
            'development': {
                'db_path': './dev.db',
                'migrations_dir': './migrations',
                'auto_migrate': True,
                'backup_before_migrate': False
            },
            'testing': {
                'db_path': './test.db',
                'migrations_dir': './migrations',
                'auto_migrate': True,
                'backup_before_migrate': False
            },
            'staging': {
                'db_path': './staging.db',
                'migrations_dir': './migrations',
                'auto_migrate': False,
                'backup_before_migrate': True
            },
            'production': {
                'db_path': './production.db',
                'migrations_dir': './migrations',
                'auto_migrate': False,
                'backup_before_migrate': True
            }
        }

    def get_config(self, environment: str) -> Dict:
        """获取环境配置"""
        return self.environments.get(environment, {})

    def get_current_environment(self) -> str:
        """获取当前环境"""
        return os.getenv('ENVIRONMENT', 'development')
```

### 5.2 环境特定迁移

```python
class EnvironmentAwareMigration:
    def __init__(self, version_control: SimpleVersionControl,
                 environment: str):
        self.version_control = version_control
        self.environment = environment

    def load_migrations(self) -> List[Dict]:
        """加载迁移（考虑环境）"""
        all_migrations = self.version_control.load_migrations()

        # 过滤环境特定迁移
        filtered = []
        for migration in all_migrations:
            # 检查是否有环境限制
            if self.is_migration_for_environment(migration):
                filtered.append(migration)

        return filtered

    def is_migration_for_environment(self, migration: Dict) -> bool:
        """检查迁移是否适用于当前环境"""
        # 检查文件名中的环境标记
        # 例如: 001_initial_schema_prod.sql 只在生产环境运行
        name = migration['name'].lower()

        if '_prod' in name or '_production' in name:
            return self.environment == 'production'
        elif '_dev' in name or '_development' in name:
            return self.environment in ['development', 'testing']
        elif '_test' in name or '_testing' in name:
            return self.environment == 'testing'
        else:
            return True  # 默认适用于所有环境
```

### 5.3 部署流程

```python
class DeploymentManager:
    def __init__(self, version_control: SimpleVersionControl,
                 environment: str):
        self.version_control = version_control
        self.environment = environment
        self.config = EnvironmentConfig()

    def deploy(self, backup: bool = True) -> Dict:
        """部署迁移"""
        env_config = self.config.get_config(self.environment)

        # 1. 备份（如果需要）
        if backup and env_config.get('backup_before_migrate', False):
            self.backup_database()

        # 2. 验证
        validation = self.version_control.validate_migrations()
        if not validation['valid']:
            return {
                'success': False,
                'reason': '迁移验证失败',
                'issues': validation['issues']
            }

        # 3. 执行迁移
        try:
            self.version_control.migrate()
            return {
                'success': True,
                'message': '迁移成功'
            }
        except Exception as e:
            return {
                'success': False,
                'reason': str(e)
            }

    def backup_database(self):
        """备份数据库"""
        from datetime import datetime
        import shutil

        backup_path = f"{self.version_control.db_path}.{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        shutil.copy2(self.version_control.db_path, backup_path)
        print(f"数据库已备份到: {backup_path}")
```

---

## 6. 工具对比与选型

### 6.1 工具对比矩阵

| 特性 | Flyway | Liquibase | Alembic | 自定义 |
|------|--------|-----------|---------|--------|
| **易用性** | ⭐⭐⭐⭐⭐ | ⭐⭐⭐ | ⭐⭐⭐⭐ | ⭐⭐ |
| **SQL支持** | ⭐⭐⭐⭐⭐ | ⭐⭐⭐⭐ | ⭐⭐⭐ | ⭐⭐⭐⭐⭐ |
| **回滚支持** | ⭐⭐ | ⭐⭐⭐⭐⭐ | ⭐⭐⭐⭐ | ⭐⭐⭐ |
| **多格式** | ⭐⭐ | ⭐⭐⭐⭐⭐ | ⭐⭐ | ⭐⭐⭐⭐⭐ |
| **Python集成** | ⭐⭐ | ⭐⭐ | ⭐⭐⭐⭐⭐ | ⭐⭐⭐⭐⭐ |
| **学习曲线** | 低 | 中 | 中 | 高 |
| **社区支持** | ⭐⭐⭐⭐ | ⭐⭐⭐⭐ | ⭐⭐⭐⭐ | - |

### 6.2 选型建议

**选择Flyway如果**：

- 需要简单快速的解决方案
- 主要使用SQL迁移
- 团队熟悉Java生态

**选择Liquibase如果**：

- 需要强大的回滚功能
- 需要多格式支持
- 需要复杂的变更管理

**选择Alembic如果**：

- 使用Python/SQLAlchemy
- 需要自动生成迁移
- Python团队

**选择自定义如果**：

- 有特殊需求
- 需要完全控制
- 轻量级解决方案

---

## 7. 最佳实践

### 7.1 版本控制最佳实践

1. **命名规范**
   - 使用统一的命名格式
   - 包含版本号和描述
   - 避免特殊字符

2. **迁移脚本**
   - 保持脚本幂等性
   - 避免破坏性变更
   - 提供回滚脚本

3. **版本管理**
   - 版本号递增
   - 记录变更原因
   - 维护变更日志

### 7.2 多环境部署最佳实践

1. **环境隔离**
   - 每个环境独立数据库
   - 环境特定配置
   - 环境特定迁移

2. **部署流程**
   - 开发 -> 测试 -> 预发布 -> 生产
   - 每个环境验证
   - 生产环境人工审核

3. **回滚准备**
   - 维护回滚脚本
   - 测试回滚流程
   - 快速回滚机制

---

## 8. 🔗 相关资源

### 内部资源

- [开发工作流与CI/CD集成](./08.17-SQLite开发工作流与CI-CD集成.md) - CI/CD基础
- [智能化CI/CD实践](./08.18-智能化CI-CD实践指南.md) - 智能化实践
- [DataOps实践](./08.19-DataOps驱动的数据库CI-CD实践.md) - DataOps实践

### 外部资源

- [Flyway文档](https://flywaydb.org/documentation/) - Flyway官方文档
- [Liquibase文档](https://docs.liquibase.com/) - Liquibase官方文档
- [Alembic文档](https://alembic.sqlalchemy.org/) - Alembic官方文档

---

**最后更新**: 2025-12-05
**维护者**: Data-Science Team

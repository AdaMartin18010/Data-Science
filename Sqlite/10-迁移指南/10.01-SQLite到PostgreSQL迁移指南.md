...
- [SQLite 到 PostgreSQL 迁移指南](#sqlite-到-postgresql-迁移指南)
  ...
  - [📊 思维导图](#-思维导图)
  - [4. 迁移决策框架](#4-迁移决策框架)
    - [4.1. 何时需要迁移](#41-何时需要迁移)
      - [4.1.1. 触发迁移的业务场景](#411-触发迁移的业务场景)
      - [4.1.2. 技术指标阈值](#412-技术指标阈值)
  - [5. 迁移成本评估](#5-迁移成本评估)
    - [5.1. 成本维度分析](#51-成本维度分析)
    - [5.2. 迁移风险评估](#52-迁移风险评估)
      - [5.2.1. 风险矩阵](#521-风险矩阵)
  - [6. 数据模型转换](#6-数据模型转换)
    - [6.1. 数据类型映射](#61-数据类型映射)
      - [6.1.1. 核心类型映射表](#611-核心类型映射表)
      - [6.1.2. 动态类型处理](#612-动态类型处理)
      - [6.1.3. 类型转换策略](#613-类型转换策略)
    - [6.2. 表结构转换](#62-表结构转换)
      - [6.2.1. 主键转换](#621-主键转换)
      - [6.2.2. 外键转换](#622-外键转换)
      - [6.2.3. 默认值转换](#623-默认值转换)
    - [6.3. 约束转换](#63-约束转换)
      - [6.3.1. CHECK约束](#631-check约束)
      - [6.3.2. UNIQUE约束](#632-unique约束)
    - [6.4. 索引转换](#64-索引转换)
      - [6.4.1. 基本索引](#641-基本索引)
      - [6.4.2. 复合索引](#642-复合索引)
      - [6.4.3. 特殊索引](#643-特殊索引)
  - [7. SQL语法差异处理](#7-sql语法差异处理)
    - [7.1. DDL差异](#71-ddl差异)
      - [7.1.1. CREATE TABLE差异](#711-create-table差异)
      - [7.1.2. ALTER TABLE差异](#712-alter-table差异)
    - [7.2. DML差异](#72-dml差异)
      - [7.2.1. INSERT差异](#721-insert差异)
      - [7.2.2. UPDATE差异](#722-update差异)
    - [7.3. 函数差异](#73-函数差异)
      - [7.3.1. 字符串函数](#731-字符串函数)
      - [7.3.2. 日期时间函数](#732-日期时间函数)
      - [7.3.3. 聚合函数](#733-聚合函数)
    - [7.4. 特性差异](#74-特性差异)
      - [7.4.1. 窗口函数](#741-窗口函数)
      - [7.4.2. CTE（公共表表达式）](#742-cte公共表表达式)
  - [8. 架构特性转换](#8-架构特性转换)
    - [8.1. 并发控制转换](#81-并发控制转换)
      - [8.1.1. SQLite WAL模式 → PostgreSQL MVCC](#811-sqlite-wal模式--postgresql-mvcc)
      - [8.1.2. 锁机制转换](#812-锁机制转换)
    - [8.2. 事务处理转换](#82-事务处理转换)
      - [8.2.1. 事务隔离级别](#821-事务隔离级别)
      - [8.2.2. SAVEPOINT处理](#822-savepoint处理)
    - [8.3. 存储引擎差异](#83-存储引擎差异)
      - [8.3.1. 文件结构](#831-文件结构)
      - [8.3.2. 存储优化](#832-存储优化)
    - [8.4. 连接管理转换](#84-连接管理转换)
      - [8.4.1. 连接方式](#841-连接方式)
  - [9. 连接池](#9-连接池)
  - [10. 迁移工具和方案](#10-迁移工具和方案)
    - [10.1. 官方工具](#101-官方工具)
      - [10.1.1. pgloader](#1011-pgloader)
  - [11. 第三方工具](#11-第三方工具)
    - [11.1. SQLite to PostgreSQL Converter](#111-sqlite-to-postgresql-converter)
      - [11.1.1. 自定义Python脚本](#1111-自定义python脚本)
  - [12. 自定义迁移脚本](#12-自定义迁移脚本)
    - [12.1. 完整迁移脚本框架](#121-完整迁移脚本框架)
  - [13. 成熟案例参考](#13-成熟案例参考)
    - [13.1. 案例1：Django项目迁移](#131-案例1django项目迁移)
  - [14. 案例2：Rails项目迁移](#14-案例2rails项目迁移)
  - [15. 案例3：Node.js项目迁移](#15-案例3nodejs项目迁移)
  - [16. 迁移执行流程](#16-迁移执行流程)
    - [16.1. 迁移前准备](#161-迁移前准备)
      - [16.1.1. 环境准备清单](#1611-环境准备清单)
      - [16.1.2. 数据备份](#1612-数据备份)
  - [17. 环境测试](#17-环境测试)
  - [18. 数据迁移步骤](#18-数据迁移步骤)
    - [18.1. 步骤1：迁移表结构](#181-步骤1迁移表结构)
  - [19. 步骤2：迁移数据](#19-步骤2迁移数据)
  - [20. 步骤3：迁移索引](#20-步骤3迁移索引)
  - [21. 步骤4：迁移约束](#21-步骤4迁移约束)
  - [22. 应用代码迁移](#22-应用代码迁移)
    - [22.1. ORM迁移](#221-orm迁移)
  - [23. 原生SQL迁移](#23-原生sql迁移)
  - [24. 迁移后验证](#24-迁移后验证)
    - [24.1. 数据完整性验证](#241-数据完整性验证)
      - [24.1.1. 性能验证](#2411-性能验证)
  - [25. 性能优化调整](#25-性能优化调整)
    - [25.1. 查询优化差异](#251-查询优化差异)
      - [25.1.1. 查询计划器](#2511-查询计划器)
      - [25.1.2. 优化建议](#2512-优化建议)
    - [25.2. 索引策略调整](#252-索引策略调整)
      - [25.2.1. 索引类型选择](#2521-索引类型选择)
    - [25.3. 连接池配置](#253-连接池配置)
      - [25.3.1. PostgreSQL连接池配置](#2531-postgresql连接池配置)
  - [26. PgBouncer配置](#26-pgbouncer配置)
    - [26.1. 缓存策略调整](#261-缓存策略调整)
      - [26.1.1. PostgreSQL缓存配置](#2611-postgresql缓存配置)
  - [27. 常见问题和解决方案](#27-常见问题和解决方案)
    - [27.1. 数据类型问题](#271-数据类型问题)
      - [27.1.1. 问题1：INTEGER溢出](#2711-问题1integer溢出)
      - [27.1.2. 问题2：日期时间格式](#2712-问题2日期时间格式)
    - [27.2. 并发问题](#272-并发问题)
      - [27.2.1. 问题1：锁等待](#2721-问题1锁等待)
    - [27.3. 性能问题](#273-性能问题)
      - [27.3.1. 问题1：查询变慢](#2731-问题1查询变慢)
    - [27.4. 兼容性问题](#274-兼容性问题)
      - [27.4.1. 问题1：函数不兼容](#2741-问题1函数不兼容)
  - [28. 成熟案例研究](#28-成熟案例研究)
    - [28.1. 案例一：Web应用迁移](#281-案例一web应用迁移)
      - [28.1.1. 背景](#2811-背景)
      - [28.1.2. 迁移方案](#2812-迁移方案)
      - [28.1.3. 迁移结果](#2813-迁移结果)
    - [28.2. 案例二：数据分析平台迁移](#282-案例二数据分析平台迁移)
      - [28.2.1. 背景](#2821-背景)
      - [28.2.2. 迁移方案](#2822-迁移方案)
      - [28.2.3. 迁移结果](#2823-迁移结果)
    - [28.3. 案例三：移动应用后端迁移](#283-案例三移动应用后端迁移)
      - [28.3.1. 背景](#2831-背景)
      - [28.3.2. 迁移方案](#2832-迁移方案)
      - [28.3.3. 迁移结果](#2833-迁移结果)
  - [29. 📊 SQLite vs PostgreSQL 多维对比矩阵](#29--sqlite-vs-postgresql-多维对比矩阵)
    - [29.1. 迁移决策多维对比矩阵](#291-迁移决策多维对比矩阵)
    - [29.2. 迁移工具对比矩阵](#292-迁移工具对比矩阵)
    - [29.3. 数据类型映射对比矩阵](#293-数据类型映射对比矩阵)
    - [29.4. 迁移策略对比矩阵](#294-迁移策略对比矩阵)
  - [30. 📊 迁移检查清单](#30--迁移检查清单)
    - [30.1. 迁移前](#301-迁移前)
    - [30.2. 迁移中](#302-迁移中)
    - [30.3. 迁移后](#303-迁移后)
...
#### 8.1.1. SQLite WAL模式 → PostgreSQL MVCC
...
### 11.1. SQLite to PostgreSQL Converter
...
## 29. 📊 SQLite vs PostgreSQL 多维对比矩阵

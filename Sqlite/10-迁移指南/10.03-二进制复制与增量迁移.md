...
- [SQLite 到 PostgreSQL 二进制复制与增量迁移](#sqlite-到-postgresql-二进制复制与增量迁移)
  ...
  - [📑 目录](#-目录)
  - [3. 📊 思维导图](#3--思维导图)
  - [4. 二进制数据复制机制](#4-二进制数据复制机制)
    - [4.1. SQLite二进制存储格式](#41-sqlite二进制存储格式)
      - [4.1.1. SQLite数据库文件结构](#411-sqlite数据库文件结构)
      - [4.1.2. 页结构分析](#412-页结构分析)
      - [4.1.3. 二进制数据存储](#413-二进制数据存储)
  - [5. PostgreSQL二进制存储格式](#5-postgresql二进制存储格式)
    - [5.1. PostgreSQL存储格式](#51-postgresql存储格式)
      - [5.1.1. BYTEA存储格式](#511-bytea存储格式)
  - [6. 二进制数据转换策略](#6-二进制数据转换策略)
    - [6.1. 转换策略矩阵](#61-转换策略矩阵)
      - [6.1.1. 二进制转换实现](#611-二进制转换实现)
  - [7. BLOB数据迁移](#7-blob数据迁移)
    - [7.1. 大BLOB迁移策略](#71-大blob迁移策略)
  - [8. 全量迁移策略](#8-全量迁移策略)
    - [8.1. 全量迁移架构](#81-全量迁移架构)
      - [8.1.1. 迁移架构设计](#811-迁移架构设计)
      - [8.1.2. 迁移流程图](#812-迁移流程图)
    - [8.2. 表级全量迁移](#82-表级全量迁移)
      - [8.2.1. 单表迁移策略](#821-单表迁移策略)
  - [9. 多表迁移策略](#9-多表迁移策略)
  - [10. 数据完整性验证](#10-数据完整性验证)
    - [10.1. 行数验证](#101-行数验证)
      - [10.1.1. 数据校验和验证](#1011-数据校验和验证)
  - [11. 逐行验证](#11-逐行验证)
  - [12. 性能优化策略](#12-性能优化策略)
    - [12.1. 批量操作优化](#121-批量操作优化)
  - [13. 并行迁移](#13-并行迁移)
  - [14. 增量迁移策略](#14-增量迁移策略)
    - [14.1. 增量迁移架构](#141-增量迁移架构)
      - [14.1.1. 增量迁移架构设计](#1411-增量迁移架构设计)
    - [14.2. 变更捕获机制](#142-变更捕获机制)
      - [14.2.1. 策略1：触发器方式](#1421-策略1触发器方式)
  - [15. 策略2：WAL日志解析](#15-策略2wal日志解析)
  - [16. 策略3：应用层变更捕获](#16-策略3应用层变更捕获)
  - [17. 增量同步策略](#17-增量同步策略)
    - [17.1. 实时同步](#171-实时同步)
      - [17.1.1. 批量同步](#1711-批量同步)
  - [18. 冲突解决机制](#18-冲突解决机制)
    - [18.1. 冲突检测](#181-冲突检测)
  - [19. 冲突解决策略](#19-冲突解决策略)
  - [20. 数据一致性保证](#20-数据一致性保证)
    - [20.1. 事务一致性](#201-事务一致性)
      - [20.1.1. 事务边界处理](#2011-事务边界处理)
  - [21. 数据校验机制](#21-数据校验机制)
    - [21.1. 校验和验证](#211-校验和验证)
  - [22. 回滚策略](#22-回滚策略)
  - [23. 迁移工具实现](#23-迁移工具实现)
    - [23.1. 二进制复制工具](#231-二进制复制工具)
  - [24. 增量同步工具](#24-增量同步工具)
  - [25. 监控与报告](#25-监控与报告)
  - [26. 实战案例](#26-实战案例)
    - [26.1. 大型数据库全量迁移](#261-大型数据库全量迁移)
    - [26.2. 在线增量迁移](#262-在线增量迁移)
    - [26.3. 零停机迁移](#263-零停机迁移)
  - [27. 🔗 相关资源](#27--相关资源)
  - [28. 🔗 交叉引用](#28--交叉引用)
    - [28.1. 理论模型 🆕](#281-理论模型-)
    - [28.2. 设计模型 🆕](#282-设计模型-)

# SQLite 到 PostgreSQL 数据迁移与语义模型兼容性

> **创建日期**：2025-11-13
> **最后更新**：2025-01-15
> **适用版本**：SQLite 3.31+ 至 3.47.x → PostgreSQL 12+

---

## 📋 概述

本文档深入分析SQLite到PostgreSQL数据迁移过程中的语义模型兼容性，包括数据语义、业务语义、领域模型转换，以及迁移后的数据分析和处理策略。

---

## 📑 目录

- [SQLite 到 PostgreSQL 数据迁移与语义模型兼容性](#sqlite-到-postgresql-数据迁移与语义模型兼容性)
  - [📋 概述](#-概述)
  - [📑 目录](#-目录)
  - [📊 思维导图](#-思维导图)
  - [一、语义模型理论基础](#一语义模型理论基础)
    - [1.1 数据语义模型](#11-数据语义模型)
      - [SQLite数据语义模型](#sqlite数据语义模型)
      - [PostgreSQL数据语义模型](#postgresql数据语义模型)
    - [1.2 业务语义模型](#12-业务语义模型)
      - [业务语义定义](#业务语义定义)
    - [1.3 领域模型](#13-领域模型)
      - [领域模型定义](#领域模型定义)
    - [1.4 语义转换理论](#14-语义转换理论)
      - [语义等价性](#语义等价性)
      - [语义转换规则](#语义转换规则)
  - [二、数据语义兼容性分析](#二数据语义兼容性分析)
    - [2.1 值语义映射](#21-值语义映射)
      - [整数语义映射](#整数语义映射)
      - [浮点数语义映射](#浮点数语义映射)
      - [文本语义映射](#文本语义映射)
    - [2.2 类型语义转换](#22-类型语义转换)
      - [动态类型到静态类型转换](#动态类型到静态类型转换)
      - [类型语义损失分析](#类型语义损失分析)
    - [2.3 约束语义转换](#23-约束语义转换)
      - [CHECK约束语义转换](#check约束语义转换)
      - [外键约束语义转换](#外键约束语义转换)
    - [2.4 关系语义转换](#24-关系语义转换)
      - [一对一关系语义](#一对一关系语义)
      - [一对多关系语义](#一对多关系语义)
      - [多对多关系语义](#多对多关系语义)
  - [三、业务语义兼容性分析](#三业务语义兼容性分析)
    - [3.1 业务规则语义](#31-业务规则语义)
      - [业务规则定义](#业务规则定义)
      - [业务规则语义转换](#业务规则语义转换)
    - [3.2 业务流程语义](#32-业务流程语义)
      - [状态流转语义](#状态流转语义)
    - [3.3 业务逻辑语义](#33-业务逻辑语义)
      - [计算逻辑语义](#计算逻辑语义)
    - [3.4 业务约束语义](#34-业务约束语义)
      - [复杂业务约束](#复杂业务约束)
  - [四、领域模型转换](#四领域模型转换)
    - [4.1 实体模型转换](#41-实体模型转换)
      - [实体识别](#实体识别)
    - [4.2 关系模型转换](#42-关系模型转换)
      - [聚合关系转换](#聚合关系转换)
    - [4.3 聚合模型转换](#43-聚合模型转换)
      - [聚合根识别](#聚合根识别)
    - [4.4 值对象转换](#44-值对象转换)
      - [值对象识别](#值对象识别)
  - [五、数据迁移语义保证](#五数据迁移语义保证)
    - [5.1 语义等价性验证](#51-语义等价性验证)
      - [验证框架](#验证框架)
    - [5.2 语义完整性检查](#52-语义完整性检查)
      - [完整性检查清单](#完整性检查清单)
    - [5.3 语义一致性保证](#53-语义一致性保证)
      - [一致性保证机制](#一致性保证机制)
  - [六、迁移后数据分析](#六迁移后数据分析)
    - [6.1 数据质量分析](#61-数据质量分析)
      - [数据质量指标](#数据质量指标)
    - [6.2 语义一致性分析](#62-语义一致性分析)
      - [语义一致性检查](#语义一致性检查)
    - [6.3 性能语义分析](#63-性能语义分析)
      - [性能语义验证](#性能语义验证)
    - [6.4 业务语义验证](#64-业务语义验证)
      - [业务语义验证](#业务语义验证)
  - [七、数据处理策略](#七数据处理策略)
    - [7.1 数据清洗](#71-数据清洗)
      - [数据清洗流程](#数据清洗流程)
    - [7.2 数据转换](#72-数据转换)
      - [数据格式转换](#数据格式转换)
    - [7.3 数据验证](#73-数据验证)
      - [数据验证流程](#数据验证流程)
    - [7.4 数据修复](#74-数据修复)
      - [数据修复策略](#数据修复策略)
  - [八、语义模型兼容性多维对比矩阵](#八语义模型兼容性多维对比矩阵)
    - [8.1 数据语义模型对比矩阵](#81-数据语义模型对比矩阵)
    - [8.2 业务语义转换策略对比矩阵](#82-业务语义转换策略对比矩阵)
    - [8.3 迁移策略对比矩阵](#83-迁移策略对比矩阵)
  - [九、实战案例](#九实战案例)
    - [9.1 电商系统迁移](#91-电商系统迁移)
    - [9.2 金融系统迁移](#92-金融系统迁移)
    - [9.3 日志系统迁移](#93-日志系统迁移)
  - [🔗 相关资源](#-相关资源)
  - [📚 参考资料](#-参考资料)

---

## 📊 思维导图

```mermaid
mindmap
  root((数据迁移与语义模型兼容性))
    语义模型理论基础
      数据语义模型
        SQLite数据语义模型
        PostgreSQL数据语义模型
      业务语义模型
        业务语义定义
        业务规则
      领域模型
        领域模型定义
        实体关系
      语义转换理论
        语义等价性
        语义转换规则
    数据语义兼容性分析
      值语义映射
        整数语义映射
        浮点数语义映射
        文本语义映射
      类型语义转换
        动态类型到静态类型转换
        类型语义损失分析
      约束语义转换
        CHECK约束语义转换
        外键约束语义转换
      关系语义转换
        一对一关系语义
        一对多关系语义
        多对多关系语义
    业务语义兼容性分析
      业务规则语义
        业务规则定义
        业务规则语义转换
      业务流程语义
        状态流转语义
      业务逻辑语义
        计算逻辑语义
      业务约束语义
        复杂业务约束
    领域模型转换
      实体模型转换
        实体识别
      关系模型转换
        聚合关系转换
      聚合模型转换
        聚合根识别
      值对象转换
        值对象识别
    数据迁移语义保证
      语义等价性验证
        验证框架
      语义完整性检查
        完整性检查清单
      语义一致性保证
        一致性保证机制
    迁移后数据分析
      数据质量分析
        数据质量指标
      语义一致性分析
        语义一致性检查
      性能语义分析
        性能语义验证
      业务语义验证
        业务语义验证
    数据处理策略
      数据清洗
        数据清洗流程
      数据转换
        数据格式转换
      数据验证
        数据验证流程
      数据修复
        数据修复策略
    实战案例
      电商系统迁移
        案例说明
      金融系统迁移
        案例说明
      日志系统迁移
        案例说明
```

---

## 一、语义模型理论基础

### 1.1 数据语义模型

#### SQLite数据语义模型

**核心特征**：

```text
SQLite数据语义 = {
    类型语义: 动态类型，运行时确定
    值语义: 存储类语义（NULL, INTEGER, REAL, TEXT, BLOB）
    约束语义: 运行时检查，可绕过
    关系语义: 外键可选，延迟检查
}
```

**语义层次**：

1. **物理语义**：存储格式、编码方式
2. **逻辑语义**：数据类型、约束规则
3. **业务语义**：业务含义、业务规则

#### PostgreSQL数据语义模型

**核心特征**：

```text
PostgreSQL数据语义 = {
    类型语义: 静态类型，编译时确定
    值语义: 强类型语义，类型匹配
    约束语义: 编译时检查，不可绕过
    关系语义: 外键强制，立即检查
}
```

**语义层次**：

1. **物理语义**：TOAST存储、压缩
2. **逻辑语义**：严格类型、约束
3. **业务语义**：领域模型、业务规则

### 1.2 业务语义模型

#### 业务语义定义

**业务语义 = 数据在业务上下文中的含义**:

**示例：价格语义**:

```sql
-- SQLite: 价格存储
CREATE TABLE products (
    id INTEGER PRIMARY KEY,
    price REAL  -- 语义：价格，单位：元，精度：2位小数
);

-- 业务语义：
-- 1. 价格必须 > 0
-- 2. 价格单位：人民币元
-- 3. 价格精度：2位小数
-- 4. 价格范围：0.01 - 999999.99
```

**PostgreSQL业务语义表达**：

```sql
-- PostgreSQL: 价格语义表达
CREATE DOMAIN price_type AS NUMERIC(10,2)
    CHECK (VALUE > 0 AND VALUE <= 999999.99);

CREATE TABLE products (
    id SERIAL PRIMARY KEY,
    price price_type NOT NULL  -- 语义：价格，单位：元，精度：2位小数
);
```

### 1.3 领域模型

#### 领域模型定义

**领域模型 = 业务概念在数据模型中的表示**:

**示例：订单领域模型**:

**SQLite领域模型**：

```sql
-- 订单实体
CREATE TABLE orders (
    id INTEGER PRIMARY KEY,
    user_id INTEGER,           -- 语义：用户ID，外键
    total REAL,                 -- 语义：订单总额
    status TEXT,                -- 语义：订单状态（pending/paid/shipped）
    created_at INTEGER          -- 语义：创建时间（Unix时间戳）
);

-- 领域语义：
-- 1. 订单属于用户（1对多关系）
-- 2. 订单总额 = 所有订单项金额之和
-- 3. 订单状态流转：pending → paid → shipped
-- 4. 订单创建时间不可修改
```

**PostgreSQL领域模型**：

```sql
-- 订单状态枚举
CREATE TYPE order_status AS ENUM ('pending', 'paid', 'shipped');

-- 订单实体
CREATE TABLE orders (
    id SERIAL PRIMARY KEY,
    user_id INTEGER REFERENCES users(id),
    total NUMERIC(10,2) NOT NULL CHECK (total > 0),
    status order_status NOT NULL DEFAULT 'pending',
    created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP
);

-- 领域语义表达：
-- 1. 外键约束：订单必须属于有效用户
-- 2. CHECK约束：订单总额必须 > 0
-- 3. 枚举类型：订单状态只能是预定义值
-- 4. 默认值：创建时间自动设置
```

### 1.4 语义转换理论

#### 语义等价性

**定义**：两个数据模型在语义上等价，当且仅当：

1. 所有业务规则在两个模型中表达一致
2. 所有数据约束在两个模型中效果相同
3. 所有业务操作在两个模型中结果相同

#### 语义转换规则

**规则1：类型语义转换**:

```text
SQLite类型语义 → PostgreSQL类型语义
- 保持业务语义不变
- 增强类型约束
- 保持值语义等价
```

**规则2：约束语义转换**:

```text
SQLite约束语义 → PostgreSQL约束语义
- 运行时约束 → 编译时约束
- 可选约束 → 强制约束
- 延迟检查 → 立即检查
```

**规则3：关系语义转换**:

```text
SQLite关系语义 → PostgreSQL关系语义
- 可选外键 → 强制外键
- 延迟检查 → 立即检查
- 级联操作增强
```

---

## 二、数据语义兼容性分析

### 2.1 值语义映射

#### 整数语义映射

**SQLite整数语义**：

```sql
-- SQLite: INTEGER存储类
-- 语义：任意大小的整数（1-8字节自适应）
INSERT INTO users(age) VALUES (25);        -- 语义：年龄，25岁
INSERT INTO products(stock) VALUES (1000);  -- 语义：库存，1000件
INSERT INTO orders(total_cents) VALUES (1999);  -- 语义：总金额，19.99元（以分为单位）
```

**语义分析**：

- **age**: 业务语义 = 年龄，范围 0-150
- **stock**: 业务语义 = 库存，范围 >= 0
- **total_cents**: 业务语义 = 金额（分），范围 >= 0

**PostgreSQL语义映射**：

```sql
-- PostgreSQL: 基于业务语义选择类型
CREATE TABLE users (
    age SMALLINT CHECK (age >= 0 AND age <= 150)  -- 语义：年龄，范围约束
);

CREATE TABLE products (
    stock INTEGER CHECK (stock >= 0)  -- 语义：库存，非负约束
);

CREATE TABLE orders (
    total_cents BIGINT CHECK (total_cents >= 0)  -- 语义：金额（分），大整数
);
```

#### 浮点数语义映射

**SQLite浮点数语义**：

```sql
-- SQLite: REAL存储类
-- 语义：IEEE 754双精度浮点数
INSERT INTO products(price) VALUES (19.99);      -- 语义：价格，19.99元
INSERT INTO measurements(value) VALUES (3.14159);  -- 语义：测量值，3.14159
```

**语义分析**：

- **price**: 业务语义 = 价格，精度要求2位小数
- **value**: 业务语义 = 测量值，精度要求高

**PostgreSQL语义映射**：

```sql
-- PostgreSQL: 基于业务语义选择类型
CREATE TABLE products (
    price NUMERIC(10,2) CHECK (price > 0)  -- 语义：价格，固定精度
);

CREATE TABLE measurements (
    value DOUBLE PRECISION  -- 语义：测量值，高精度
);
```

#### 文本语义映射

**SQLite文本语义**：

```sql
-- SQLite: TEXT存储类
-- 语义：UTF-8/UTF-16文本
INSERT INTO users(email) VALUES ('user@example.com');  -- 语义：邮箱地址
INSERT INTO products(name) VALUES ('商品名称');        -- 语义：商品名称
INSERT INTO articles(content) VALUES ('文章内容...');   -- 语义：文章内容
```

**语义分析**：

- **email**: 业务语义 = 邮箱地址，格式约束，长度限制
- **name**: 业务语义 = 商品名称，长度限制
- **content**: 业务语义 = 文章内容，无长度限制

**PostgreSQL语义映射**：

```sql
-- PostgreSQL: 基于业务语义选择类型和约束
CREATE TABLE users (
    email VARCHAR(255) CHECK (email ~* '^[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Z|a-z]{2,}$')
);

CREATE TABLE products (
    name VARCHAR(200) NOT NULL
);

CREATE TABLE articles (
    content TEXT  -- 语义：文章内容，无长度限制
);
```

### 2.2 类型语义转换

#### 动态类型到静态类型转换

**转换策略**：

**策略1：严格转换（推荐）**:

```sql
-- SQLite: 动态类型
CREATE TABLE mixed_data (
    id INTEGER PRIMARY KEY,
    value NUMERIC  -- 可以存储任何类型
);

-- 数据分析：检查实际存储类型
-- 发现：value列实际存储的都是整数

-- PostgreSQL: 严格类型
CREATE TABLE mixed_data (
    id SERIAL PRIMARY KEY,
    value INTEGER NOT NULL  -- 基于数据分析确定类型
);
```

**策略2：兼容转换（保守）**:

```sql
-- PostgreSQL: 保持灵活性
CREATE TABLE mixed_data (
    id SERIAL PRIMARY KEY,
    value TEXT  -- 转换为文本，保持灵活性
);
```

#### 类型语义损失分析

**损失类型1：精度语义损失**:

```sql
-- SQLite: REAL存储
INSERT INTO prices(amount) VALUES (19.999999);

-- 业务语义：价格，期望精度2位小数
-- 实际存储：19.999999（6位小数）

-- PostgreSQL: NUMERIC(10,2)
-- 转换后：20.00
-- 语义损失：精度从6位降到2位
-- 影响：可能影响精确计算
```

**损失类型2：范围语义损失**:

```sql
-- SQLite: INTEGER（8字节）
INSERT INTO big_ids(id) VALUES (9223372036854775807);

-- 业务语义：大整数ID

-- PostgreSQL: INTEGER（4字节）
-- 转换失败：值超出范围
-- 解决方案：使用BIGINT
```

### 2.3 约束语义转换

#### CHECK约束语义转换

**SQLite CHECK约束**：

```sql
CREATE TABLE products (
    id INTEGER PRIMARY KEY,
    price REAL CHECK (price > 0),
    stock INTEGER CHECK (stock >= 0 AND stock <= 10000)
);
```

**语义分析**：

- **price > 0**: 业务语义 = 价格必须为正数
- **stock范围**: 业务语义 = 库存范围 0-10000

**PostgreSQL CHECK约束**：

```sql
CREATE TABLE products (
    id SERIAL PRIMARY KEY,
    price NUMERIC(10,2) CHECK (price > 0),  -- 语义：价格必须为正数
    stock INTEGER CHECK (stock >= 0 AND stock <= 10000)  -- 语义：库存范围
);
```

**语义等价性**：✅ 完全等价

#### 外键约束语义转换

**SQLite外键语义**：

```sql
-- SQLite: 可选外键
CREATE TABLE orders (
    id INTEGER PRIMARY KEY,
    user_id INTEGER  -- 语义：用户ID，但可能无效
);

-- 业务语义：订单必须属于有效用户
-- 但SQLite不强制检查
```

**PostgreSQL外键语义**：

```sql
-- PostgreSQL: 强制外键
CREATE TABLE orders (
    id SERIAL PRIMARY KEY,
    user_id INTEGER REFERENCES users(id)  -- 语义：订单必须属于有效用户
);

-- 业务语义：订单必须属于有效用户
-- PostgreSQL强制检查
```

**语义增强**：PostgreSQL增强了业务语义的强制力

### 2.4 关系语义转换

#### 一对一关系语义

**SQLite一对一关系**：

```sql
-- SQLite: 一对一关系（应用层保证）
CREATE TABLE users (
    id INTEGER PRIMARY KEY,
    name TEXT
);

CREATE TABLE user_profiles (
    user_id INTEGER PRIMARY KEY,  -- 语义：一对一关系
    bio TEXT
);
```

**PostgreSQL一对一关系**：

```sql
-- PostgreSQL: 一对一关系（数据库层保证）
CREATE TABLE users (
    id SERIAL PRIMARY KEY,
    name VARCHAR(255) NOT NULL
);

CREATE TABLE user_profiles (
    user_id INTEGER PRIMARY KEY REFERENCES users(id) ON DELETE CASCADE,
    bio TEXT,
    CONSTRAINT unique_user_profile UNIQUE (user_id)  -- 语义：一对一关系
);
```

#### 一对多关系语义

**SQLite一对多关系**：

```sql
-- SQLite: 一对多关系
CREATE TABLE users (
    id INTEGER PRIMARY KEY,
    name TEXT
);

CREATE TABLE orders (
    id INTEGER PRIMARY KEY,
    user_id INTEGER  -- 语义：多对一关系（多个订单属于一个用户）
);
```

**PostgreSQL一对多关系**：

```sql
-- PostgreSQL: 一对多关系（外键约束）
CREATE TABLE users (
    id SERIAL PRIMARY KEY,
    name VARCHAR(255) NOT NULL
);

CREATE TABLE orders (
    id SERIAL PRIMARY KEY,
    user_id INTEGER NOT NULL REFERENCES users(id) ON DELETE CASCADE
    -- 语义：多对一关系，级联删除
);
```

#### 多对多关系语义

**SQLite多对多关系**：

```sql
-- SQLite: 多对多关系（中间表）
CREATE TABLE users (
    id INTEGER PRIMARY KEY,
    name TEXT
);

CREATE TABLE roles (
    id INTEGER PRIMARY KEY,
    name TEXT
);

CREATE TABLE user_roles (
    user_id INTEGER,
    role_id INTEGER,
    PRIMARY KEY (user_id, role_id)  -- 语义：多对多关系
);
```

**PostgreSQL多对多关系**：

```sql
-- PostgreSQL: 多对多关系（外键约束）
CREATE TABLE users (
    id SERIAL PRIMARY KEY,
    name VARCHAR(255) NOT NULL
);

CREATE TABLE roles (
    id SERIAL PRIMARY KEY,
    name VARCHAR(255) NOT NULL
);

CREATE TABLE user_roles (
    user_id INTEGER NOT NULL REFERENCES users(id) ON DELETE CASCADE,
    role_id INTEGER NOT NULL REFERENCES roles(id) ON DELETE CASCADE,
    PRIMARY KEY (user_id, role_id)  -- 语义：多对多关系，级联删除
);
```

---

## 三、业务语义兼容性分析

### 3.1 业务规则语义

#### 业务规则定义

**业务规则 = 业务领域的约束和逻辑**:

**示例：电商系统业务规则**:

**规则1：订单总额计算**:

```sql
-- SQLite: 业务规则（应用层实现）
-- 规则：订单总额 = 所有订单项金额之和

-- 应用层代码
def calculate_order_total(order_id):
    items = db.execute("SELECT price, quantity FROM order_items WHERE order_id = ?", order_id)
    total = sum(item.price * item.quantity for item in items)
    db.execute("UPDATE orders SET total = ? WHERE id = ?", total, order_id)
```

**PostgreSQL: 业务规则（数据库层实现）**:

```sql
-- PostgreSQL: 使用生成列或触发器
CREATE TABLE orders (
    id SERIAL PRIMARY KEY,
    total NUMERIC(10,2) GENERATED ALWAYS AS (
        SELECT COALESCE(SUM(price * quantity), 0)
        FROM order_items
        WHERE order_id = orders.id
    ) STORED
);

-- 语义：订单总额自动计算，保证一致性
```

#### 业务规则语义转换

**转换策略**：

**策略1：数据库层实现（推荐）**:

- 优点：数据一致性保证
- 缺点：数据库逻辑复杂

**策略2：应用层实现（兼容）**:

- 优点：逻辑集中
- 缺点：需要保证一致性

### 3.2 业务流程语义

#### 状态流转语义

**SQLite状态流转**：

```sql
-- SQLite: 状态存储
CREATE TABLE orders (
    id INTEGER PRIMARY KEY,
    status TEXT  -- 语义：订单状态
);

-- 应用层状态流转逻辑
def update_order_status(order_id, new_status):
    current_status = get_order_status(order_id)

    # 业务规则：状态流转
    valid_transitions = {
        'pending': ['paid', 'cancelled'],
        'paid': ['shipped', 'refunded'],
        'shipped': ['delivered'],
        'delivered': [],
        'cancelled': [],
        'refunded': []
    }

    if new_status not in valid_transitions.get(current_status, []):
        raise ValueError("Invalid status transition")

    db.execute("UPDATE orders SET status = ? WHERE id = ?", new_status, order_id)
```

**PostgreSQL状态流转**：

```sql
-- PostgreSQL: 状态枚举 + 触发器
CREATE TYPE order_status AS ENUM ('pending', 'paid', 'shipped', 'delivered', 'cancelled', 'refunded');

CREATE TABLE orders (
    id SERIAL PRIMARY KEY,
    status order_status NOT NULL DEFAULT 'pending'
);

-- 状态流转触发器
CREATE OR REPLACE FUNCTION validate_order_status_transition()
RETURNS TRIGGER AS $$
BEGIN
    -- 业务规则：状态流转验证
    IF (OLD.status = 'pending' AND NEW.status NOT IN ('paid', 'cancelled')) OR
       (OLD.status = 'paid' AND NEW.status NOT IN ('shipped', 'refunded')) OR
       (OLD.status = 'shipped' AND NEW.status != 'delivered') OR
       (OLD.status IN ('delivered', 'cancelled', 'refunded')) THEN
        RAISE EXCEPTION 'Invalid status transition from % to %', OLD.status, NEW.status;
    END IF;
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;

CREATE TRIGGER order_status_transition_trigger
    BEFORE UPDATE ON orders
    FOR EACH ROW
    WHEN (OLD.status IS DISTINCT FROM NEW.status)
    EXECUTE FUNCTION validate_order_status_transition();

-- 语义：状态流转在数据库层保证
```

### 3.3 业务逻辑语义

#### 计算逻辑语义

**SQLite计算逻辑**：

```sql
-- SQLite: 应用层计算
-- 业务逻辑：计算订单折扣

def calculate_discount(order_id):
    order = db.execute("SELECT total, user_id FROM orders WHERE id = ?", order_id).fetchone()
    user = db.execute("SELECT vip_level FROM users WHERE id = ?", order.user_id).fetchone()

    # 业务逻辑：VIP用户折扣
    if user.vip_level == 'gold':
        discount = order.total * 0.1  -- 10%折扣
    elif user.vip_level == 'silver':
        discount = order.total * 0.05  -- 5%折扣
    else:
        discount = 0

    return discount
```

**PostgreSQL计算逻辑**：

```sql
-- PostgreSQL: 数据库函数
CREATE OR REPLACE FUNCTION calculate_discount(order_id INTEGER)
RETURNS NUMERIC(10,2) AS $$
DECLARE
    order_total NUMERIC(10,2);
    user_vip_level VARCHAR(20);
    discount NUMERIC(10,2);
BEGIN
    -- 获取订单信息
    SELECT o.total, u.vip_level
    INTO order_total, user_vip_level
    FROM orders o
    JOIN users u ON o.user_id = u.id
    WHERE o.id = order_id;

    -- 业务逻辑：VIP用户折扣
    IF user_vip_level = 'gold' THEN
        discount := order_total * 0.1;
    ELSIF user_vip_level = 'silver' THEN
        discount := order_total * 0.05;
    ELSE
        discount := 0;
    END IF;

    RETURN discount;
END;
$$ LANGUAGE plpgsql;

-- 语义：计算逻辑在数据库层实现
```

### 3.4 业务约束语义

#### 复杂业务约束

**SQLite业务约束**：

```sql
-- SQLite: 应用层约束
-- 业务约束：用户不能同时拥有超过3个未完成订单

def create_order(user_id):
    # 检查约束
    pending_count = db.execute(
        "SELECT COUNT(*) FROM orders WHERE user_id = ? AND status = 'pending'",
        user_id
    ).fetchone()[0]

    if pending_count >= 3:
        raise ValueError("User cannot have more than 3 pending orders")

    # 创建订单
    db.execute("INSERT INTO orders (user_id, status) VALUES (?, 'pending')", user_id)
```

**PostgreSQL业务约束**：

```sql
-- PostgreSQL: 数据库约束
CREATE OR REPLACE FUNCTION check_pending_orders_limit()
RETURNS TRIGGER AS $$
DECLARE
    pending_count INTEGER;
BEGIN
    -- 业务约束：检查未完成订单数量
    SELECT COUNT(*) INTO pending_count
    FROM orders
    WHERE user_id = NEW.user_id AND status = 'pending';

    IF pending_count >= 3 THEN
        RAISE EXCEPTION 'User cannot have more than 3 pending orders';
    END IF;

    RETURN NEW;
END;
$$ LANGUAGE plpgsql;

CREATE TRIGGER pending_orders_limit_trigger
    BEFORE INSERT ON orders
    FOR EACH ROW
    WHEN (NEW.status = 'pending')
    EXECUTE FUNCTION check_pending_orders_limit();

-- 语义：业务约束在数据库层保证
```

---

## 四、领域模型转换

### 4.1 实体模型转换

#### 实体识别

**SQLite实体模型**：

```sql
-- 用户实体
CREATE TABLE users (
    id INTEGER PRIMARY KEY,
    name TEXT,
    email TEXT,
    created_at INTEGER
);

-- 实体语义：
-- 1. 用户是业务实体
-- 2. 用户有唯一标识（id）
-- 3. 用户有属性（name, email, created_at）
```

**PostgreSQL实体模型**：

```sql
-- 用户实体（增强语义）
CREATE TABLE users (
    id SERIAL PRIMARY KEY,
    name VARCHAR(255) NOT NULL,
    email VARCHAR(255) NOT NULL UNIQUE,  -- 语义：邮箱唯一
    created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP
);

-- 实体语义增强：
-- 1. 类型约束：name和email非空
-- 2. 唯一性约束：email唯一
-- 3. 默认值：created_at自动设置
```

### 4.2 关系模型转换

#### 聚合关系转换

**SQLite聚合关系**：

```sql
-- SQLite: 订单聚合
CREATE TABLE orders (
    id INTEGER PRIMARY KEY,
    user_id INTEGER,
    total REAL
);

CREATE TABLE order_items (
    id INTEGER PRIMARY KEY,
    order_id INTEGER,
    product_id INTEGER,
    quantity INTEGER,
    price REAL
);

-- 聚合语义：订单聚合订单项（应用层保证）
```

**PostgreSQL聚合关系**：

```sql
-- PostgreSQL: 订单聚合（数据库层保证）
CREATE TABLE orders (
    id SERIAL PRIMARY KEY,
    user_id INTEGER NOT NULL REFERENCES users(id) ON DELETE CASCADE,
    total NUMERIC(10,2) NOT NULL
);

CREATE TABLE order_items (
    id SERIAL PRIMARY KEY,
    order_id INTEGER NOT NULL REFERENCES orders(id) ON DELETE CASCADE,
    product_id INTEGER NOT NULL REFERENCES products(id),
    quantity INTEGER NOT NULL CHECK (quantity > 0),
    price NUMERIC(10,2) NOT NULL CHECK (price > 0)
);

-- 聚合语义：级联删除保证聚合完整性
```

### 4.3 聚合模型转换

#### 聚合根识别

**SQLite聚合模型**：

```sql
-- SQLite: 订单聚合（应用层管理）
-- 聚合根：orders
-- 聚合成员：order_items

-- 应用层代码
class Order:
    def __init__(self, order_id):
        self.id = order_id
        self.items = []  # 聚合成员

    def add_item(self, product_id, quantity, price):
        # 业务规则：聚合内逻辑
        self.items.append({
            'product_id': product_id,
            'quantity': quantity,
            'price': price
        })

    def save(self):
        # 保存聚合
        db.execute("INSERT INTO orders ...")
        for item in self.items:
            db.execute("INSERT INTO order_items ...", item)
```

**PostgreSQL聚合模型**：

```sql
-- PostgreSQL: 订单聚合（数据库层保证）
-- 聚合根：orders
-- 聚合成员：order_items

-- 使用外键和级联操作保证聚合完整性
CREATE TABLE orders (
    id SERIAL PRIMARY KEY,
    -- 聚合根属性
);

CREATE TABLE order_items (
    id SERIAL PRIMARY KEY,
    order_id INTEGER NOT NULL REFERENCES orders(id) ON DELETE CASCADE,
    -- 聚合成员属性
);

-- 语义：数据库层保证聚合完整性
```

### 4.4 值对象转换

#### 值对象识别

**SQLite值对象**：

```sql
-- SQLite: 地址值对象（扁平化存储）
CREATE TABLE users (
    id INTEGER PRIMARY KEY,
    name TEXT,
    address_street TEXT,
    address_city TEXT,
    address_zip TEXT
);

-- 值对象语义：地址是值对象，没有独立标识
```

**PostgreSQL值对象**：

```sql
-- PostgreSQL: 地址值对象（复合类型）
CREATE TYPE address_type AS (
    street VARCHAR(255),
    city VARCHAR(100),
    zip VARCHAR(20)
);

CREATE TABLE users (
    id SERIAL PRIMARY KEY,
    name VARCHAR(255) NOT NULL,
    address address_type  -- 语义：地址值对象
);

-- 值对象语义：使用复合类型表达值对象
```

---

## 五、数据迁移语义保证

### 5.1 语义等价性验证

#### 验证框架

```python
class SemanticEquivalenceValidator:
    def __init__(self, sqlite_conn, pg_conn):
        self.sqlite_conn = sqlite_conn
        self.pg_conn = pg_conn

    def validate_type_semantics(self, table_name, column_name):
        """验证类型语义等价性"""
        # 1. 检查SQLite实际类型
        sqlite_types = self._analyze_sqlite_types(table_name, column_name)

        # 2. 检查PostgreSQL类型
        pg_type = self._get_pg_type(table_name, column_name)

        # 3. 验证语义等价性
        return self._compare_type_semantics(sqlite_types, pg_type)

    def validate_constraint_semantics(self, table_name):
        """验证约束语义等价性"""
        # 1. 检查SQLite约束
        sqlite_constraints = self._get_sqlite_constraints(table_name)

        # 2. 检查PostgreSQL约束
        pg_constraints = self._get_pg_constraints(table_name)

        # 3. 验证语义等价性
        return self._compare_constraint_semantics(sqlite_constraints, pg_constraints)

    def validate_relationship_semantics(self, table_name):
        """验证关系语义等价性"""
        # 1. 检查SQLite关系
        sqlite_relations = self._get_sqlite_relations(table_name)

        # 2. 检查PostgreSQL关系
        pg_relations = self._get_pg_relations(table_name)

        # 3. 验证语义等价性
        return self._compare_relationship_semantics(sqlite_relations, pg_relations)
```

### 5.2 语义完整性检查

#### 完整性检查清单

```python
def check_semantic_integrity(sqlite_conn, pg_conn):
    """语义完整性检查"""
    checks = {
        'type_semantics': check_type_semantics(sqlite_conn, pg_conn),
        'constraint_semantics': check_constraint_semantics(sqlite_conn, pg_conn),
        'relationship_semantics': check_relationship_semantics(sqlite_conn, pg_conn),
        'business_rule_semantics': check_business_rule_semantics(sqlite_conn, pg_conn),
        'data_semantics': check_data_semantics(sqlite_conn, pg_conn)
    }

    return checks
```

### 5.3 语义一致性保证

#### 一致性保证机制

```python
def ensure_semantic_consistency(sqlite_conn, pg_conn, migration_plan):
    """保证语义一致性"""
    # 1. 预迁移语义分析
    pre_analysis = analyze_semantics(sqlite_conn)

    # 2. 执行迁移
    execute_migration(sqlite_conn, pg_conn, migration_plan)

    # 3. 后迁移语义验证
    post_analysis = analyze_semantics(pg_conn)

    # 4. 语义一致性验证
    consistency = verify_semantic_consistency(pre_analysis, post_analysis)

    if not consistency:
        rollback_migration(pg_conn)
        raise SemanticConsistencyError("Semantic consistency check failed")

    return True
```

---

## 六、迁移后数据分析

### 6.1 数据质量分析

#### 数据质量指标

```python
def analyze_data_quality(pg_conn):
    """分析数据质量"""
    quality_metrics = {
        'completeness': analyze_completeness(pg_conn),
        'accuracy': analyze_accuracy(pg_conn),
        'consistency': analyze_consistency(pg_conn),
        'validity': analyze_validity(pg_conn)
    }

    return quality_metrics

def analyze_completeness(pg_conn):
    """分析数据完整性"""
    cursor = pg_conn.cursor()

    completeness_report = {}

    # 检查每个表的NULL值比例
    cursor.execute("""
        SELECT table_name, column_name,
               COUNT(*) as total_rows,
               COUNT(*) FILTER (WHERE {column_name} IS NULL) as null_count,
               ROUND(100.0 * COUNT(*) FILTER (WHERE {column_name} IS NULL) / COUNT(*), 2) as null_percentage
        FROM information_schema.columns
        WHERE table_schema = 'public'
        GROUP BY table_name, column_name
    """)

    return cursor.fetchall()
```

### 6.2 语义一致性分析

#### 语义一致性检查

```python
def analyze_semantic_consistency(pg_conn):
    """分析语义一致性"""
    # 1. 检查类型语义一致性
    type_consistency = check_type_semantic_consistency(pg_conn)

    # 2. 检查约束语义一致性
    constraint_consistency = check_constraint_semantic_consistency(pg_conn)

    # 3. 检查关系语义一致性
    relationship_consistency = check_relationship_semantic_consistency(pg_conn)

    return {
        'type_consistency': type_consistency,
        'constraint_consistency': constraint_consistency,
        'relationship_consistency': relationship_consistency
    }
```

### 6.3 性能语义分析

#### 性能语义验证

```python
def analyze_performance_semantics(pg_conn):
    """分析性能语义"""
    # 1. 查询性能分析
    query_performance = analyze_query_performance(pg_conn)

    # 2. 索引使用分析
    index_usage = analyze_index_usage(pg_conn)

    # 3. 连接性能分析
    connection_performance = analyze_connection_performance(pg_conn)

    return {
        'query_performance': query_performance,
        'index_usage': index_usage,
        'connection_performance': connection_performance
    }
```

### 6.4 业务语义验证

#### 业务语义验证

```python
def validate_business_semantics(pg_conn, business_rules):
    """验证业务语义"""
    violations = []

    for rule in business_rules:
        result = execute_business_rule_check(pg_conn, rule)
        if not result['passed']:
            violations.append({
                'rule': rule['name'],
                'violations': result['violations']
            })

    return {
        'passed': len(violations) == 0,
        'violations': violations
    }
```

---

## 七、数据处理策略

### 7.1 数据清洗

#### 数据清洗流程

```python
def clean_migrated_data(pg_conn):
    """清洗迁移后的数据"""
    # 1. 清理NULL值
    clean_null_values(pg_conn)

    # 2. 清理重复数据
    clean_duplicates(pg_conn)

    # 3. 清理无效数据
    clean_invalid_data(pg_conn)

    # 4. 标准化数据格式
    normalize_data_format(pg_conn)
```

### 7.2 数据转换

#### 数据格式转换

```python
def transform_data_format(pg_conn):
    """转换数据格式"""
    # 1. 日期时间格式转换
    transform_datetime_format(pg_conn)

    # 2. 数值格式转换
    transform_numeric_format(pg_conn)

    # 3. 文本格式转换
    transform_text_format(pg_conn)
```

### 7.3 数据验证

#### 数据验证流程

```python
def validate_migrated_data(pg_conn, validation_rules):
    """验证迁移后的数据"""
    validation_results = {}

    for rule in validation_rules:
        result = execute_validation_rule(pg_conn, rule)
        validation_results[rule['name']] = result

    return validation_results
```

### 7.4 数据修复

#### 数据修复策略

```python
def repair_migrated_data(pg_conn, repair_rules):
    """修复迁移后的数据"""
    repair_results = {}

    for rule in repair_rules:
        result = execute_repair_rule(pg_conn, rule)
        repair_results[rule['name']] = result

    return repair_results
```

---

## 八、语义模型兼容性多维对比矩阵

### 8.1 数据语义模型对比矩阵

| 维度 | SQLite动态类型 | PostgreSQL静态类型 | 转换难度 | 语义损失 |
|------|---------------|-------------------|---------|---------|
| **类型检查** | 运行时 | 编译时 | ⭐⭐⭐ | ⭐⭐⭐ |
| **类型转换** | 自动 | 显式 | ⭐⭐⭐⭐ | ⭐⭐⭐ |
| **NULL处理** | 宽松 | 严格 | ⭐⭐⭐ | ⭐⭐ |
| **数值精度** | 动态 | 固定 | ⭐⭐⭐⭐ | ⭐⭐⭐ |
| **字符串编码** | UTF-8 | 可配置 | ⭐⭐⭐ | ⭐⭐ |
| **日期时间** | TEXT/REAL | TIMESTAMP | ⭐⭐⭐⭐ | ⭐⭐⭐ |

### 8.2 业务语义转换策略对比矩阵

| 维度 | 直接映射 | 语义转换 | 业务规则重写 | 领域模型重构 |
|------|---------|---------|------------|------------|
| **实施难度** | ⭐⭐⭐⭐⭐ | ⭐⭐⭐ | ⭐⭐ | ⭐ |
| **语义保真度** | ⭐⭐⭐ | ⭐⭐⭐⭐ | ⭐⭐⭐⭐⭐ | ⭐⭐⭐⭐⭐ |
| **性能影响** | ⭐⭐⭐⭐⭐ | ⭐⭐⭐⭐ | ⭐⭐⭐ | ⭐⭐⭐ |
| **适用场景** | 简单业务 | 中等复杂度 | 复杂业务 | 大型系统 |
| **推荐度** | ⭐⭐⭐⭐ | ⭐⭐⭐⭐⭐推荐 | ⭐⭐⭐⭐推荐 | ⭐⭐⭐推荐 |

### 8.3 迁移策略对比矩阵

| 维度 | 全量迁移 | 增量迁移 | 双写迁移 | 灰度迁移 |
|------|---------|---------|---------|---------|
| **停机时间** | 长 | 短 | 无 | 短 |
| **数据一致性** | ⭐⭐⭐⭐⭐ | ⭐⭐⭐⭐ | ⭐⭐⭐ | ⭐⭐⭐⭐ |
| **实施复杂度** | ⭐⭐⭐ | ⭐⭐⭐⭐ | ⭐⭐⭐⭐⭐ | ⭐⭐⭐⭐ |
| **风险控制** | ⭐⭐⭐ | ⭐⭐⭐⭐ | ⭐⭐⭐⭐⭐ | ⭐⭐⭐⭐⭐ |
| **适用场景** | 小数据量 | 大数据量 | 高可用要求 | 生产环境 |
| **推荐度** | ⭐⭐⭐ | ⭐⭐⭐⭐推荐 | ⭐⭐⭐⭐⭐推荐 | ⭐⭐⭐⭐⭐推荐 |

## 九、实战案例

### 9.1 电商系统迁移

**场景**：电商系统从SQLite迁移到PostgreSQL

**语义模型转换**：

- 订单聚合模型转换
- 商品分类树模型转换
- 用户权限模型转换

### 9.2 金融系统迁移

**场景**：金融系统从SQLite迁移到PostgreSQL

**语义模型转换**：

- 账户模型转换
- 交易模型转换
- 风控规则模型转换

### 9.3 日志系统迁移

**场景**：日志系统从SQLite迁移到PostgreSQL

**语义模型转换**：

- 日志聚合模型转换
- 时间序列模型转换
- 索引模型转换

---

## 🔗 相关资源

- [10.01 SQLite到PostgreSQL迁移指南](./10.01-SQLite到PostgreSQL迁移指南.md)
- [10.02 SQL兼容性深度分析](./10.02-SQL兼容性深度分析.md)
- [10.03 二进制复制与增量迁移](./10.03-二进制复制与增量迁移.md)

---

## 🔗 交叉引用

### 理论模型 🆕

- ⭐⭐⭐ [数据模型理论](../11-理论模型/11.02-数据模型理论.md) - 语义模型理论、领域模型理论、数据语义理论
- ⭐ [系统理论模型](../11-理论模型/11.01-系统理论模型.md) - 语义系统理论

### 设计模型 🆕

- ⭐⭐⭐ [设计决策](../12-设计模型/12.04-设计决策.md) - 语义模型转换决策、领域模型设计决策
- ⭐⭐ [系统演进](../12-设计模型/12.05-系统演进.md) - 语义模型演进

---

## 📚 参考资料

- [领域驱动设计](https://www.domainlanguage.com/ddd/)
- [数据语义建模](https://www.w3.org/TR/vocab-dcat/)
- [PostgreSQL领域类型](https://www.postgresql.org/docs/current/extend-type-system.html)

---

**维护者**：Data-Science Team
**最后更新**：2025-01-15

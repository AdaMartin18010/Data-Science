...
- [编译执行模型：SQL → 字节码 → 磁盘操作](#编译执行模型sql--字节码--磁盘操作)
  ...
  - [12.3. 编译执行模型正反例证明](#123-编译执行模型正反例证明)
    - [12.3.1. 预编译语句正反例](#1231-预编译语句正反例)
      - [正例：预编译语句高效执行](#正例预编译语句高效执行)
      - [反例：字符串拼接每次编译](#反例字符串拼接每次编译)
    - [12.3.2. 查询优化正反例](#1232-查询优化正反例)
      - [正例：查询优化器选择最优索引](#正例查询优化器选择最优索引)
      - [反例：无索引全表扫描](#反例无索引全表扫描)
  ...
  - [17. 🔗 交叉引用](#17--交叉引用)
    - [17.1. 相关文档](#171-相关文档)
      ...
      - [17.1.7. 知识图谱与导航 🆕](#1717-知识图谱与导航-)
    - [17.2. 相关概念链接 🆕](#172-相关概念链接-)
      - [17.2.1. 执行模型概念](#1721-执行模型概念)
      - [17.2.2. 执行模型路径](#1722-执行模型路径)
...
2     Integer        123   1     0                    0   r[1]=123
3     SeekGE         0     11    1     1              0   key=r[1]
4     IdxGT          0     11    1     1              0   key=r[1]
5     Column         0     1     2                    0   r[2]=users.name
6     Column         0     2     3                    0   r[3]=users.age
...
┌────────────────────────────────────────────────────────────────────┐
│ 指令: Init 0 12 0                                                  │
│ 动作: 初始化VDBE，跳转到addr=12                                   │
│ 状态: PC=12                                                        │
└────────────────────────────────────────────────────────────────────┘

Step 1 (PC=12): Transaction
┌────────────────────────────────────────────────────────────────────┐
│ 指令: Transaction 0 0 1 0                                          │
│ 动作: 开始读事务                                                   │
│ 状态: 事务状态=READING, PC=13                                     │
└────────────────────────────────────────────────────────────────────┘
...
Step 3 (PC=1): OpenRead
┌────────────────────────────────────────────────────────────────────┐
│ 指令: OpenRead 0 2 0 3                                             │
│ 动作: 打开表users（root_page=2）到游标0                           │
│ 状态: Cursor[0] = BtCursor(users), PC=2                           │
│                                                                    │
│ 内部调用:                                                          │
│   sqlite3BtreeCursor(db, root_page=2, wrFlag=0, cursor=0)         │
└────────────────────────────────────────────────────────────────────┘
...
┌────────────────────────────────────────────────────────────────────┐
│ 指令: SeekGE 0 11 1 1                                              │
│ 动作: 在游标0上查找 >= r[1](123) 的记录                           │
│       如果未找到，跳转到addr=11                                    │
│ 状态: Cursor[0]定位到id=123的记录, PC=4                           │
│                                                                    │
│ 内部调用:                                                          │
│   sqlite3BtreeMoveto(cursor[0], key=123, bias=0)                  │
│   结果: 找到记录（假设id=123存在）                                │
└────────────────────────────────────────────────────────────────────┘
...
┌────────────────────────────────────────────────────────────────────┐
│ 指令: IdxGT 0 11 1 1                                               │
│ 动作: 如果当前记录的key > r[1]，跳转到addr=11                     │
│ 状态: 当前key=123 不大于 123，继续执行, PC=5                      │
└────────────────────────────────────────────────────────────────────┘

Step 7 (PC=5): Column
┌────────────────────────────────────────────────────────────────────┐
│ 指令: Column 0 1 2                                                 │
│ 动作: 从游标0读取第1列（name）到寄存器r[2]                        │
│ 状态: r[2] = "Alice", PC=6                                        │
│                                                                    │
│ 内部调用:                                                          │
│   sqlite3BtreePayload(cursor[0], offset, length, buffer)          │
│   解析B-Tree单元格，提取name列的值                                │
│                                                                    │
│ 寄存器状态:                                                        │
│   r[1] = 123 (INTEGER)                                            │
│   r[2] = "Alice" (TEXT)                                           │
...
Step 9 (PC=7): ResultRow
┌────────────────────────────────────────────────────────────────────┐
│ 指令: ResultRow 2 2 0                                              │
│ 动作: 输出寄存器r[2]和r[3]作为结果行                              │
│ 状态: 返回给应用层: ("Alice", 30), PC=8                           │
│                                                                    │
│ 内部操作:                                                          │
│   将r[2]和r[3]复制到结果缓冲区                                    │
│   sqlite3_step()返回SQLITE_ROW                                     │
└────────────────────────────────────────────────────────────────────┘

Step 10 (PC=8): Next
┌────────────────────────────────────────────────────────────────────┐
│ 指令: Next 0 4 0 1                                                 │
│ 动作: 移动游标0到下一行，如果有数据跳转到addr=4                   │
│ 状态: 已到达表尾，继续执行, PC=9                                  │
│                                                                    │
│ 内部调用:                                                          │
│   sqlite3BtreeNext(cursor[0])                                      │
│   返回: SQLITE_OK, 但EOF=true                                      │
└────────────────────────────────────────────────────────────────────┘
...
│ 指令: Halt 0 0 0                                                   │
│ 动作: 停止执行，返回成功                                          │
│ 状态: 执行完成，sqlite3_step()返回SQLITE_DONE                     │
└────────────────────────────────────────────────────────────────────┘
...
执行过程总结:
• 总指令数: 13条
• 实际执行: 12步（跳过了addr=11的Noop）
• 返回结果: 1行 ("Alice", 30)
• 执行时间: < 1ms (假设数据在缓存中)

关键性能点:
• SeekGE: 使用B-Tree索引快速定位（O(log n)）
• Column: 从B-Tree单元格解析列值
• ResultRow: 零拷贝返回（如果可能）
```
...
start = time.time()
for i in range(10000):
    result = stmt.execute((i % 1000,)).fetchone()
...
start = time.time()
for i in range(10000):
    sql = f"SELECT * FROM users WHERE id = {i % 1000}"
...
# 查询使用索引
start = time.time()
result = conn.execute('SELECT * FROM users WHERE email = ?', ('test@example.com',)).fetchone()
elapsed = time.time() - start

# 检查查询计划
plan = conn.execute('EXPLAIN QUERY PLAN SELECT * FROM users WHERE email = ?', ('test@example.com',)).fetchall()
print(f"查询计划: {plan}")
...
- ⭐⭐ [定理依赖关系图谱](../06-形式化理论/06.06-SQLite定理依赖关系图谱.md) - 执行模型相关定理
- ⭐⭐⭐ [知识图谱与概念关系网络](../09-最新特性/09.03-SQLite知识图谱与概念关系网络.md) - 执行模型概念关系
...
- ⭐⭐⭐ [知识图谱与概念关系网络](../09-最新特性/09.03-SQLite知识图谱与概念关系网络.md) - 执行模型概念关系
- ⭐⭐ [文档依赖关系图](../00-项目导航/06-文档依赖关系图.md) - 执行模型文档依赖
- ⭐⭐ [术语标准化词典](../00-项目导航/03-术语词典/SQLite术语标准化词典.md) - 执行模型术语索引

### 17.2. 相关概念链接 🆕

#### 17.2.1. 执行模型概念

- **VDBE虚拟机** → [知识图谱：VDBE概念](../09-最新特性/09.03-SQLite知识图谱与概念关系网络.md#核心本体编译执行)
- **查询计划** → [知识图谱：查询计划概念](../09-最新特性/09.03-SQLite知识图谱与概念关系网络.md#核心本体编译执行)
- **B-Tree操作** → [定理：C16-B-Tree操作正确性](../06-形式化理论/06.06-SQLite定理依赖关系图谱.md#c16-b-tree操作正确性)

#### 17.2.2. 执行模型路径

- **执行模型知识路径** → [知识图谱：执行模型路径](../09-最新特性/09.03-SQLite知识图谱与概念关系网络.md#路径1基础入门路径)

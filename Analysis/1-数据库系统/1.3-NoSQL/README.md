...
- 符号表征（数据模型、查询语言、事务、索引、分布式一致性等）
- 图结构（ER图、系统架构图、数据流图、分布式一致性图等）
- 向量/张量（数据嵌入、特征向量、参数矩阵）
- 自然语言（定义、注释、描述）
- 图像/可视化（结构图、流程图、查询与一致性可视化等）
这些表征可互映，提升NoSQL理论与实践表达力。

## 10. 形式化语义

- 语义域：$D$，如数据对象集、关系空间、查询模型、事务空间、一致性模型
- 解释函数：$I: S \to D$，将符号/结构映射到具体NoSQL语义对象
- 语义一致性：每个结构/查询/事务/一致性规则在$D$中有明确定义

## 11. 形式化语法与证明

- 语法规则：如数据模型定义、查询语言语法、事务规则、一致性约束、索引定义

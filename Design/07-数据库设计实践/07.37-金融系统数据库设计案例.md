...
- **SecureBank：面向高安全性银行系统的金融感知零信任架构**：SecureBank是一种专为高安全性银行系统设计的金融感知零信任架构。该框架集成了金融零信任、自适应身份评分、上下文微分段和影响驱动的安全自动化等元素。通过蒙特卡洛模拟评估，SecureBank在自动化攻击处理和身份信任适应方面表现出色（arXiv:2512.23124，2025）
- **FinML-Chain：增强金融机器学习的区块链集成数据集**：FinML-Chain提出了一个将高频链上数据与低频链下数据集成的框架，旨在为经济机制设计中的新研究问题提供基准。该框架生成模块化、可扩展的数据集，用于分析交易费用机制等经济机制（arXiv:2411.16277，2024）
- **FISHNET：金融智能生成的代理架构**：FISHNET是一种代理架构，旨在从超过98,000份监管文件中完成高度复杂的分析任务。该架构展示了在金融洞察生成方面的卓越性能，成功率达到61.8%（arXiv:2410.19727，2024）
- **DPFedBank：面向金融机构的隐私保护联邦学习框架**：DPFedBank是一个创新框架，允许金融机构在确保数据隐私的同时协作开发机器学习模型。该框架通过本地差分隐私（LDP）机制，确保数据在协作过程中保持机密性（arXiv:2410.13753，2024）
- **YashanDB在金融科技领域的应用趋势**：YashanDB支持单机主备、分布式集群及共享集群三种部署形态，适应金融领域多样化的业务场景。其先进的存储引擎和高效的SQL处理能力，满足了金融科技应用的多层次需求（InfoQ，2025）
- **金融业数据库创新发展报告**：《2024年金融业数据库创新发展报告》总结了金融业数据库技术产品的应用进展，提出了核心系统转型升级的路径和方法论。报告指出，集中式数据库在中小金融机构中仍占主导地位，而分布式数据库在大型金融机构中的重要性日益增强（搜狐，2024）
- **金融业开源技术应用与发展报告**：《2025金融业开源技术应用与发展报告》系统梳理了金融机构在开源技术应用现状、典型场景实践、发展挑战及未来趋势，揭示了开源技术对金融数字化转型的重要推动作用（搜狐，2025）

...
- [数据库备份与恢复设计模式：数据保护与灾难恢复](#数据库备份与恢复设计模式数据保护与灾难恢复)
  ...
  - [9. 2024-2025最新趋势](#9-2024-2025最新趋势)
    ...
    - [9.2. 备份方案选型矩阵](#92-备份方案选型矩阵)
    - [9.3. 云原生备份架构](#93-云原生备份架构)
    - [9.4. pgBackRest配置示例](#94-pgbackrest配置示例)
  - [10. 参考资料](#10-参考资料)
    - [10.1. 权威文献](#101-权威文献)
    - [10.2. 在线资源](#102-在线资源)
    - [10.3. 相关文档](#103-相关文档)
...
| 方案 | RPO | RTO | 成本 | 复杂度 | 适用场景 |
|------|-----|-----|------|--------|---------|
| **pg_dump** | 高 | 中 | 低 | 低 | 小型系统 |
| **WAL归档+PITR** | 低 | 中 | 中 | 中 | 生产系统 |
| **流复制** | 极低 | 低 | 中 | 中 | 高可用 |
| **pgBackRest** | 低 | 低 | 中 | 中 | 企业级 |
| **Barman** | 低 | 低 | 中 | 中 | 灾备 |
| **云托管备份** | 极低 | 低 | 高 | 低 | 云原生 |
...
- **阿里云RDS MySQL任意时间点恢复功能升级**：自2024年1月11日起，阿里云对RDS MySQL的日志备份能力进行了升级，推出了任意时间点恢复（PITR）功能。用户无需关注全量和日志备份保留时间范围的一致性，只需配置可恢复的天数，系统即可实现预期的任意时间点保护（阿里云，2024）
- **华为云备份服务（CBR）文档升级**：2024年9月，华为云对云备份CBR的帮助文档进行了焕新升级，聚焦典型入门场景，增加了页面样式，旨在帮助用户更高效地开启备份之旅（华为云，2024）
- **阿里云云备份新功能发布**：2025年9月，阿里云推出了ECS文件备份基础版，面向ECS文件场景，具备基础备份与恢复功能，特点包括一键激活、免配置、每天自动备份等，适合中小企业或开发者使用（阿里云，2025）
- **YashanDB数据库备份恢复策略与实操指南**：2025年6月，数据库砖家在InfoQ写作社区发布了关于YashanDB数据库备份与恢复的策略和实操指南，详细介绍了YashanDB的备份类型、策略实施细节和实际操作步骤（InfoQ，2025）
- **信创背景下企业数据库备份的选型、规划与部署实施**：2025年1月，信息化观察网发布了关于信创背景下企业数据库备份选型、规划与部署实施的文章，分析了备份产品选型的重要关键点，并对主流国产数据备份软件的特点进行了对比（信息化观察网，2025）
- **ResBench：评估数据库弹性的综合框架**：2025年11月，研究人员提出了ResBench，一个用于评估数据库弹性的综合框架。该框架通过模拟不利事件并在正常事务处理中注入，评估数据库在面对不利事件时的性能，提供了自动化、标准化和可视化的测试过程（arXiv:2511.11088，2025）
...
- [pgBackRest](https://pgbackrest.org/) - 企业级备份
- [Barman](https://pgbarman.org/) - 灾备工具

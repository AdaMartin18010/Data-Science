...
- [SQL:2023标准详解](#sql2023标准详解)
  ...
  - [六、实践应用](#六实践应用)
    ...
    - [6.3 PostgreSQL 18实战：SQL:2023新特性应用](#63-postgresql-18实战sql2023新特性应用)
  - [七、相关资源](#七相关资源)
    - [相关文档](#相关文档)
    - [外部资源](#外部资源)
...
### 6.3 PostgreSQL 18实战：SQL:2023新特性应用

**Property Graph查询（SQL:2023 Part 16）**：

```sql
-- PostgreSQL 18 + Apache AGE扩展：图查询
...
-- SQL:2023风格图遍历查询
WITH RECURSIVE friends(person_id, friend_id, degree) AS (
    -- 使用图边表
    SELECT from_id, to_id, 1
    FROM social_graph_edges
    WHERE from_id = 1  -- Alice

    UNION ALL

    SELECT f.person_id, e.to_id, f.degree + 1
    FROM friends f
    JOIN social_graph_edges e ON f.friend_id = e.from_id
    WHERE f.degree < 3  -- 3度好友内
)
...
**MERGE语句实战（SQL:2023增强）**：

```sql
-- PostgreSQL 15+支持MERGE（SQL:2023标准）
...
WHEN MATCHED THEN
    UPDATE SET
        name = source.name,
        price = source.price,
        stock = target.stock + source.stock_change,
...
-- 返回受影响的行
-- PostgreSQL 18: MERGE支持RETURNING
MERGE INTO products AS target
USING staging_products AS source
ON target.product_id = source.product_id
WHEN MATCHED THEN
...
RETURNING product_id, name, xmax_action;  -- xmax_action显示操作类型
```

**JSON_TABLE增强（PostgreSQL 18）**：
...
SELECT
    ...
    RANK() OVER w AS rank,
    SUM(total_amount) OVER w AS cumulative_total,
...
CREATE FUNCTION generate_time_series(
    start_time TIMESTAMPTZ,
    end_time TIMESTAMPTZ,
    step_interval INTERVAL
) RETURNS TABLE (ts TIMESTAMPTZ)
LANGUAGE SQL
AS $$
    SELECT generate_series(start_time, end_time, step_interval);
$$;

-- 使用
SELECT ts
FROM generate_time_series(

...
- [数据查询语言(DQL)](#数据查询语言dql)
  ...
  - [十一、相关资源](#十一相关资源)
    ...
    - [外部资源](#外部资源)
      ...
      - [标准文档](#标准文档)
      - [在线资源](#在线资源)
  - [十二、PostgreSQL 18 DQL新特性 🆕](#十二postgresql-18-dql新特性-)
    - [12.1 LATERAL JOIN增强](#121-lateral-join增强)
    - [12.2 窗口函数增强](#122-窗口函数增强)
    - [12.3 向量检索查询](#123-向量检索查询)
  - [十三、SQLite 3.45+ DQL新特性 🆕](#十三sqlite-345-dql新特性-)
    - [13.1 窗口函数完整支持](#131-窗口函数完整支持)
    - [13.2 全文搜索(FTS5)](#132-全文搜索fts5)
    - [13.3 JSON查询](#133-json查询)
...
## 十二、PostgreSQL 18 DQL新特性 🆕

### 12.1 LATERAL JOIN增强

```sql
-- PostgreSQL 18: LATERAL JOIN（相关子查询作为表）
SELECT
    c.customer_name,
    recent_orders.*
FROM customers c
...
```sql
-- PostgreSQL 18: FILTER子句配合窗口函数
SELECT
    sale_date,
    amount,
    SUM(amount) OVER (ORDER BY sale_date) AS running_total,
    SUM(amount) FILTER (WHERE category = 'electronics')
        OVER (ORDER BY sale_date) AS electronics_running_total;
...
```sql
-- PostgreSQL 18 + pgvector: 向量相似度搜索
SELECT
    id, title, content,
...
SELECT
    id, title,
    ts_rank_cd(tsv, query) AS text_score,
    1 - (embedding <=> query_vec) AS vector_score
FROM documents, plainto_tsquery('search term') query
WHERE tsv @@ query
ORDER BY text_score * 0.3 + (1 - (embedding <=> query_vec)) * 0.7 DESC;
```

---

## 十三、SQLite 3.45+ DQL新特性 🆕
...
FROM articles_fts
WHERE articles_fts MATCH 'sql optimization';
...
SELECT
    u.id,
    t.value AS tag
FROM users u, json_each(json_extract(u.data, '$.tags')) t;
```

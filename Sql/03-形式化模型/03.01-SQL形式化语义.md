...
- [SQL形式化语义](#sql形式化语义)
  ...
  - [二、SQL语法形式化定义](#二sql语法形式化定义)
    - [2.1 BNF语法定义](#21-bnf语法定义)
      - [2.1.1 BNF语法概述](#211-bnf语法概述)
      - [2.1.2 SQL语法BNF定义](#212-sql语法bnf定义)
      - [2.1.3 BNF语法历史背景](#213-bnf语法历史背景)
    - [2.2 EBNF扩展语法](#22-ebnf扩展语法)
      - [2.2.1 EBNF语法概述](#221-ebnf语法概述)
      - [2.2.2 SQL语法EBNF定义](#222-sql语法ebnf定义)
      - [2.2.3 EBNF与BNF的对比](#223-ebnf与bnf的对比)
  - [三、SQL语义形式化定义](#三sql语义形式化定义)
    - [3.1 语义函数定义](#31-语义函数定义)
      - [3.1.1 语义函数概述](#311-语义函数概述)
      - [3.1.2 语义函数的历史背景](#312-语义函数的历史背景)
  ...
  - [六、执行语义形式化](#六执行语义形式化)
    - [6.1 执行顺序](#61-执行顺序)
    - [6.2 执行语义](#62-执行语义)
  - [七、2025向量检索形式化语义](#七2025向量检索形式化语义)
    - [7.1 向量类型形式化定义](#71-向量类型形式化定义)
    - [7.2 向量选择操作形式化](#72-向量选择操作形式化)
    - [7.3 混合查询形式化语义](#73-混合查询形式化语义)
    - [7.4 向量检索执行语义](#74-向量检索执行语义)
    - [7.5 向量检索形式化流程图](#75-向量检索形式化流程图)
    - [7.6 2025技术对比矩阵](#76-2025技术对比矩阵)
  - [八、相关资源](#八相关资源)
    - [相关文档](#相关文档)
    - [外部资源](#外部资源)
...
设 Vector(n) 为n维向量类型，则：
- Vector(n) ⊆ ℝⁿ
- 对于 v ∈ Vector(n)，v = (v₁, v₂, ..., vₙ)
- 向量为原子类型（不可再分）
```
...
设 d: Vector(n) × Vector(n) → ℝ⁺ 为距离函数，满足：
1. 非负性：d(u, v) ≥ 0
2. 同一性：d(u, v) = 0 ⟺ u = v
3. 对称性：d(u, v) = d(v, u)
...
常用距离：
- 欧氏距离：d_L2(u, v) = √(Σᵢ(uᵢ - vᵢ)²)
- 余弦距离：d_cos(u, v) = 1 - (u·v)/(||u|| × ||v||)
- 内积距离：d_ip(u, v) = -u·v
```
...
σ_knn[k, q, A](R) = {t ∈ R | |{t' ∈ R | d(t'[A], q) < d(t[A], q)}| < k}
...
σ_hybrid(R) = σ_rel(σ_knn[k, q, A](R))

语义函数：
⟦σ_hybrid(R)⟧ = {t | t ∈ ⟦σ_knn⟧ ∧ ⟦P(t)⟧ = True}
...
```sql
-- 混合查询的SQL形式化对应
-- 语义：σ_rel(σ_knn[10, query_vec, embedding](documents))
SELECT id, title,
       1 - (embedding <=> query_vec) AS similarity
FROM documents
WHERE category = 'tech'        -- σ_rel
  AND status = 'published'     -- σ_rel
ORDER BY embedding <=> query_vec  -- σ_knn排序
LIMIT 10;                      -- k=10
```
...
```latex
扩展执行顺序：exec_order_vector

exec_order_vector(SELECT ... FROM ... WHERE ... ORDER BY vec_col <=> q LIMIT k) =
    1. FROM (关系获取)
    2. WHERE (关系选择)
    3. VECTOR_SCAN (向量索引扫描)
    4. KNN_FILTER (K近邻过滤)
    5. SELECT (投影)
    6. LIMIT (截取)
```
...
```latex
execute_vector: SQL_Vector_Statement × Database → Result

execute_vector(SELECT A FROM R WHERE P ORDER BY R.vec <=> q LIMIT k, DB) =
    let filtered = {t | t ∈ DB(R) ∧ ⟦P(t)⟧ = True}
    let ranked = rank_by_distance(filtered, q, R.vec)
    return top_k(ranked, k)[A]
```
...
| 特性 | 传统SQL语义 | 向量扩展语义 | 形式化复杂度 |
|------|------------|-------------|-------------|
| **选择操作** | 精确谓词 | 近似KNN | O(n) → O(log n) |
| **排序语义** | 确定性 | 距离排序 | 新增度量 |
| **等价性** | 代数等价 | 近似等价 | 放松条件 |
| **优化规则** | 下推/交换 | 混合优化 | 新规则集 |
| **索引语义** | B-tree | HNSW/IVF | 概率保证 |

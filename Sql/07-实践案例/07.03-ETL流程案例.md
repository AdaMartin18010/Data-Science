...
sequenceDiagram
    ...
    Source->>Extract: 提取数据
    Extract->>Extract: 增量/全量判断
    Extract->>Transform: 原始数据

    Transform->>Transform: 数据清洗
    Transform->>Transform: 数据转换
    Transform->>Validate: 转换后数据

    Validate->>Validate: 数据验证
    Validate->>Validate: 质量检查
    Validate->>Load: 验证通过数据

    Load->>Target: 加载数据
    Target-->>Load: 加载结果
    Load-->>Transform: 加载状态
    Transform-->>Extract: 处理状态
    Extract-->>Source: 提取状态
```
...
sequenceDiagram
    ...
    Scheduler->>Extract: 触发ETL任务
    Extract->>SourceDB: 查询增量数据
    SourceDB-->>Extract: 返回订单数据
    Extract->>Staging: 写入临时表
    Staging-->>Extract: 写入完成

    Extract->>Transform: 通知转换开始
    Transform->>Staging: 读取临时数据
    Transform->>Transform: 数据清洗转换
    Transform->>TargetDB: 写入目标表
    TargetDB-->>Transform: 写入完成

    Transform->>Extract: 更新提取时间
    Extract-->>Scheduler: ETL完成
```
